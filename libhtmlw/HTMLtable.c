        ...
        for (y = 0; y < t->numRows; y++) {
            ...
            for (x = 0; x < t->numColumns; x++) {
                fprintf(stderr, "colWidth=%d,rowHeight=%d | ",
                        t->table[y * t->numColumns + x].colWidth, t->table[y * t->numColumns + x].rowHeight);
            }
            fprintf(stderr, "\n---------------------------------------------------\n");
        }
...
List tableList;
{
    ...
    while (rowList) {
        ...
        field = (TableField *) ListHead(rowList);
        while (field) {
            memcpy(&(t->table[y * t->numColumns + x]), field, sizeof(TableField));
            x++;
            field = (TableField *) ListNext(rowList);
        }
        y++;
...
int pageWidth;                  /* width in pixels of output page */
{
    ...
    for (y = 0; y < t->numRows; y++) {
        ...
        for (x = 0; x < t->numColumns; x++) {
            ...
            if (field->type == F_TEXT) {
                field->maxWidth = XTextWidth(field->font, field->text, strlen(field->text));
        ...
        sumMaxWidth = (sumMaxWidth > maxWidthOfRow) ? sumMaxWidth : maxWidthOfRow;
        sumMinWidth = (sumMinWidth > minWidthOfRow) ? sumMinWidth : minWidthOfRow;
    }
            ...
            if (numAdjacent) {
                ...
                for (xx = x; xx < x + numAdjacent; xx++) {
                    t->table[y * t->numColumns + xx].maxWidth
                        = t->table[y * t->numColumns + x].maxWidth / (numAdjacent + 1);
                }
            }
            x += numAdjacent;
        }
            ...
            if (numAdjacent) {
                ...
                for (yy = y; yy < y + numAdjacent; yy++) {
                    t->table[yy * t->numColumns + x].minHeight
                        = t->table[y * t->numColumns + x].minHeight / (numAdjacent + 1);
                }
            }
            y += numAdjacent;
        }
    ...
    if (sumMaxWidth < pageWidth) {
        ...
        for (x = 0; x < t->numColumns; x++) {
            ...
            for (y = 0; y < t->numRows; y++) {
                t->table[y * t->numColumns + x].colWidth = maxWidthOfColumn + 2 * FIELD_BORDER_SPACE;
            }
        }
        for (y = 0; y < t->numRows; y++) {
            ...
            for (x = 0; x < t->numColumns; x++) {
                maxHeightOfRow =
                    ...
                    maxHeightOfRow : t->table[y * t->numColumns + x].minHeight;
            }
            /* assign uniform height to row */
            for (x = 0; x < t->numColumns; x++) {
                t->table[y * t->numColumns + x].rowHeight = maxHeightOfRow + 2 * FIELD_BORDER_SPACE;
            }
        }
        ...
        for (y = 0; y < t->numRows; y++) {
            for (x = 0; x < t->numColumns; x++) {
                field = &(t->table[y * t->numColumns + x]);
                field->formattedText = (char **)malloc(sizeof(char *));
                if (field->text) {
...
*/
            ...
            for (y = 0; y < t->numRows; y++) {
                field = &(t->table[y * t->numColumns + x]);
                field->colWidth = (int)(percentToShrink * ((float)
                                                           CalculateMaxWidthOfColumn(t, x)));
                field->rowHeight = 0;
                numAdjacent = TableHowManyConnectedHorizFields(t, x, y);
                /* calculate the width including connected */
                accumulateColWidth = field->colWidth;
                for (xx = x + 1; xx < x + numAdjacent + 1; xx++) {
                    accumulateColWidth += ((percentToShrink * ((float)
                                                               CalculateMaxWidthOfColumn(t, xx))));
                }
                ...
                PourText(field->text, field->font,
                         accumulateColWidth,
                         &(field->rowHeight), hw->html.percent_vert_space, &(field->formattedText), &(field->numLines));

#ifndef DISABLE_TRACE
                if (htmlwTrace) {
                    fprintf(stderr,
                            "poured field %d,%d is dims %d,%d: %%shrink=%f\n",
                ...
                if (numAdjacent) {
                    ...
                    for (xx = x; xx < x + numAdjacent; xx++) {
                        t->table[y * t->numColumns + xx].colWidth
                            = t->table[y * t->numColumns + x].colWidth / (numAdjacent + 1);
                    }
                }
                x += numAdjacent;
            }
                ...
                if (numAdjacent) {
                    ...
                    for (yy = y; yy < y + numAdjacent; yy++) {
                        t->table[yy * t->numColumns + x].rowHeight
                            = t->table[y * t->numColumns + x].rowHeight / (numAdjacent + 1);
                    }
                }
                y += numAdjacent;
            }
        ...
        for (y = 0; y < t->numRows; y++) {
            ...
            for (x = 0; x < t->numColumns; x++) {
                maxHeightOfRow = (maxHeightOfRow >
                                  t->table[y * t->numColumns +
                                           x].rowHeight) ? maxHeightOfRow : t->table[y * t->numColumns + x].rowHeight;
            }
            /* assign height */
            for (x = 0; x < t->numColumns; x++) {
                t->table[y * t->numColumns + x].rowHeight = maxHeightOfRow;
            }
        }
        ...
        for (x = 0; x < t->numColumns; x++) {
            ...
            for (y = 0; y < t->numRows; y++) {
                maxWidthOfColumn = (maxWidthOfColumn >
                                    t->table[y * t->numColumns +
                                             x].colWidth) ? maxWidthOfColumn : t->table[y * t->numColumns + x].colWidth;
            }
...
int *columnCount;
{
    ...
    if (rowCount > 1) {
        ...
        previousRow = (List) ListGetIndexedEntry(tableList, rowCount - 2);  /*zero indexed */
...
int width, height;              /* space allowed for displaying */
{
    ...
    for (yy = 0; yy < field->numLines; yy++) {
        stringWidth = XTextWidth(field->font, field->formattedText[yy], strlen(field->formattedText[yy]));
...
*/
        ...
        XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, 1, LineSolid, CapNotLast, JoinMiter);
        ...
        XDrawString(XtDisplay(hw), XtWindow(hw->html.view),
                    ...
                    field->formattedText[yy], strlen(field->formattedText[yy]));

        placeY += lineHeight;
    }

    XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, eptr->table_data->borders, LineSolid, CapNotLast, JoinMiter);
...
int *expandWidth, *expandHeight;    /* returned */
{
    ...
    *expandHeight = t->table[y * t->numColumns + x].rowHeight;
    ...
    if (x < t->numColumns) {
        /* do width */
        while ((x < t->numColumns) && t->table[y * t->numColumns + x].contHoriz) {
            (*expandWidth) += t->table[y * t->numColumns + x].colWidth;
            x++;
        }
    }
    ...
    if (y < t->numRows) {
        /* do height */
        while ((y < t->numRows) && t->table[y * t->numColumns + x].contVert) {
            (*expandHeight) += t->table[y * t->numColumns + x].rowHeight;
            y++;
        }
    }
        ...
        for (xx = 0; xx < eptr->table_data->numColumns; xx++) {
            ...
            if (eptr->table_data->borders) {
                if (!field->contVert) { /* draw above line */
                    XDrawLine(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC,
                              /*hw->manager.bottom_shadow_GC, */
                              vertMarker, horizMarker, vertMarker + colWidth, horizMarker);
/*
...
*/
                ...
                if (!field->contHoriz) {    /* draw left side */
                    XDrawLine(XtDisplay(hw),
                              XtWindow(hw->html.view),
                              hw->html.drawGC, vertMarker, horizMarker, vertMarker, horizMarker + rowHeight);
                }
            }
            TableGetExpandedDimensions(eptr->table_data, xx, yy, &expandedWidth, &expandedHeight);
            /* fill in field */
            TableDisplayField(hw, eptr, field, vertMarker, horizMarker, expandedWidth, expandedHeight);
