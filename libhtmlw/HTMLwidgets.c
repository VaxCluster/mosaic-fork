...
void setTextSize(w, columns, lines)
Widget w;
int columns;
int lines;
{
    ...
    XtVaGetValues(w, XtNfont, &font,
                  XtNleftMargin, &lm, XtNrightMargin, &rm, XtNtopMargin, &tm, XtNbottomMargin, &bm, NULL);
...
void CBListDestroy(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
    ...
    XtVaGetValues(w, XtNlist, string_list, XtNnumberStrings, &item_count,   /*ddt4/3/95 */
                  NULL);
...
void CBTextDestroy(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
...
void CBoption(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
...
int CollectSubmitInfo(fptr, name_list, value_list)
FormInfo *fptr;
char ***name_list;
char ***value_list;
{
    ...
    cbdata.attribute_names = (char **)malloc(cbdata.attribute_count * sizeof(char *));
    cbdata.attribute_values = (char **)malloc(cbdata.attribute_count * sizeof(char *));
    ...
    /***   cvarela@ncsa.uiuc.edu:  August 17, 1994
        ...
        if (wptr->name) {
            ...
            case W_TEXTFIELD:
#ifdef MOTIF
                cbdata.attribute_values[cnt] = XmTextFieldGetString(wptr->w);
#else
                XtVaGetValues(wptr->w, XtNstring, &(cbdata.attribute_values[cnt]), NULL);
#endif                          /* MOTIF */
                if ((cbdata.attribute_values[cnt] != NULL) && (cbdata.attribute_values[cnt][0] == '\0')) {
                ...
                if (list_cnt == 0) {
                    ...
                    for (j = 0; j < list_cnt; j++) {
                        cbdata.attribute_names[cnt + j]
                            = wptr->name;
#ifdef MOTIF
                        XmStringGetLtoR(str_list[j], XmSTRING_DEFAULT_CHARSET, &val);
#else
                        val = str_list[j];
#endif                          /* MOTIF */
                        if ((val != NULL) && (val[0] == '\0')) {
            ...
            case W_JOT:
                argcnt = 0;
                XtSetArg(arg[argcnt], XmNuserData, (XtPointer *) & child);
                argcnt++;
                XtGetValues(wptr->w, arg, argcnt);
                cbdata.attribute_values[cnt] = EJB_JOTfromJot(child);
                break;
#endif                          /* MOTIF */
            default:
                cbdata.attribute_values[cnt] = NULL;
...
void ImageSubmitForm(fptr, event, name, x, y)
FormInfo *fptr;
XEvent *event;
char *name;
int x, y;
{
    ...
    if ((name != NULL) && (name[0] != '\0')) {
        cbdata.attribute_names[cnt] = (char *)malloc(strlen(name) + strlen(X_NAME) + 2);
    ...
    cnt++;
    if ((name != NULL) && (name[0] != '\0')) {
        cbdata.attribute_names[cnt] = (char *)malloc(strlen(name) + strlen(Y_NAME) + 2);
        strcpy(cbdata.attribute_names[cnt], name);
        strcat(cbdata.attribute_names[cnt], ".");
...
void CBSubmitForm(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
    ...
    WbFormCallbackData cbdata;
#ifdef MOTIF
    XmPushButtonCallbackStruct *pb = (XmPushButtonCallbackStruct *) call_data;
#endif                          /* MOTIF */
    ...
    cbdata.attribute_count = CollectSubmitInfo(fptr, &cbdata.attribute_names, &cbdata.attribute_values);
 ...
 */
void CBChangeRadio(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
    ...
    int cnt, count;
#ifdef MOTIF
    XmToggleButtonCallbackStruct *tb = (XmToggleButtonCallbackStruct *) call_data;
#else
    Boolean state;
#endif                          /* MOTIF */
    ...
    while ((wptr != NULL) && (cnt < count)) {
        ...
        if ((wptr->type == W_RADIOBOX) &&
            ...
            (wptr->name != NULL) && (name != NULL) && (strcmp(wptr->name, name) == 0)) {
            XmToggleButtonSetState(wptr->w, False, False);
        }
#else
        if ((wptr->type == W_RADIOBOX) &&
            (wptr->w != w) && (wptr->name != NULL) && (name != NULL) && (strcmp(wptr->name, name) == 0)) {
            XtVaGetValues(wptr->w, XtNstate, &state, NULL);
            if (state) {
 ...
 */
void CBPasswordModify(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
 ...
 */
void CBActivateField(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
...
void CBResetForm(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
...
#else
            XtVaGetValues(wptr->w, XtNuseStringInPlace, &stringInPlace, XtNlength, &length, NULL);
            if (stringInPlace) {
                XtVaGetValues(wptr->w, XtNstring, &txt, NULL);
            }
            if (wptr->value == NULL) {
                if (stringInPlace) {
                    if (txt)
                        *txt = '\0';
                    XtVaSetValues(wptr->w, XtNstring, txt, NULL);
                } else {
...
#else
            XtVaSetValues(wptr->w, XtNstring, wptr->value ? wptr->value : "", NULL);
#endif                          /* MOTIF */
            break;
        case W_PASSWORD:
            if (wptr->value == NULL) {
...
#else
                XtVaSetValues(wptr->w, XtNstring, wptr->password, NULL);
#endif                          /* MOTIF */
        ...
        case W_LIST:
            {
                ...
                XtVaGetValues(wptr->w, XtNchildren, &wl, NULL);
                child = *++wl;
                XtVaGetValues(child, XtNlist, &string_list, XtNnumberStrings, &list_cnt, NULL);
#endif                          /* MOTIF */
                ...
                if (wptr->value != NULL) {
                    ...
                    if (vlist_cnt > 0) {
                        ...
                        argcnt++;
                        XtSetArg(arg[argcnt], XmNselectedItemCount, vlist_cnt);
                        argcnt++;
...
void PrepareFormEnd(hw, w, fptr)
HTMLWidget hw;
Widget w;
FormInfo *fptr;
{
#ifdef MOTIF
    XtAddCallback(w, XmNactivateCallback, (XtCallbackProc) CBSubmitForm, (caddr_t) fptr);
#else
    XtAddCallback(w, XtNcallback, (XtCallbackProc) CBSubmitForm, (caddr_t) fptr);
#endif                          /* MOTIF */
}

void PrepareFormReset(hw, w, fptr)
HTMLWidget hw;
Widget w;
FormInfo *fptr;
{
#ifdef MOTIF
    XtAddCallback(w, XmNactivateCallback, (XtCallbackProc) CBResetForm, (caddr_t) fptr);
#else
...
HTMLWidget hw;
{
     ...
     */
    while (XCheckWindowEvent(XtDisplay(hw->html.view), XtWindow(hw->html.view), ExposureMask, &event) == True) {
    }
}
...
Boolean AlreadyChecked(hw, fptr, name)
HTMLWidget hw;
FormInfo *fptr;
char *name;
{
    ...
    while (wptr != NULL) {
        if ((wptr->id >= fptr->start) &&
            ...
            (wptr->checked == True) && (wptr->name != NULL) && (name != NULL) && (strcmp(wptr->name, name) == 0)) {
            radio_checked = True;
            break;
        }
        wptr = wptr->next;
...
WidgetInfo *AddNewWidget(hw, fptr, w, type, id, x, y, width, height, name, value, mapping, checked)
HTMLWidget hw;
FormInfo *fptr;
Widget w;
int type;
 ...
 */
XFontStruct *GetWidgetFont(hw, wptr)
HTMLWidget hw;
WidgetInfo *wptr;
{
    Widget child;
    ...
    if (wptr->type == W_OPTIONMENU) {
        ...
        argcnt = 0;
        XtSetArg(arg[argcnt], XmNfontList, &font_list);
        argcnt++;
        XtGetValues(child, arg, argcnt);
#else
 ...
 */
char *ComposeCommaList(list, cnt)
char **list;
int cnt;
{
    int i;
 ...
 * lone ' back to "
 */
void UnMuckTextAreaValue(value)
char *value;
{
    char *tptr;
...
char *MapOptionReturn(val, mapping)
char *val;
char **mapping;
{
    int cnt;
...
char **MakeOptionMappings(list1, list2, list_cnt)
char **list1;
char **list2;
int list_cnt;
{
 ...
 */
WidgetInfo *MakeWidget(hw, text, x, y, id, fptr)
HTMLWidget hw;
char *text;
int x, y;
int id;
FormInfo *fptr;
{
    ...
    if (wlist == NULL) {
        ...
        if ((type_str != NULL)
            && (my_strcasecmp(type_str, "checkbox") == 0)) {
            XmString label;
            ...
            w = XmCreateToggleButton(hw->html.view, widget_name, arg, argcnt);

            if (!hw->html.focus_follows_mouse) {
                XtOverrideTranslations(w, XtParseTranslationTable(traversal_table));
                XtOverrideTranslations(w, XtParseTranslationTable("<Btn1Down>: Arm() traversal_current()"));
            }
            ...
            XmStringFree(label);
                   ...
                   && (my_strcasecmp(type_str, "hidden") == 0)) {
            type = W_HIDDEN;
            value = ParseMarkTag(text, MT_INPUT, "VALUE");
            if (value == NULL) {
                value = (char *)malloc(1);
            ...
            w = NULL;
                   ...
                   && (my_strcasecmp(type_str, "radio") == 0)) {
            XmString label;
            ...
            XtSetMappedWhenManaged(w, False);
            XtManageChild(w);
            XtAddCallback(w, XmNvalueChangedCallback, (XtCallbackProc) CBChangeRadio, (caddr_t) fptr);

            XmStringFree(label);
        } else if ((type_str != NULL)
                   && (my_strcasecmp(type_str, "submit") == 0)) {
            XmString label;
            ...
            w = XmCreatePushButton(hw->html.view, widget_name, arg, argcnt);
            if (!hw->html.focus_follows_mouse)
                XtOverrideTranslations(w, XtParseTranslationTable(traversal_table));
            XtSetMappedWhenManaged(w, False);
            XtManageChild(w);
            ...
            PrepareFormEnd(hw, w, fptr);
                   ...
                   && (my_strcasecmp(type_str, "reset") == 0)) {
            XmString label;
            ...
            PrepareFormReset(hw, w, fptr);
                   ...
                   && (my_strcasecmp(type_str, "button") == 0)) {
            XmString label;
            ...
            argcnt++;
            frame = XmCreateFrame(hw->html.view, "Frame", arg, argcnt);
            ...
            argcnt++;
            w = XmCreateDrawingArea(frame, widget_name, arg, argcnt);
            XtManageChild(w);

            NewJot(w, width, height);
            XtAddEventHandler(w, ExposureMask, 0, EVJotExpose, (XtPointer) hw);
            XtAddEventHandler(w, ButtonPressMask, 0, EVJotPress, (XtPointer) hw);
            XtAddEventHandler(w, ButtonMotionMask, 0, EVJotMove, (XtPointer) hw);
            XtAddEventHandler(w, ButtonReleaseMask, 0, EVJotRelease, (XtPointer) hw);
            ...
            if (label != NULL) {
                XmStringFree(label);
                   ...
                   && (my_strcasecmp(type_str, "select") == 0)) {
            XmString label;
            Widget scroll;
            ...
            if (type == W_OPTIONMENU) {
                ...
                XmString xmstr;
                argcnt = 0;
                pulldown = XmCreatePulldownMenu((Widget) hw->html.view, widget_name, arg, argcnt);
                ...
                for (i = 0; i < list_cnt; i++) {
                    ...
                    XtSetArg(arg[argcnt], XmNlabelString, label);
                    argcnt++;
                    button = XmCreatePushButton(pulldown, bname, arg, argcnt);
                    XtManageChild(button);
                    XmStringFree(label);
                    if (!hw->html.focus_follows_mouse)
                        XtOverrideTranslations(button, XtParseTranslationTable(traversal_table));
                ...
                w = XmCreateOptionMenu((Widget) hw->html.view, widget_name, arg, argcnt);
                if (!hw->html.focus_follows_mouse) {
                    XtOverrideTranslations(w, XtParseTranslationTable(traversal_table));
                    XtOverrideTranslations(pulldown, XtParseTranslationTable(traversal_table));
                }
...
*/
                ...
                string_list = (XmString *) malloc(list_cnt * sizeof(XmString));
                val_list = (XmString *) malloc(vlist_cnt * sizeof(XmString));

                for (i = 0; i < list_cnt; i++) {
                    string_list[i] = XmStringCreateSimple(list[i]);
                }
                for (i = 0; i < vlist_cnt; i++) {
                    val_list[i] = XmStringCreateSimple(vlist[i]);
                }
                ...
                XtSetArg(arg[argcnt], XmNy, y);
                argcnt++;
                scroll = XmCreateScrolledWindow((Widget) hw->html.view, "Scroll", arg, argcnt);
                ...
                argcnt++;
                if (mult) {
                    XtSetArg(arg[argcnt], XmNselectionPolicy, XmEXTENDED_SELECT);
                    argcnt++;
                } else {
                    XtSetArg(arg[argcnt], XmNselectionPolicy, XmBROWSE_SELECT);
                    argcnt++;
                }
                if ((vlist_cnt > 0) && (mult)) {
                    XtSetArg(arg[argcnt], XmNselectedItems, val_list);
                    ...
                    XtSetArg(arg[argcnt], XmNselectedItems, &val_list[0]);
                    argcnt++;
                    XtSetArg(arg[argcnt], XmNselectedItemCount, 1);
                    argcnt++;
                }
            ...
            XtManageChild(w);
                   ...
                   && (my_strcasecmp(type_str, "password") == 0)) {
            type = W_PASSWORD;
            value = ParseMarkTag(text, MT_INPUT, "VALUE");
            ...
            /*XtSetArg(arg[argcnt], XmNnavigationType, XmNONE);
               argcnt++; */
            w = XmCreateTextField(hw->html.view, widget_name, arg, argcnt);
 ...
 */
            ...
            XtSetMappedWhenManaged(w, False);
#endif                          /* MOTIF1_2 */
            XtAddCallback(w, XmNactivateCallback, (XtCallbackProc) CBActivateField, (caddr_t) fptr);
            XtAddCallback(w, XmNmodifyVerifyCallback, (XtCallbackProc) CBPasswordModify, (caddr_t) fptr);
        } else if ((type_str != NULL)
                   && (my_strcasecmp(type_str, "textarea") == 0)) {
            char **list;
            int list_cnt;
            int rows, cols;
            Widget scroll;
            ...
            if (type == W_TEXTFIELD) {
                ...
                w = XmCreateTextField((Widget) hw->html.view, widget_name, arg, argcnt);
                XtOverrideTranslations(w, XtParseTranslationTable(text_translations));
                if (!hw->html.focus_follows_mouse)
                    XtOverrideTranslations(w, XtParseTranslationTable(traversal_table));
            }
             ...
             */
                ...
                XtSetArg(arg[argcnt], XmNy, y);
                argcnt++;
                scroll = XmCreateScrolledWindow(hw->html.view, "Scroll", arg, argcnt);
 ...
 */
        ...
        if (w != NULL) {
            ...
            XtVaSetValues(w,
                          ...
                          XmNbottomShadowColor, hw->html.bottom_color_SAVE, NULL);
        } else {
            width = 0;
            height = 0;
        }

        wptr = AddNewWidget(hw, fptr, w, type, id, x, y, width, height, name, value, mapping, checked);
    } else
        /*
...
FormInfo *fptr;
{
    ...
    if (wlist == NULL) {
        ...
        if ((type_str != NULL)
            ...
            argcnt = 0;
            XtSetArg(arg[argcnt], XtNx, x);
            argcnt++;
            XtSetArg(arg[argcnt], XtNy, y);
            argcnt++;
            if (tptr != NULL) {
                XtSetArg(arg[argcnt], XtNstate, True);
                argcnt++;
                checked = True;
                free(tptr);
            }
            ...
            XtSetMappedWhenManaged(w, False);
            XtManageChild(w);
            XtAddCallback(w, XtNcallback, (XtCallbackProc) CBChangeRadio, (caddr_t) fptr);
        } else if ((type_str != NULL)
                   && (my_strcasecmp(type_str, "submit") == 0)) {
            type = W_PUSHBUTTON;
            value = ParseMarkTag(text, MT_INPUT, "VALUE");
            ...
            w = XtCreateWidget(name, commandWidgetClass, hw->html.view, arg, argcnt);
            XtSetMappedWhenManaged(w, False);
            XtManageChild(w);
            PrepareFormEnd(hw, w, fptr);
        } else if ((type_str != NULL)
            ...
            if (type == W_OPTIONMENU) {
                ...
                XtSetArg(arg[argcnt], XtNy, y);
                argcnt++;
                w = XtCreateWidget(name, menuButtonWidgetClass, hw->html.view, arg, argcnt);
                argcnt = 0;
                pulldown = XtCreatePopupShell("menu", simpleMenuWidgetClass, w, arg, argcnt);
                for (i = 0; i < list_cnt; i++) {
                    char bname[30];
                    ...
                    argcnt++;
                    button = XtCreateWidget(bname, smeBSBObjectClass, pulldown, arg, argcnt);
                    XtManageChild(button);
                    XtAddCallback(button, XtNcallback, CBoption, (XtPointer) w);

                    if (i == 0) {
                        XtVaGetValues(w, XtNfont, &font, XtNinternalWidth, &iW, NULL);
                    }
                    ...
                    if ((vlist_cnt > 0) && (vlist[0] != NULL) && (strcmp(vlist[0], list[i]) == 0)) {
                        hist = button;
                        XtVaSetValues(w, XtNlabel, XtNewString(list[i]), NULL);
                    }
                ...
                argcnt++;
                scroll = XtCreateWidget("Scroll", viewportWidgetClass, hw->html.view, arg, argcnt);
                argcnt = 0;
                XtSetArg(arg[argcnt], XtNdefaultColumns, 1);
                argcnt++;
                w = XtCreateWidget(name, listWidgetClass, scroll, arg, argcnt);
                XtManageChild(w);
                ...
                if (size > 1) {
                    ...
                    XtVaGetValues(w, XtNfont, &font, XtNinternalHeight, &h, XtNwidth, &width, XtNrowSpacing, &s, NULL);
                    XtVaSetValues(scroll, XtNheight, h + size * (s + FONTHEIGHT(font)), XtNwidth, width + 20, NULL);
                }
            ...
            w = XtCreateWidget(name, asciiTextWidgetClass, hw->html.view, arg, argcnt);
            if (maxlength > 0) {
                XtAddCallback(w, XtNdestroyCallback, (XtCallbackProc) CBTextDestroy, (caddr_t) txt);
            }

            XtOverrideTranslations(w, XtParseTranslationTable("<Key>: HTMLpwdInput()"));
            XtOverrideTranslations(w, XtParseTranslationTable("<Key>Return: no-op(RingBell)"));
            ...
            if (type == W_TEXTFIELD) {
                ...
                w = XtCreateWidget(name, asciiTextWidgetClass, hw->html.view, arg, argcnt);
                if (maxlength > 0) {
                    XtAddCallback(w, XtNdestroyCallback, CBTextDestroy, (caddr_t) txt);
                }
...
/* Do it only here instead of both places above and here... -- SWP */
        ...
        if (w != NULL) {
            argcnt = 0;
            XtSetArg(arg[argcnt], XtNwidth, &width);
            argcnt++;
            XtSetArg(arg[argcnt], XtNheight, &height);
            argcnt++;
            XtGetValues(w, arg, argcnt);
        } else {
            width = 0;
        ...
        wptr = AddNewWidget(hw, fptr, w, type, id, x, y, width, height, name, value, (char **)0, checked);
        /* ^^^ ddt 4/3/95 */
    } else
        /*
         * We found this widget on the list of already created widgets.
...
*/
    ...
    if ((eptr->widget_data != NULL) && (eptr->widget_data->mapped == False) && (eptr->widget_data->w != NULL)) {
...
void traversal_forward(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
    HTMLTraverseTabGroups(w, XmTRAVERSE_NEXT_TAB_GROUP);
}

void traversal_back(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
    HTMLTraverseTabGroups(w, XmTRAVERSE_PREV_TAB_GROUP);
}

void traversal_current(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
    HTMLTraverseTabGroups(w, XmTRAVERSE_CURRENT);
}

void traversal_end(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
    Widget top;
...
void HTMLTraverseTabGroups(Widget w, int how)
{
    ...
    case XmTRAVERSE_NEXT_TAB_GROUP:
        ...
        /* automagickly scroll */
        if (XtIsManaged(hw->html.vbar) && (lptr->y > (hw->html.view_height + hw->html.scroll_y - 10))) {
            ...
            XmScrollBarSetValues(hw->html.vbar, amount, ss, in, pg_in);
        }
