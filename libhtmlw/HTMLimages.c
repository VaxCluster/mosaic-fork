    ...
    while (eptr != NULL) {
        if ((eptr->type == E_IMAGE) && (eptr->pic_data != NULL)) {
            ...
            if ((eptr->pic_data->image != None) &&
                ...
                (eptr->pic_data->image != anchored_image.image) && (eptr->pic_data->image != no_image.image)) {
                /*
                 * Don't free internal images
                 */
                if ((eptr->edata != NULL) && (strncmp(eptr->edata, INTERNAL_IMAGE, strlen(INTERNAL_IMAGE)) == 0)) {
                } else {
                    if (eptr->pic_data->image != None) {
...
*/
        ...
        newimage = XCreateImage(dsp,
                                ...
                                depth, ZPixmap, 0, (char *)bit_data, width, height, 8, bytesperline);
        break;
    case 1:
    case 2:
    case 4:
        ...
        *bitp = 0;
        ...
        newimage = XCreateImage(dsp,
                                DefaultVisual(dsp, DefaultScreen(dsp)),
                                depth, ZPixmap, 0, (char *)bit_data, (width + linepad), height, 8, bytesperline);
        break;
        /*
        ...
        for (w = (width * height); w > 0; w--) {
            temp = (((img_info->reds[(int)*datap] >> rshift) &
                    ...
                    ((img_info->greens[(int)*datap] >> gshift) &
                     theVisual->green_mask) | ((img_info->blues[(int)*datap] >> bshift) & theVisual->blue_mask));
    ...
    if (delayed_image.image == None) {
        ...
        delayed_image.image =
            XCreatePixmapFromBitmapData(XtDisplay(hw->html.view),
                                        ...
                                        DelayedImage_bits, DelayedImage_width, DelayedImage_height,
/*
 ...
 */
                                        ...
                                        hw->html.foreground,
#endif                          /* MOTIF */
                                        hw->core.background_pixel, DefaultDepthOfScreen(XtScreen(hw)));
    }
    ...
    if ((anchored == True) && (anchored_image.image == None)) {
        ...
        anchored_image.image =
            XCreatePixmapFromBitmapData(XtDisplay(hw->html.view),
                                        XtWindow(hw->html.view),
                                        AnchoredImage_bits, AnchoredImage_width, AnchoredImage_height,
/*
 * Without motif we use our own foreground resource instead of
 * using the manager's
 */
        ...
        pix = XCreatePixmap(XtDisplay(hw->html.view),
                            ...
                            (DelayedImage_height + AnchoredImage_height +
                             IMAGE_DEFAULT_BORDER), DefaultDepthOfScreen(XtScreen(hw)));
        XSetForeground(XtDisplay(hw), hw->html.drawGC, hw->core.background_pixel);
        XFillRectangle(XtDisplay(hw->html.view), pix,
                       hw->html.drawGC, 0, 0,
                       DelayedImage_width, (DelayedImage_height + AnchoredImage_height + IMAGE_DEFAULT_BORDER));
        XCopyArea(XtDisplay(hw->html.view),
                  anchored_image.image, pix, hw->html.drawGC, 0, 0, AnchoredImage_width, AnchoredImage_height, 0, 0);
        XCopyArea(XtDisplay(hw->html.view),
                  delayed_image.image, pix, hw->html.drawGC,
                  0, 0, DelayedImage_width, DelayedImage_height, 0, (AnchoredImage_height + IMAGE_DEFAULT_BORDER));
...
Boolean anchored;
{
    ...
    if (anchored == True) {
        ...
        anchored_image.height = DelayedImage_height + AnchoredImage_height + IMAGE_DEFAULT_BORDER;
...
HTMLWidget hw;
{
    if (no_image.image == None) {
        ...
        no_image.image = XCreatePixmapFromBitmapData(XtDisplay(hw),
                                                     XtWindow(hw->html.view),
                                                     NoImage_bits, NoImage_width, NoImage_height,
/*
 * Without motif we use our own foreground resource instead of
 * using the manager's
 */
...
int clip;
{
    ...
    vinfo.visualid = XVisualIDFromVisual(DefaultVisual(XtDisplay(hw), DefaultScreen(XtDisplay(hw))));
    ...
    if (clip) {
        ...
        need_to_dither = True;
        black_pixel = BlackPixel(XtDisplay(hw), DefaultScreen(XtDisplay(hw)));
        white_pixel = WhitePixel(XtDisplay(hw), DefaultScreen(XtDisplay(hw)));
    } else {
        need_to_dither = False;
    }
    XFree((char *)vptr);
        ...
        for (i = 0; i < img_info->num_colors; i++) {
            ...
            if ((Vclass == TrueColor) || (Vclass == DirectColor)) {
                ...
                Mapping[i] = ((tmpcolr.red >> 5) * 11 +
                              (tmpcolr.green >> 5) * 16 + (tmpcolr.blue >> 5) * 5) / (65504 / 64);
            } else {
                FindColor(XtDisplay(hw),
                          (installed_colormap ? installed_cmap : DefaultColormapOfScreen(XtScreen(hw))), &tmpcolr);
    ...
    if (tmpdata == NULL) {
        ...
        if (need_to_dither == True) {
            ...
            if (clip) {
                for (ptr2 = tmpdata, ptr = img_info->clip_data; ptr2 < tmpdata + (size - 1); ptr2++, ptr++) {
                    *ptr2 = Mapping[(int)*ptr];
                }
            } else {
                for (ptr2 = tmpdata, ptr = img_info->image_data; ptr2 < tmpdata + (size - 1); ptr2++, ptr++) {
                    *ptr2 = Mapping[(int)*ptr];
                }
            }
        ...
        tmpimage = MakeImage(XtDisplay(hw), tmpdata, img_info->width, img_info->height, depth, img_info, clip);

        /* Caught by Purify; should be OK. */
        free(tmpdata);

        Img = XCreatePixmap(XtDisplay(hw), XtWindow(hw->html.view), img_info->width, img_info->height, depth);
    }
    ...
    if ((tmpimage == NULL) || (Img == None)) {
        ...
        XPutImage(XtDisplay(hw), Img, hw->html.drawGC, tmpimage, 0, 0, 0, 0, img_info->width, img_info->height);
        XDestroyImage(tmpimage);
    }
