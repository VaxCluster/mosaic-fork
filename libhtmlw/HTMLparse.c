...
unsigned char map_table[256] = {
    ...
    114, 115,
    116, 117, 118, 119, 120, 121, 122, 91, 92, 93, 94, 95, 96, 97, 98, 99,
    100, 101, 102,
 ...
 * return 1 if equal, 0 otherwise.
 */
int caseless_equal_prefix(str1, str2, cnt)
char *str1;
char *str2;
int cnt;
{
 ...
 */
char ExpandEscapes(esc, endp, termination)
char *esc;
char **endp;
int termination;
{
    ...
    if (*esc == '#') {
        ...
        while (AmpEscapes[cnt].tag != NULL) {
            ...
            if ((escLen == ampLen)
                ...
                *endp = (char *)(esc + strlen(AmpEscapes[cnt].tag));
                break;
            }
            cnt++;
        }
...
char *txt;
{
    ...
    ptr = txt;
    while (*ptr != '\0') {
        if ((*ptr == '&') && ((isalpha((int)*(ptr + 1))) || (*(ptr + 1) == '#'))) {
            break;
        }
        ptr++;
    }
    ...
    while (*ptr != '\0') {
        ...
        *ptr = tchar;
         ...
         */
        val = ExpandEscapes(text, &tend, ((space_terminated << 1) + unterminated));
        if (val != '\0') {
            if (unterminated) {
                tchar = *tend;
                *tend = '\0';
 ...
 */
struct mark_up *get_mark(start, endp)
char *start;
char **endp;
{
    char *ptr;
 ...
 */
char *get_plain_text(start, endp)
char *start;
char **endp;
{
    char *ptr;
    ...
    while (*ptr != '\0') {
        ...
        if ((*ptr == '<') &&
            ...
            mp = get_mark(ptr, &ep);
            if (mp != NULL) {
                if (((mp->type == M_PLAIN_TEXT) || (mp->type == M_LISTING_TEXT)) && (mp->is_end)) {
                    if (mp->end != NULL) {
                        free((char *)mp->end);
                    }
                    free((char *)mp);
 ...
 */
struct mark_up *HTMLParse(old_list, str, hw)
    ...
    while (*start != '\0') {
        ...
        if ((mark != NULL) && (mark->type == M_PLAIN_FILE) && (!mark->is_end)) {
            ...
            text = tptr;
                   ...
                   ((mark->type == M_PLAIN_TEXT) || (mark->type == M_LISTING_TEXT)) && (!mark->is_end)) {
...
/* ABSOLUTE CHEEZE OF THE FINEST KIND - bjs - 9/21/95 */
            if (mark->type == M_DOC_BODY && mark->start) {
                ...
                if (!NoBodyImages(hw)) {
                    tmp_bgname = ParseMarkTag(mark->start, MT_DOC_BODY, "background");
                }
                if (!NoBodyColors(hw)) {
                    for (i = 0; atts[i]; i++) {
                        tmp = ParseMarkTag(mark->start, MT_DOC_BODY, atts[i]);
                        if (tmp) {
 ...
 */
char *AnchorTag(ptrp, startp, endp)
char **ptrp;
char **startp;
char **endp;
{
