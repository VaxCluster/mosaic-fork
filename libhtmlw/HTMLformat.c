 ...
 */
struct ele_rec *CreateElement(hw, type, fp, x, y, edata, w, h, bw)
HTMLWidget hw;
int type;
XFontStruct *fp;
int x, y;
...
int bw;
{
    ...
    case E_TEXT:
        ...
        if (AnchorText != NULL) {
            ...
            eptr->anchorSubject = ParseMarkTag(AnchorText, MT_ANCHOR, AT_SUBJECT);
            if (!eptr->anchorSubject) {
                eptr->anchorSubject = ParseMarkTag(AnchorText, MT_ANCHOR, AT_TITLE);
            }
        } else {
    ...
    case E_BULLET:
        ...
        if (BaseLine == -100) {
            ...
            if (LineBottom == 0) {
                 ...
                 * BaseLine
                 */
                if ((LineHeight - baseline) > (LineBottom - baseline)) {
                    LineBottom = LineHeight - baseline;
                } else {
    ...
    case E_LINEFEED:
        ...
        if (AnchorText != NULL) {
            ...
            if ((Current != NULL) &&
                ((Current->anchorHRef == NULL) || (tptr == NULL) || (strcmp(Current->anchorHRef, tptr) != 0))) {
                if (tptr)
                    free(tptr);
                eptr->anchorHRef = NULL;
                eptr->anchorName = NULL;
 ...
 */
    ...
    case E_IMAGE:
        ...
        if (hw->html.resolveImage != NULL) {
             ...
             */
            if ((edata != NULL) && (strncmp(edata, INTERNAL_IMAGE, strlen(INTERNAL_IMAGE)) == 0)) {
                internal = 1;
            } else {
                internal = 0;
            }
             ...
             */
            if (((hw->html.delay_images == True) && (!internal)) || currently_delaying_images == 1) {
                /*
                 ...
                 */
                    ...
                    if (eptr->anchorHRef == NULL) {
                        eptr->pic_data = DelayedImageData(hw, False);
                     ...
                     */
                        ...
                        eptr->pic_data = DelayedImageData(hw, True);
                        eptr->pic_data->delayed = 1;
                    }
                }
            } else {
...
*/
        ...
        eptr->table_data = MakeTable(hw, edata, (x + IMAGE_DEFAULT_BORDER), (y + IMAGE_DEFAULT_BORDER));
    ...
    case E_WIDGET:
        ...
        eptr->widget_data = MakeWidget(hw, edata,
                                       (x + IMAGE_DEFAULT_BORDER), (y + IMAGE_DEFAULT_BORDER), WidgetId, CurrentForm);
 ...
 */
void SetElement(hw, type, fp, x, y, edata, w, h, bw)
HTMLWidget hw;
int type;
XFontStruct *fp;
int x, y;
...
int bw;
{
     ...
     * add it.
     */
    if ((hw->html.formatted_elements == NULL) || ((Current != NULL) && (Current->next == NULL))) {
    ...
    case E_LINEFEED:
        ...
        if (AnchorText != NULL) {
            ...
            tptr = ParseMarkTag(AnchorText, MT_ANCHOR, AT_HREF);
            if ((eptr->prev != NULL) &&
                ((eptr->prev->anchorHRef == NULL) || (tptr == NULL) || (strcmp(eptr->prev->anchorHRef, tptr) != 0))) {
                if (tptr)
                    free(tptr);
                eptr->anchorHRef = NULL;
                eptr->anchorName = NULL;
 ...
 */
void ConditionalLineFeed(hw, x, y, state)
HTMLWidget hw;
int *x, *y;
int state;
{
...
static void AdjustBaseLine()
{
    ...
    if (BaseLine == -100) {
        ...
        while ((eptr->prev != NULL) && (eptr->prev->line_number == line)) {
            eptr = eptr->prev;
...
int *x, *y;
{
    ...
    if (hw->html.font->descent > hw->html.font->max_bounds.descent) {
        LineHeight = hw->html.font->max_bounds.ascent + hw->html.font->descent;
    } else {
        LineHeight = hw->html.font->max_bounds.ascent + hw->html.font->max_bounds.descent;
    }
 ...
 * list item. Create and add the element record for it.
 */
void ListNumberPlace(hw, x, y, val)
HTMLWidget hw;
int *x, *y;
int val;
{
    ...
    width = hw->html.font->max_bounds.lbearing + hw->html.font->max_bounds.rbearing;
 ...
 */
void PreformatPlace(hw, mptr, x, y, width)
HTMLWidget hw;
struct mark_up *mptr;
int *x, *y;
unsigned int width;
{
    ...
    while (*end != '\0') {
        ...
        if ((*end == '\r') || (*end == '\f')) {
             ...
             */
            while (((int)((unsigned char)*end) < 128) && (isspace(*end))) {
                if (*end == '\n') {
                    break;
                } else if (*end == '\t') {
                    tab_count++;
            ...
            while (((int)((unsigned char)*end) > 127) || ((!isspace(*end)) && (*end != '\0'))) {
                end++;
                char_cnt++;
            }
        }
...
#else
            XTextExtents(currentFont, ptr, strlen(ptr), &dir, &ascent, &descent, &all);
#endif                          /* ASSUME_FIXED_WIDTH_PRE */

            if (*start == '\n') {
                if ((line != NULL) && (line[0] != '\0')) {
                    SetElement(hw, E_TEXT, currentFont, line_x, *y, line, NULL, NULL, IMAGE_DEFAULT_BORDER);
                    /*
 ...
 */
void FormatPlace(hw, mptr, x, y, width)
HTMLWidget hw;
struct mark_up *mptr;
int *x, *y;
unsigned int width;
{
    ...
    while (*end != '\0') {
        ...
        if (start != end) {
            ...
            *end = '\0';
            ...
            if (((*x + all.width + MarginW) <= width) || (nobreak)) {
                ...
                XTextExtents(currentFont, tptr2, strlen(tptr2), &dir, &ascent, &descent, &all);

                if (line == NULL) {
                    tlen = strlen(tptr2) + 1;
                } else {
                    tlen = strlen(line) + strlen(tptr2) + 1;
                }
                if (tlen > CompLineLen) {
                    CompLineLen += COMP_LINE_BUF_LEN;
                    if (tlen > CompLineLen) {
         ...
         */
            ...
            XTextExtents(currentFont, spc, strlen(spc), &dir, &ascent, &descent, &all);
 ...
 */
void ImagePlace(hw, mptr, x, y, width)
HTMLWidget hw;
struct mark_up *mptr;
int *x, *y;
unsigned int width;
{
    ...
    if ((!Preformat) && (NeedSpace > 0)) {
        ...
        XTextExtents(currentFont, tptr, strlen(tptr), &dir, &ascent, &descent, &all);
        SetElement(hw, E_TEXT, currentFont, *x, *y, tptr, NULL, NULL, IMAGE_DEFAULT_BORDER);
        /*
         * Save width here to avoid an 
         * XTextExtents call later.
         */
    ...
    if ((Current->pic_data != NULL) && (!Preformat)) {
        ...
        if ((hw->html.border_images == True) || ((Current->anchorHRef != NULL) && (!Current->pic_data->internal))) {
        ...
        if (((*x + Current->pic_data->width + extra + MarginW) > width) &&
            (Current->prev != NULL) && (Current->prev->line_number == LineNumber)) {
    ...
    if (Current->pic_data != NULL) {
        ...
        if (tptr != NULL) {
             ...
             */
            if ((CurrentForm != NULL) && ((Current->anchorHRef == NULL) || (IsDelayedHRef(hw, Current->anchorHRef)))) {
    ...
    if (Current->pic_data != NULL) {
        ...
        *x = *x + Current->pic_data->width + extra;
        ...
        if (Current->alignment == ALIGN_TOP) {
            ...
            if ((Current->pic_data->height + extra - BaseLine) > LineBottom) {
                LineBottom = Current->pic_data->height + extra - BaseLine;
            }
            ...
            Current->y_offset = BaseLine - (Current->pic_data->height + extra);
        } else if ((Current->pic_data->height + extra) > BaseLine) {
            struct ele_rec *eptr;
            int line, incy;
 ...
 */
void WidgetPlace(hw, mptr, x, y, width)
HTMLWidget hw;
struct mark_up *mptr;
int *x, *y;
unsigned int width;
{
    ...
    if ((Current->widget_data != NULL) && (!Preformat)) {
        ...
        if (((*x + Current->widget_data->width + extra + MarginW) >
             width) && (Current->prev != NULL) && (Current->prev->line_number == LineNumber)) {
            WidgetId--;
            Current = Current->prev;
            LineFeed(hw, x, y);
            SetElement(hw, E_WIDGET, currentFont, *x, *y, mptr->start, NULL, NULL, IMAGE_DEFAULT_BORDER);
    ...
    if (Current->widget_data != NULL) {
         ...
         */
            ...
            border = ((Current->widget_data->height + extra) - (fp->max_bounds.ascent + fp->max_bounds.descent));
        ...
        if (BaseLine == -100) {
             ...
             */
            if (LineBottom == 0) {
                LineBottom = Current->widget_data->height + extra - baseline;
            }
            /*
             ...
             */
                ...
                if ((Current->widget_data->height + extra - baseline) > (LineBottom - baseline)) {
                    LineBottom = Current->widget_data->height + extra - baseline;
                } else {
                    LineBottom = LineBottom - baseline;
                }
         ...
         */
             ...
             * old one.
             */
            if ((Current->widget_data->height + extra - baseline) > LineBottom) {
                LineBottom = Current->widget_data->height + extra - baseline;
            }
        } else
            /*
 ...
 */
static void ProcessOption(sptr)
SelectInfo *sptr;
{
    int i, cnt;
    char **tarray;
 ...
 */
char *TextAreaAddValue(value, text)
char *value;
char *text;
{
    int extra;
 ...
 */
static void TriggerMarkChanges(hw, mptr, x, y)
HTMLWidget hw;
struct mark_up **mptr;
int *x, *y;
{
    ...
    /* If we are not in a tag that belongs in the HEAD, end the HEAD
       section  - amb */
    if (InDocHead)
        if ((type != M_TITLE) && (type != M_NONE) && (type != M_BASE) && (type != M_INDEX) && (type != M_COMMENT)) {
            Ignore = 0;
            InDocHead = 0;
        }
    ...
    if ((Ignore) && (!InDocHead) && (type != M_TITLE) && (type != M_NONE) &&
        (type != M_SELECT) && (type != M_OPTION) && (type != M_TEXTAREA) && (type != M_DOC_HEAD)) {
        return;
    }
    ...
    case M_NONE:
        if ((Ignore) && (CurrentSelect == NULL) && (TextAreaBuf == NULL)) {
            if (TitleText == NULL) {
                TitleText = (char *)malloc(strlen((*mptr)->text) + 1);
                strcpy(TitleText, (*mptr)->text);
            } else {
                char *tptr;

                tptr = (char *)
                    malloc(strlen(TitleText) + strlen((*mptr)->text) + 1);
            ...
            if (CurrentSelect->option_buf != NULL) {
                ...
                tptr = (char *)malloc(strlen(CurrentSelect->option_buf) + strlen((*mptr)->text) + 1);
    ...
    case M_BOLD:
        if (mark->is_end) {
            ...
            PushFont(currentFont);
            if (currentFont == hw->html.fixed_font || currentFont == hw->html.fixeditalic_font)
                font = hw->html.fixedbold_font;
            else if (currentFont == hw->html.plain_font || currentFont == hw->html.plainitalic_font)
    ...
    case M_ITALIC:
        if (mark->is_end) {
            ...
            PushFont(currentFont);
            if (currentFont == hw->html.fixed_font || currentFont == hw->html.fixedbold_font)
                font = hw->html.fixeditalic_font;
            else if (currentFont == hw->html.plain_font || currentFont == hw->html.plainbold_font)
 ...
 */
            ...
            if (tptr != NULL) {
                ...
                if (Internal == True) {
                    ...
                    hptr = FindHRef(hw->html.my_visited_hrefs, tptr);
                    if (hptr != NULL) {
                 ...
                 */
                else if (hw->html.previously_visited_test != NULL) {
                    if ((*(visitTestProc)
    ...
    case M_SELECT:
        if (CurrentForm != NULL) {
            if ((mark->is_end) && (CurrentSelect != NULL)) {
                ...
                options = ComposeCommaList(CurrentSelect->options, CurrentSelect->option_cnt);
                returns = ComposeCommaList(CurrentSelect->returns, CurrentSelect->option_cnt);
                value = ComposeCommaList(CurrentSelect->value, CurrentSelect->value_cnt);
                ...
                len = strlen(MT_INPUT) +
                    strlen(options) +
                    strlen(returns) + strlen(value) + strlen(" type=select options=\"\" returns=\"\" value=\"\"");
                buf = (char *)malloc(len + strlen(CurrentSelect->mptr->start) + 1);
                strcpy(buf, MT_INPUT);
                ...
                strcat(buf, "\"");
                strcat(buf, (char *)
                       (CurrentSelect->mptr->start + strlen(MT_SELECT)));
                /*
                ...
                WidgetPlace(hw, CurrentSelect->mptr, x, y, Width);
                /*
        ...
        if (CurrentForm != NULL) {
            ...
            if ((tptr != NULL) && (caseless_equal(tptr, "image"))) {
                ...
                tptr = (char *)malloc(strlen((*mptr)->start) +
                                      strlen(" ISMAP") + strlen(MT_IMAGE) - strlen(MT_INPUT) + 1);
    ...
    case M_DIRECTORY:
         ...
         */
        if ((TextIndent <= MarginW) || ((mark->is_end) && ((TextIndent - ((INDENT_SPACES + 1) * width)) <= MarginW))) {
            ConditionalLineFeed(hw, x, y, 2);
        }
        if (mark->is_end) {
    ...
    case M_DESC_TEXT:
         ...
         * after a title.
         */
        if ((DescType->compact) && (DescType->type == D_TITLE) && (*x < (TextIndent + (INDENT_SPACES * width)))) {
            NeedSpace = 0;
        } else {
            ConditionalLineFeed(hw, x, y, 1);
        }
    ...
    case M_INDEX:
        ...
        if (CurrentForm == NULL) {
            ...
            CurrentForm->format = ParseMarkTag(mark->start, MT_INDEX, "FORMAT");
            CurrentForm->method = ParseMarkTag(mark->start, MT_INDEX, "METHOD");
            CurrentForm->enctype = ParseMarkTag(mark->start, MT_INDEX, "ENCTYPE");
            CurrentForm->enc_entity = ParseMarkTag(mark->start, MT_INDEX, "ENCENTITY");
            CurrentForm->start = WidgetId;
            CurrentForm->end = -1;
             ...
             */
            mark_tmp.text = (char *)
                malloc(strlen("This is a searchable index.  Enter search keywords: ")
                       + 1);
             ...
             */
            mark_tmp.start = (char *)malloc(strlen("input SIZE=25 NAME=\"isindex\"") + 1);
             ...
             */
            mark_tmp.start = (char *)malloc(strlen("input TYPE=\"submit\"") + 1);
 ...
 */
    ...
    hw->html.line_array = MakeLineList(hw->html.formatted_elements, LineNumber);
...
struct ele_rec *eptr;
{
    ...
    y1 = eptr->y + eptr->y_offset + eptr->font->max_bounds.ascent - eptr->font->ascent;
    ...
    if (eptr->selected == False) {
        if (hw->html.bg_image) {
            HTMLDrawBackgroundImage((Widget) hw, x1, y1, width, height);
        } else {
            XFillRectangle(XtDisplay(hw->html.view), XtWindow(hw->html.view), hw->html.drawGC, x1, y1, width, height);
        }
    }
}
 ...
 */
void PartialRefresh(hw, eptr, start_pos, end_pos, fg, bg)
HTMLWidget hw;
struct ele_rec *eptr;
...
unsigned long fg, bg;
{
    ...
    if (start_pos != 0) {
        ...
        if (eptr->font == hw->html.plain_font) {
            ...
            XTextExtents(eptr->font, (char *)eptr->edata, start_pos, &dir, &nascent, &descent, &all);
        }
#else
        XTextExtents(eptr->font, (char *)eptr->edata, start_pos, &dir, &nascent, &descent, &all);
#endif                          /* ASSUME_FIXED_WIDTH_PRE */
...
#ifndef NO_EXTRA_FILLS
    {
        ...
        if ((!partial) && (eptr->width != 0)) {
            ...
            if (eptr->font == hw->html.plain_font) {
                ...
                XTextExtents(eptr->font, (char *)tdata, tlen, &dir, &nascent, &descent, &all);
            }
#else
            XTextExtents(eptr->font, (char *)tdata, tlen, &dir, &nascent, &descent, &all);
#endif                          /* ASSUME_FIXED_WIDTH_PRE */
        ...
        if (height > 0) {
            if (!hw->html.bg_image)
                XFillRectangle(XtDisplay(hw),
                               ...
                               hw->html.drawGC, x, y, (unsigned int)all.width, (unsigned int)height);
        }
            ...
            if (!hw->html.bg_image)
                XFillRectangle(XtDisplay(hw),
                               ...
                               (int)(y + eptr->font->max_bounds.ascent +
                                     eptr->font->descent), (unsigned int)all.width, (unsigned int)height);
        }
        width = all.width;
    }
#endif                          /* NO_EXTRA_FILLS */

    if (bg != hw->html.view->core.background_pixel || NoBodyImages(hw)
        || !hw->html.bg_image) {
        XSetForeground(XtDisplay(hw), hw->html.drawGC, bg);
        XSetBackground(XtDisplay(hw), hw->html.drawGC, fg);

        XFillRectangle(XtDisplay(hw),
                       XtWindow(hw->html.view), hw->html.drawGC, x, y, width, ascent + eptr->font->descent);

        XSetForeground(XtDisplay(hw), hw->html.drawGC, fg);
        XSetBackground(XtDisplay(hw), hw->html.drawGC, bg);

        XDrawString(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC, x, y + ascent, (char *)tdata, tlen);
    } else {
        XSetForeground(XtDisplay(hw), hw->html.drawGC, bg);
        XSetBackground(XtDisplay(hw), hw->html.drawGC, fg);
        ...
        HTMLDrawBackgroundImage((Widget) hw,
                                ...
                                (x < 0 ?
                                 (width + x) :
                                 width), (y < 0 ? (ascent + eptr->font->descent + y) : (ascent + eptr->font->descent)));
    ...
    if (eptr->underline_number) {
        ...
        if (eptr->dashed_underline) {
            XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, 1, LineOnOffDash, CapButt, JoinBevel);
        } else {
            XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, 1, LineSolid, CapButt, JoinBevel);
        }
        ...
        ly = (int)(y + eptr->font->max_bounds.ascent + eptr->font->descent - 1);

        for (i = 0; i < eptr->underline_number; i++) {
            XDrawLine(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC, x, ly, (int)(x + width), ly);
            ly -= 2;
        }
    }
    ...
    if (eptr->strikeout == True) {
        ...
        ly = (int)(y + eptr->font->max_bounds.ascent + eptr->font->descent - 1);
        ly = ly - ((hw->html.font->max_bounds.ascent + hw->html.font->descent) / 2);
 ...
 */
void TextRefresh(hw, eptr, start_pos, end_pos)
HTMLWidget hw;
struct ele_rec *eptr;
int start_pos, end_pos;
{
    if (eptr->selected == False) {
        PartialRefresh(hw, eptr, start_pos, end_pos, eptr->fg, eptr->bg);
        ...
        if (start_pos < eptr->start_pos) {
            PartialRefresh(hw, eptr, start_pos, eptr->start_pos - 1, eptr->fg, eptr->bg);
            start_pos = eptr->start_pos;
        }
        if (end_pos > eptr->end_pos) {
            PartialRefresh(hw, eptr, eptr->end_pos + 1, end_pos, eptr->fg, eptr->bg);
...
*/
    width = eptr->font->max_bounds.lbearing + eptr->font->max_bounds.rbearing;
    /*
     * Deal with bad Lucidia descents.
     */
    if (eptr->font->descent > eptr->font->max_bounds.descent) {
        line_height = eptr->font->max_bounds.ascent + eptr->font->descent;
    } else {
        line_height = eptr->font->max_bounds.ascent + eptr->font->max_bounds.descent;
    }
    x1 = eptr->x;
    ...
    if (eptr->indent_level && (eptr->indent_level % 2)) {   /* odd & !0 */
        ...
        if (eptr->indent_level == 1 || (eptr->indent_level % 4) == 1) {
            XFillArc(XtDisplay(hw),
                     XtWindow(hw->html.view), hw->html.drawGC, (x1 - width), y1, (width / 2), (width / 2), 0, 23040);
        } else {
        ...
        if (eptr->indent_level == 0 || (eptr->indent_level % 4) == 2) {
            XFillRectangle(XtDisplay(hw),
                           XtWindow(hw->html.view), hw->html.drawGC, (x1 - width), y1, (width / 2), (width / 2));
        } else {
...
struct ele_rec *eptr;
{
    ...
    XSetForeground(XtDisplay(hw), hw->html.drawGC, eptr->bg);
    if (!hw->html.bg_image)
        XFillRectangle(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC, x1, y1, width, height);
    y1 = y1 + (height / 2) - 1;

    XSetLineAttributes(XtDisplay(hw), hw->html.drawGC, 1, LineSolid, CapButt, JoinBevel);
#ifdef MOTIF
    XDrawLine(XtDisplay(hw), XtWindow(hw->html.view), hw->manager.bottom_shadow_GC, x1, y1, (int)(x1 + width), y1);
    XDrawLine(XtDisplay(hw), XtWindow(hw->html.view), hw->manager.top_shadow_GC, x1, y1 + 1, (int)(x1 + width), y1 + 1);
#else
    /* changing the GC back and forth is not the most efficient way.... */
    XSetForeground(XtDisplay(hw), hw->html.drawGC, eptr->fg);
    XDrawLine(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC, x1, y1, (int)(x1 + width), y1);
    XDrawLine(XtDisplay(hw), XtWindow(hw->html.view), hw->html.drawGC, x1, y1 + 1, (int)(x1 + width), y1 + 1);
#endif
}
...
struct ele_rec *eptr;
{
    ...
    if (eptr->pic_data != NULL) {
        ...
        if ((hw->html.border_images == True) || ((eptr->anchorHRef != NULL) && (!eptr->pic_data->internal))) {
        ...
        if (extra) {
            XFillRectangle(XtDisplay(hw),
                           ...
                           x, y, (eptr->pic_data->width + (2 * extra)), extra);
            XFillRectangle(XtDisplay(hw),
                           XtWindow(hw->html.view), hw->html.drawGC,
                           x, (y + eptr->pic_data->height + extra), (eptr->pic_data->width + (2 * extra)), extra);
            XFillRectangle(XtDisplay(hw),
                           XtWindow(hw->html.view), hw->html.drawGC,
                           x, y, extra, (eptr->pic_data->height + (2 * extra)));
            XFillRectangle(XtDisplay(hw),
                           XtWindow(hw->html.view), hw->html.drawGC,
                           (x + eptr->pic_data->width + extra), y, extra, (eptr->pic_data->height + (2 * extra)));
        }
            ...
            if (eptr->pic_data->image_data != NULL) {
                ...
                if (eptr->pic_data->transparent && eptr->pic_data->clip == None) {
                    eptr->pic_data->clip =
                        ...
                        (XtDisplay(hw),
                         XtWindow(hw->html.view),
                         eptr->pic_data->clip_data, eptr->pic_data->width, eptr->pic_data->height, 1, 0, 1);
                ...
                if (eptr->pic_data->delayed) {
                    if ((eptr->anchorHRef != NULL) &&
                        (!IsDelayedHRef(hw, eptr->anchorHRef)) && (!IsIsMapForm(hw, eptr->anchorHRef))) {
                     ...
                     */
                    if ((eptr->pic_data->fetched) && (hw->html.resolveDelayedImage != NULL)) {
                        ImageInfo *pdata;

                        pdata = eptr->pic_data;
                        eptr->pic_data = (*(resolveImageProc)
                                          (hw->html.resolveDelayedImage))
                            (hw, eptr->edata);
                        if (eptr->pic_data != NULL) {
                            eptr->pic_data->delayed = 0;
                            /*
                             ...
                             */
                            eptr->pic_data->ismap = pdata->ismap;
                            eptr->pic_data->fptr = pdata->fptr;
                            eptr->pic_data->internal = pdata->internal;
                            eptr->pic_data->text = pdata->text;
                        } else {
                            eptr->pic_data = NoImageData(hw);
                            eptr->pic_data->delayed = 0;
                            eptr->pic_data->internal = 0;
        ...
        if (eptr->pic_data->image != None) {
            if (eptr->pic_data->transparent) {
                ...
                XCopyArea(XtDisplay(hw),
                          ...
                          XtWindow(hw->html.view),
                          hw->html.drawGC,
                          0, 0, eptr->pic_data->width, eptr->pic_data->height, (x + extra), (y + extra));
        ...
        if ((eptr->pic_data->delayed) && (eptr->anchorHRef != NULL) &&
            ...
            XFillRectangle(XtDisplay(hw->html.view),
                           XtWindow(hw->html.view),
                           hw->html.drawGC, x, (y + AnchoredHeight(hw)), (eptr->pic_data->width + (2 * extra)), extra);
        }
    }
}

void RefreshTextRange(hw, start, end)
HTMLWidget hw;
 ...
 */
struct ele_rec *LocateElement(hw, x, y, pos)
HTMLWidget hw;
int x, y;
int *pos;
{
     ...
     */
    guess = y / (hw->html.font->max_bounds.ascent + hw->html.font->max_bounds.descent);
    if (guess > (hw->html.line_count - 1)) {
        guess = hw->html.line_count - 1;
    }
    while (guess > 0) {
        if ((hw->html.line_array[guess] != NULL) && (hw->html.line_array[guess]->y <= y)) {
            break;
        }
        guess--;
    }
     ...
     */
    if (eptr->font->descent > eptr->font->max_bounds.descent) {
        ty2 = eptr->y + eptr->font->max_bounds.ascent + eptr->font->descent;
    } else {
        ty2 = eptr->y + eptr->font->max_bounds.ascent + eptr->font->max_bounds.descent;
    }
    line = eptr->line_number;
    /*
     ...
     */
    if (((line + 1) < hw->html.line_count) && (hw->html.line_array[line + 1] != NULL)) {
    ...
    while ((eptr != NULL) && (eptr->line_number <= (end + 1))) {
        ...
        if (eptr->type == E_TEXT) {
            ...
            tx1 = eptr->x;
            XTextExtents(eptr->font, (char *)eptr->edata, eptr->edata_len - 1, &dir, &ascent, &descent, &all);
    ...
    if (rptr != NULL) {
        ...
        XTextExtents(rptr->font, (char *)rptr->edata, (epos + 1), &dir, &ascent, &descent, &all);
 ...
 */
void strcpy_or_grow(str, slen, blen, add)
char **str;
int *slen;
int *blen;
char *add;
 ...
 * to indent lines.
 */
char *ParseTextToString(elist, startp, endp, start_pos, end_pos, space_width, lmargin)
 ...
 * to indent lines.
 */
char *ParseTextToPrettyString(hw, elist, startp, endp, start_pos, end_pos, space_width, lmargin)
HTMLWidget hw;
struct ele_rec *elist;
struct ele_rec *startp;
struct ele_rec *endp;
    ...
    while ((eptr != NULL) && (eptr != end)) {
        ...
        if (eptr->type == E_BULLET) {
            ...
            if (newline) {
                ...
                lead_spaces = spaces;
                for (i = 0; i < spaces; i++) {
                    strcpy_or_grow(&line_buf, &l_slen, &l_blen, " ");
                }
            }
            newline = 0;
...
struct mark_up *list;
{
    ...
    while (mptr != NULL) {
         ...
         */
        if ((mptr->type == M_PLAIN_TEXT) || (mptr->type == M_PLAIN_FILE) || (mptr->type == M_PREFORMAT)) {
            if (mptr->is_end) {
                plain_text--;
                if (plain_text < 0) {
                    plain_text = 0;
