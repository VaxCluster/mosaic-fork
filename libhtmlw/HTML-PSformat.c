...
static int PShex(unsigned char val, int flush)
{
    ...
    if (!flush) {
        hexline[PS_hexi++] = (char)digit[((unsigned)val >> (unsigned)4) & (unsigned)0x0f];
    ...
    if (n_saved_ftns > 0) {
        ...
        PSprintf
            ("gsave 0.2 setlinewidth newpath %.2f %.2f M %.2f 0 RL stroke\n",
             ...
             (page_dimens.bot_margin + (footnote_ptsize * n_saved_ftns) + 4), (page_dimens.text_width * 0.4));
        for (i = 0; n_saved_ftns; n_saved_ftns--, i++) {
            PSprintf
                ("newpath %.2f %.2f M RF %.2f SF (%d) S 3 -2 R RF %d SF (%s) S\n",
                 page_dimens.left_margin,
                 page_dimens.bot_margin + 5 + (n_saved_ftns -
                                               1) * footnote_ptsize,
                 (0.7 * footnote_ptsize), cur_ftn_no - n_saved_ftns, footnote_ptsize, footnotes[i]);
        }
        PSprintf("grestore\n");
    }
    PSprintf("showpage\n");
...
static void PSheader(char *title, int font, char *url, char *time_str)
{
    ...
    strftime(time_buf, sizeof(time_buf), "Printed %a %b %e %T %Y", localtime(&clock));
#else
...
*/
    ...
    if (HTML_Print_Headers) {
        ...
        PSprintf("  newpath %.2f %.2f M %.2f 0 RL stroke",
                 page_dimens.left_margin, (page_dimens.bot_margin + page_dimens.text_height), page_dimens.text_width);
        PSprintf("  newpath %.2f %.2f M %.2f 0 RL stroke\n",
                 page_dimens.left_margin, page_dimens.bot_margin, page_dimens.text_width);
        PSprintf("  BF 12 SF %.2f %.2f M (%s) S\n",
                 page_dimens.left_margin, (page_dimens.bot_margin + page_dimens.text_height + 6), title);
        ...
        PSprintf
            ("  (%s) dup stringwidth pop %.2f E sub %.2f M S grestore\n",
             time_str, page_dimens.left_margin + page_dimens.text_width, page_dimens.bot_margin - 12);
    }
    PSprintf("  %.2f %.2f translate %.5f %.5f scale } D\n",
             page_dimens.left_margin, page_dimens.bot_margin + page_dimens.text_height, Points_Pixel, Points_Pixel);
    PSinit_latin1();
...
static void PSfootnote(char *href, double height)
{
    ...
    if (n_saved_ftns == ftn_array_size) {
        ...
        if (!footnotes) {
            ...
            footnotes = (char **)realloc((void *)footnotes, (ftn_array_size * sizeof(char *)));
        }
        if (footnotes == NULL) {
#ifndef DISABLE_TRACE
            if (htmlwTrace) {
 ...
 *
 */
static void PStext(HTMLWidget hw, struct ele_rec *eptr, int fontfamily, String s)
{
    String s2;
    String stmp;
    unsigned char ch;
 ...
 *
 */
static void PSbullet(HTMLWidget hw, struct ele_rec *eptr, int fontfamily)
{
    int width = (eptr->font->max_bounds.lbearing + eptr->font->max_bounds.rbearing);
    ...
    if ((eptr->next != NULL)
        && ((eptr->next->type == E_TEXT) || (eptr->next->type == E_IMAGE))) {
 ...
 *
 */
static void PShrule(HTMLWidget hw, struct ele_rec *eptr, int fontfamily)
{
 ...
 *	5	option menu
 */
static void PSwidget(HTMLWidget hw, struct ele_rec *eptr, int fontfamily)
{
 ...
 *
*/
static int PSrle_encode(unsigned char *scanline, unsigned char *rleline, int wide)
{
...
static void PScolormap(int color, int nc, int *rmap, int *gmap, int *bmap)
{
    ...
    for (i = 0; i < nc; i++) {
        if (color)
            PSprintf("%02x%02x%02x ", rmap[i] >> 8, gmap[i] >> 8, bmap[i] >> 8);
        else
...
static void PSimage(HTMLWidget hw, struct ele_rec *eptr, int fontfamily)
{
...
#	define Is_fg(i,n)  (i->reds[n]==fg_color.red &&			\
    ...
    if (((ncolors == 2)
        ...
        || ((ncolors == 1)
            && (Isgray(img, 0) || Is_bg(img, 0) || Is_fg(img, 0)))) {
    ...
    if (colortype == F_BWDITHER) {
        ...
        if ((ncolors == 2 &&
            ...
            (ncolors == 1 && MONO(img->reds[0], img->greens[0], img->blues[0]) > MONO(127, 127, 127))) {
            flipbw = 1;
        }
...
String ParseTextToPSString(HTMLWidget hw,
                           ...
                           struct ele_rec *endp,
                           int start_pos,
                           int end_pos, int space_width, int lmargin, int fontfamily, char *url, char *time_str)
{
    int xpos, ypos, epos;
    int height;
    double pagewidth;
...
#ifndef MOTIF
    XtVaGetValues((Widget) hw, XtNforeground, &fg_pixel, NULL);
#endif
    ...
    XQueryColor(XtDisplay(hw->html.view),
                (installed_colormap ?
                 installed_cmap :
                 DefaultColormap(XtDisplay(hw->html.view), DefaultScreen(XtDisplay(hw->html.view)))), &fg_color);
    XQueryColor(XtDisplay(hw->html.view),
                (installed_colormap ?
                 installed_cmap :
    ...
    page_dimens.text_height = (page_dimens.page_height - page_dimens.top_margin - page_dimens.bot_margin);
    page_dimens.text_width = (page_dimens.page_width - page_dimens.left_margin - page_dimens.right_margin);
    ...
    while ((eptr != NULL) && (eptr != end)) {
        ...
        switch (eptr->type) {
        case E_TEXT:
            PStext(hw, eptr, fontfamily, (String) ((eptr == start) ? (eptr->edata + start_pos) : eptr->edata));
            break;
