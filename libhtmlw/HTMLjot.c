...
void ClearJot(hw, w, width, height)
HTMLWidget hw;
Widget w;
int width, height;
{
    ...
    if (jptr->pix != NULL) {
        XSetForeground(XtDisplay(w), hw->html.drawGC, hw->core.background_pixel);
        XFillRectangle(XtDisplay(w), jptr->pix, hw->html.drawGC, 0, 0, jptr->width, jptr->height);
    }
...
void AddStroke(jptr, sptr, drawing)
JotInfo *jptr;
Stroke *sptr;
Boolean drawing;
{
...
XEvent *event;
{
    ...
    if (jptr->pix == NULL) {
        jptr->pix = XCreatePixmap(XtDisplay(w), XtWindow(w),
                                  jptr->width, jptr->height, XDefaultDepth(XtDisplay(w), XDefaultScreen(XtDisplay(w))));
        if (jptr->pix == NULL) {
            return;
        }
        XSetForeground(XtDisplay(w), hw->html.drawGC, hw->core.background_pixel);
    ...
    XCopyArea(XtDisplay(w), jptr->pix, XtWindow(w),
              hw->html.drawGC, ExEvent->x, ExEvent->y, ExEvent->width, ExEvent->height, ExEvent->x, ExEvent->y);
}

void EVJotPress(w, data, event)
Widget w;
XtPointer data;
XEvent *event;
{
 ...
 */
    XSetForeground(XtDisplay(w), hw->html.drawGC,
                   ...
                   hw->html.foreground);
#endif                          /* MOTIF */
    XDrawPoint(XtDisplay(w), XtWindow(w), hw->html.drawGC, sptr->x, sptr->y);
    if (jptr->pix != NULL) {
        XDrawPoint(XtDisplay(w), jptr->pix, hw->html.drawGC, sptr->x, sptr->y);
    }

    AddStroke(jptr, sptr, True);
}

void EVJotMove(w, data, event)
Widget w;
XtPointer data;
XEvent *event;
{
 ...
 */
    XSetForeground(XtDisplay(w), hw->html.drawGC,
                   ...
                   hw->html.foreground);
#endif                          /* MOTIF */
    XDrawLine(XtDisplay(w), XtWindow(w), hw->html.drawGC, jptr->last_x, jptr->last_y, sptr->x, sptr->y);
    if (jptr->pix != NULL) {
        XDrawLine(XtDisplay(w), jptr->pix, hw->html.drawGC, jptr->last_x, jptr->last_y, sptr->x, sptr->y);
    }

    AddStroke(jptr, sptr, True);
}

void EVJotRelease(w, data, event)
Widget w;
XtPointer data;
XEvent *event;
{
...
unsigned char *JOTfromJot(w, buffer_len)
Widget w;
int *buffer_len;
{
    int i, cnt;
...
*/
    ...
    total = inkRecordBundleSize + inkRecordPenDataSize(dlen) + inkRecordEndSize;
    buffer = (unsigned char *)malloc(total);
/*
...
*/
    memcpy((char *)(buffer + inkRecordBundleSize), (char *)pptr, inkRecordPenDataSize(dlen));
/*
	bcopy((char *)eptr, (char *)(buffer + inkRecordBundleSize +
		inkRecordPenDataSize(dlen)), inkRecordEndSize);
*/
    memcpy((char *)(buffer + inkRecordBundleSize + inkRecordPenDataSize(dlen)), (char *)eptr, inkRecordEndSize);
    free((char *)iptr);
    free((char *)pptr);
    free((char *)eptr);
    *buffer_len = total;
