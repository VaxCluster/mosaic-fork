...
static void _HTMLInput(Widget w, XEvent * event, String * params, Cardinal * num_params);
#ifndef MOTIF
static void _HTMLpwdInput(Widget w, XEvent * event, String * params, Cardinal * num_params);
#endif
...
static Boolean SetValues(HTMLWidget current, HTMLWidget request, HTMLWidget new);
...
hw_do_bg(Widget w, char *bgname)
{
    ...
    if (hw->html.delay_images == True || currently_delaying_images == 1) {
        pic_data = (*(resolveImageProc) (hw->html.resolveImage)) (hw, bgname, 1, NULL, NULL);
        ...
        pic_data = (*(resolveImageProc) (hw->html.resolveImage)) (hw, bgname, 0, NULL, NULL);
 ...
 */
        ...
        hw->html.drawGC = XCreateGC(XtDisplay(hw), XtWindow(hw->html.view), valuemask, &values);
    }
}
 ...
 */
static void DrawExpose(w, data, event)
Widget w;
caddr_t data;
XEvent *event;
{
...
*/
          (event->xany.type == GraphicsExpose) || event->xany.type == VisibilityNotify)) {
        return;
    }
...
*/
    ...
    while (((XCheckWindowEvent(dsp, win, ExposureMask | VisibilityChangeMask, &NewEvent) == True))) {
        if (NewEvent.xany.type == Expose ||
/*
		    (hw->html.obscured && NewEvent.xany.type==GraphicsExpose) ||
*/
            (NewEvent.xany.type == GraphicsExpose) || NewEvent.xany.type == VisibilityNotify) {
            ...
            /* The following block makes sure the exposure area is not
               ...
               over. --SWP */
            if ((y > (ny + nheight) &&
                 (y - (ny + nheight)) > MAX_Y_EXP_DIFF) || (y1 < ny && (ny - y1) > MAX_Y_EXP_DIFF)) {
#ifndef DISABLE_TRACE
                if (htmlwTrace) {
    ...
    while (wptr != NULL) {
        if (wptr->w != NULL) {
            ...
            if (((y > 0 &&
                ...
                ((x > 0 &&
                  x <= hw->html.view_width) || ((x + wptr->width) > 0 && (x + wptr->width) <= hw->html.view_width))) {
                wptr->seeable = 1;
                XtMoveWidget(w, x, y);
 ...
 */
static void setScrollBar(sb, topPosition, totalLength, currentLength)
Widget sb;
        ...
        if (value > hw->html.scroll_y) {
            ...
            if (dy > hw->html.view_height) {
                ...
                XClearArea(XtDisplay(hw),
                           XtWindow(hw->html.view), 0, 0, hw->html.view_width, hw->html.view_height, False);
                ...
                XCopyArea(XtDisplay(hw),
                          ...
                          XtWindow(hw->html.view),
                          hw->html.drawGC, 0, dy, hw->html.view_width, hw->html.view_height - dy, 0, 0);
                hw->html.scroll_y = value;
                XClearArea(XtDisplay(hw),
                           XtWindow(hw->html.view), 0, (int)hw->html.view_height - dy, hw->html.view_width, dy, False);
                ViewRedisplay(hw, 0, (int)hw->html.view_height - dy, hw->html.view_width, dy);
            }
        }
        /*
         ...
         */
            ...
            if (dy > hw->html.view_height) {
                ...
                XCopyArea(XtDisplay(hw),
                          XtWindow(hw->html.view),
                          XtWindow(hw->html.view),
                          hw->html.drawGC, 0, 0, hw->html.view_width, hw->html.view_height - dy, 0, dy);
                hw->html.scroll_y = value;
                XClearArea(XtDisplay(hw), XtWindow(hw->html.view), 0, 0, hw->html.view_width, dy, False);

                ViewRedisplay(hw, 0, 0, hw->html.view_width, dy);
        ...
        if (value > hw->html.scroll_x) {
            ...
            if (dx > hw->html.view_width) {
                ...
                XCopyArea(XtDisplay(hw),
                          XtWindow(hw->html.view),
                          XtWindow(hw->html.view),
                          hw->html.drawGC, dx, 0, hw->html.view_width - dx, hw->html.view_height, 0, 0);
                hw->html.scroll_x = value;
                XClearArea(XtDisplay(hw),
                           XtWindow(hw->html.view), (int)hw->html.view_width - dx, 0, dx, hw->html.view_height, False);

                ViewRedisplay(hw, (int)hw->html.view_width - dx, 0, dx, hw->html.view_height);
            }
        }
        /*
         ...
         */
            ...
            if (dx > hw->html.view_width) {
                ...
                XCopyArea(XtDisplay(hw),
                          XtWindow(hw->html.view),
                          XtWindow(hw->html.view),
                          hw->html.drawGC, 0, 0, hw->html.view_width - dx, hw->html.view_height, dx, 0);
                hw->html.scroll_x = value;

                XClearArea(XtDisplay(hw), XtWindow(hw->html.view), 0, 0, dx, hw->html.view_height, False);

                ViewRedisplay(hw, 0, 0, dx, hw->html.view_height);
            }
        }
    }
 ...
 * Either the vertical or hortizontal scrollbar has been moved
 */
void ScrollMove(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
...
#ifndef MOTIF
void JumpMove(w, client_data, call_data)
Widget w;
caddr_t client_data;
caddr_t call_data;
{
    HTMLWidget hw = (HTMLWidget) client_data;
    int value = (int)(*(float *)call_data * (w == hw->html.vbar ? hw->html.doc_height : hw->html.doc_width));
    ScrollToPos(w, hw, value);
}
#endif
...
#endif
{
    ...
    XtAddEventHandler((Widget) hw->html.view,
                      ExposureMask | VisibilityChangeMask, True, (XtEventHandler) DrawExpose, (caddr_t) hw);
    hw->html.obscured = 0;
     ...
     */
    XtAppAddActions(XtWidgetToApplicationContext(hw->html.view), SpareActionsList, XtNumber(SpareActionsList));
    ...
    if (hw->html.vbar == NULL) {
        argcnt = 0;
#ifdef MOTIF
        XtSetArg(arg[argcnt], XmNorientation, XmVERTICAL);
        argcnt++;
        hw->html.vbar = XtCreateWidget("Vbar", xmScrollBarWidgetClass, (Widget) hw, arg, argcnt);
#else
        XtSetArg(arg[argcnt], XtNorientation, XtorientVertical);
        argcnt++;
        hw->html.vbar = XtCreateWidget("Vbar", scrollbarWidgetClass, (Widget) hw, arg, argcnt);
#endif
     ...
     */
#ifdef MOTIF
    XtAddCallback(hw->html.vbar, XmNvalueChangedCallback, (XtCallbackProc) ScrollMove, (caddr_t) hw);

    XtAddCallback(hw->html.vbar, XmNdragCallback, (XtCallbackProc) ScrollMove, (caddr_t) hw);

#else
    XtAddCallback(hw->html.vbar, XtNjumpProc, (XtCallbackProc) JumpMove, (caddr_t) hw);
    XtAddCallback(hw->html.vbar, XtNscrollProc, (XtCallbackProc) ScrollMove, (caddr_t) hw);
#endif
    ...
    if (hw->html.hbar == NULL) {
        argcnt = 0;
#ifdef MOTIF
        XtSetArg(arg[argcnt], XmNorientation, XmHORIZONTAL);
        argcnt++;
        hw->html.hbar = XtCreateWidget("Hbar", xmScrollBarWidgetClass, (Widget) hw, arg, argcnt);
#else
        XtSetArg(arg[argcnt], XtNorientation, XtorientHorizontal);
        argcnt++;
        hw->html.hbar = XtCreateWidget("Hbar", scrollbarWidgetClass, (Widget) hw, arg, argcnt);
#endif
     ...
     */
#ifdef MOTIF
    XtAddCallback(hw->html.hbar, XmNvalueChangedCallback, (XtCallbackProc) ScrollMove, (caddr_t) hw);
    XtAddCallback(hw->html.hbar, XmNdragCallback, (XtCallbackProc) ScrollMove, (caddr_t) hw);
#else
    XtAddCallback(hw->html.hbar, XtNjumpProc, (XtCallbackProc) JumpMove, (caddr_t) hw);
    XtAddCallback(hw->html.hbar, XtNscrollProc, (XtCallbackProc) ScrollMove, (caddr_t) hw);
#endif
}
...
#endif
{
    ...
    XtResizeWidget(hw->html.view, hw->html.view_width, hw->html.view_height, hw->html.view->core.border_width);
    /*
    ...
    if (hw->html.use_vbar == True) {
         ...
         * the viewing area
         */
        XtResizeWidget(hw->html.vbar, hw->html.vbar->core.width, hw->html.view_height + (2 *
#ifdef MOTIF
        ...
        if (hw->html.doc_height == 0) {
            ...
            /* Eric -- your previous equation wasn't doing it.
               This isn't either... */
            ss = (int)((float)hw->html.view_height *
                       ((float)hw->html.view_height / (float)(hw->html.doc_height - (int)hw->html.view_height)));
        ...
        XtSetArg(arg[argcnt], XmNpageIncrement,
                 hw->html.view_height > DEFAULT_INCREMENT ? hw->html.view_height - DEFAULT_INCREMENT : 1);
        argcnt++;
        XtSetValues(hw->html.vbar, arg, argcnt);
#else
        setScrollBar(hw->html.vbar, hw->html.scroll_y, hw->html.doc_height, hw->html.view_height);
#endif                          /* MOTIF */
    ...
    if (hw->html.use_hbar == True) {
         ...
         * the viewing area
         */
        XtResizeWidget(hw->html.hbar, hw->html.view_width + (2 *
#ifdef MOTIF
                                                             hw->manager.shadow_thickness
#else
                                                             0
...
#ifdef NOT_RIGHT
            ss = hw->html.view_width * hw->html.view_width / hw->html.doc_width;
        ...
        argcnt++;
        XtSetArg(arg[argcnt], XmNpageIncrement,
                 hw->html.view_width > DEFAULT_INCREMENT ? hw->html.view_width - DEFAULT_INCREMENT : 1);
        argcnt++;
        XtSetValues(hw->html.hbar, arg, argcnt);
#else
        setScrollBar(hw->html.hbar, hw->html.scroll_x, hw->html.doc_width, hw->html.view_width);
#endif                          /* MOTIF */
    }
...
#endif
{
    ...
    if (hw->html.doc_width > hw->html.view_width) {
        ...
        if (hw->html.hbar_top == True) {
            if (hw->html.use_vbar == True) {
                XtMoveWidget(hw->html.vbar, hw->html.vbar->core.x, sheight);
            }
            ...
            if (hw->html.vbar_right == True) {
                ...
                XtMoveWidget(hw->html.hbar, swidth, (hw->core.height - sheight));
            }
        }
        XtManageChild(hw->html.hbar);
...
HTMLWidget new;
#else
Initialize(HTMLWidget request, HTMLWidget new)
#endif
{
    /*
    ...
    new->html.html_header_objects = HTMLParse(NULL, request->html.header_text, new);
    new->html.html_footer_objects = HTMLParse(NULL, request->html.footer_text, new);
...
void DebugHook(x, y, width, height)
...
int width, height;
#else
ViewRedisplay(HTMLWidget hw, int x, int y, int width, int height)
#endif
{
    int sx, sy;
    int doc_x, doc_y;
     ...
     */
    guess = doc_y / (hw->html.font->max_bounds.ascent + hw->html.font->max_bounds.descent);
    if (guess > end) {
        guess = end;
    }
    while (guess > 0) {
        if ((hw->html.line_array[guess] != NULL) && (hw->html.line_array[guess]->y < doc_y)) {
            break;
        }
        guess--;
    }
...
#endif
{
    ...
    if (hw->html.drawGC != NULL) {
        ...
        hw->manager.top_shadow_color = hw->manager.top_shadow_color ? 0 : 1;
        hw->manager.bottom_shadow_color = hw->manager.bottom_shadow_color ? 0 : 1;

        XtVaSetValues(hw->html.view,
                      XmNbackground, hw->core.background_pixel, XmNtopShadowColor, r, XmNbottomShadowColor, b, NULL);
...
Region region;
#else
Redisplay(HTMLWidget hw, XEvent *event, Region region)
#endif
{
...
#endif
{
    ...
    if ((tempw == hw->html.view_width) &&
        (((hw->html.use_vbar == True) &&
         ...
         ((hw->html.use_vbar == False) && ((hw->core.height - sheight - (2 * st)) >= hw->html.doc_height)))) {
        /*
         * Super special case where the size of the window hasn't
         * changed at ALL!
         */
        if (((hw->html.use_hbar == True) && (hw->html.view_height == (hw->core.height - sheight - (2 * st))))
            || ((hw->html.use_hbar == False)
                && (hw->html.view_height == (hw->core.height - (2 * st))))) {
            return;
        }
 ...
 */
static void FindSelectAnchor(hw, aptr)
HTMLWidget hw;
struct ele_rec *aptr;
{
    struct ele_rec *eptr;
    ...
    while ((eptr->prev != NULL) &&
           (eptr->prev->anchorHRef != NULL) && (strcmp(eptr->prev->anchorHRef, eptr->anchorHRef) == 0)) {
        eptr = eptr->prev;
    }
    ...
    while ((eptr->next != NULL) &&
           (eptr->next->anchorHRef != NULL) && (strcmp(eptr->next->anchorHRef, eptr->anchorHRef) == 0)) {
        eptr = eptr->next;
    }
 ...
 * from start to end.
 */
static void DrawSelection(hw, start, end, start_pos, end_pos)
HTMLWidget hw;
struct ele_rec *start;
struct ele_rec *end;
 ...
 */
static void EraseSelection(hw, start, end, start_pos, end_pos)
HTMLWidget hw;
struct ele_rec *start;
struct ele_rec *end;
int start_pos, end_pos;
 ...
 * flashing.
 */
static void ChangeSelection(hw, start, end, start_pos, end_pos)
HTMLWidget hw;
struct ele_rec *start;
struct ele_rec *end;
int start_pos, end_pos;
{
    ...
    if ((ElementLessThan(old_end, new_start, old_end_pos, new_start_pos)) ||
        ...
        if (new_start_pos != 0) {
            EraseSelection(hw, old_start, new_start, old_start_pos, new_start_pos - 1);
        } else {
            EraseSelection(hw, old_start, new_start->prev, old_start_pos, new_start->prev->edata_len - 2);
        }
        if (old_end_pos < (old_end->edata_len - 2)) {
            DrawSelection(hw, old_end, new_end, old_end_pos + 1, new_end_pos);
        } else {
        ...
        if (old_start_pos != 0) {
            DrawSelection(hw, new_start, old_start, new_start_pos, old_start_pos - 1);
        } else {
            DrawSelection(hw, new_start, old_start->prev, new_start_pos, old_start->prev->edata_len - 2);
        }
        if (new_end_pos < (new_end->edata_len - 2)) {
            EraseSelection(hw, new_end, old_end, new_end_pos + 1, old_end_pos);
        } else {
            EraseSelection(hw, new_end->next, old_end, 0, old_end_pos);
        }
    } else if ((ElementLessThan(new_start, old_start,
                                new_start_pos, old_start_pos)) ||
               (ElementLessThan(old_end, new_end, old_end_pos, new_end_pos))) {
...
Cardinal *num_params;           /* unused */
{
     ...
     */
    EraseSelection(hw, hw->html.new_start, hw->html.new_end, hw->html.new_start_pos, hw->html.new_end_pos);

    /*
     * We want to erase the currently selected text, but still save the
     * selection internally in case we don't create a new one.
     */
    EraseSelection(hw, hw->html.select_start, hw->html.select_end, hw->html.sel_start_pos, hw->html.sel_end_pos);
...
Cardinal *num_params;           /* unused */
{
    ...
    if ((eptr != NULL) && (eptr->internal == False)) {
        ...
        if (SwapElements(hw->html.new_start, hw->html.new_end, hw->html.new_start_pos, hw->html.new_end_pos)) {
            if (SwapElements(eptr, hw->html.new_end, epos, hw->html.new_end_pos)) {
            ...
            if (SwapElements(eptr, hw->html.new_start, epos, hw->html.new_start_pos)) {
                start = hw->html.new_start;
                start_pos = hw->html.new_start_pos;
                end = eptr;
                end_pos = epos;
...
Cardinal *num_params;           /* unused */
{
     ...
     * Ignore special internal text
     */
    if ((eptr != NULL) && ((eptr != hw->html.new_end) || (epos != hw->html.new_end_pos)) && (eptr->internal == False)) {
        start = hw->html.new_start;
        start_pos = hw->html.new_start_pos;
        end = eptr;
        end_pos = epos;
...
Cardinal *num_params;
{
    ...
    if (((BuEvent->button == Button1) || (BuEvent->button == Button2)) &&
        (eptr != NULL) &&
        (hw->html.active_anchor != NULL) &&
        (eptr == hw->html.active_anchor) && ((BuEvent->time - hw->html.but_press_time) < CLICK_TIME)) {
    ...
    if ((eptr != NULL) && (eptr->internal == False) && (hw->html.new_end != NULL)) {
        start = hw->html.new_start;
        start_pos = hw->html.new_start_pos;
        end = eptr;
        end_pos = epos;
            ...
            if (buffer >= 0) {
                if (hw->html.fancy_selections == True) {
                    text = ParseTextToPrettyString(hw,
                                                   ...
                                                   hw->html.sel_end_pos,
                                                   hw->html.font->max_bounds.width, hw->html.margin_width);
                } else {
                    text =
                        ParseTextToString(hw->html.formatted_elements,
                                          hw->html.select_start,
                                          hw->html.select_end,
                                          hw->html.sel_start_pos,
                                          hw->html.sel_end_pos, hw->html.font->max_bounds.width, hw->html.margin_width);
                }
                XStoreBuffer(XtDisplay((Widget) hw), text, strlen(text), buffer);
                if (text != NULL) {
                    free(text);
                }
            } else {
                XtOwnSelection((Widget) hw, atoms[i], BuEvent->time, (XtConvertSelectionProc)
                               ConvertSelection,
                               (XtLoseSelectionProc) LoseSelection, (XtSelectionDoneProc) SelectionDone);
            }
        }
        free((char *)atoms);
    } else if (eptr == NULL) {
...
static void TrackMotion(w, event, params, num_params)
Widget w;
XEvent *event;
String *params;                 /* unused */
Cardinal *num_params;           /* unused */
{
    ...
    } else if (event->type == LeaveNotify || event->type == FocusOut || event->type == Expose) {
        /* Wipe out. */
    ...
    if (eptr != NULL && eptr != hw->html.cached_tracked_ele && eptr->anchorHRef != NULL) {
    ...
    else if (eptr != NULL && hw->html.cached_tracked_ele != NULL && eptr->anchorHRef == NULL) {
        LEAVING_ANCHOR(hw);
    }
...
Cardinal *num_params;           /* unused */
#else
_HTMLInput(Widget w, XEvent *event, String *params, /* unused */
           Cardinal *num_params)    /* unused */
#endif
{
    HTMLWidget hw = (HTMLWidget) XtParent(w);
    ...
    if (event->type == ButtonRelease) {
        ...
        if (eptr != NULL) {
            if (eptr->anchorHRef != NULL) {
                ...
                if ((IsDelayedHRef(hw, eptr->anchorHRef)) && (hw->html.resolveDelayedImage != NULL)) {
                    ...
                    if (eptr->pic_data == NULL) {
                         ...
                         */
                        if ((eptr->edata != NULL) &&
                            (strncmp(eptr->edata, INTERNAL_IMAGE, strlen(INTERNAL_IMAGE)) == 0)) {
                    ...
                    ViewClearAndRefresh(hw);
                           ...
                           (eptr->anchorHRef != NULL) && (IsIsMapForm(hw, eptr->anchorHRef))) {
                    eptr->pic_data = (*(resolveImageProc)
                                      (hw->html.resolveDelayedImage)) (hw, eptr->edata);
                    ...
                    ViewClearAndRefresh(hw);
                           ...
                           (((event->xbutton.y + hw->html.scroll_y) -
                             (eptr->y + eptr->y_offset)) > AnchoredHeight(hw))) {
                    eptr->pic_data = (*(resolveImageProc)
                                      (hw->html.resolveDelayedImage)) (hw, eptr->edata);
                    ...
                    ImageSubmitForm(eptr->pic_data->fptr, event, eptr->pic_data->text, form_x, form_y);
                } else {
                    ...
                    if (eptr->type == E_IMAGE && eptr->pic_data && eptr->pic_data->ismap) {
                        buf = (char *)
                        ...
                        sprintf(buf, "%s?%d,%d",
                                eptr->anchorHRef,
                                event->xbutton.x +
                                hw->html.scroll_x - eptr->x, event->xbutton.y + hw->html.scroll_y - eptr->y);
                    }
                    /*
                    ...
                    cbdata.text = tptr;
                    XtCallCallbackList((Widget) hw, hw->html.anchor_callback, (XtPointer) & cbdata);
                    if (alloced)
                        free(buf);
                    if (tptr != NULL)
                        free(tptr);
...
Cardinal *num_params;           /* unused */
#else
_HTMLpwdInput(Widget w, XEvent *event, String *params,  /* unused */
              Cardinal *num_params) /* unused */
#endif
{
    char buffer[50];
    ...
    if (event->type == KeyPress) {
        ...
        length = XLookupString((XKeyEvent *) event, buffer, 50, &ks, NULL);
        keySymString = XKeysymToString(ks);
        XtVaGetValues(w,
                      XtNinsertPosition, &insertPos, XtNuseStringInPlace, &stringInPlace, XtNlength, &maxLength, NULL);
        ...
        if ((!strcmp("BackSpace", keySymString))
            ...
            if (passwdLength > 0) {
                ...
                XtCallActionProc(w,
                                 insertPos >
                                 -1 ? "delete-previous-character" : "delete-next-character", event, NULL, 0);
            }
        ...
        if (length == 1) {
            ...
            if (passwdLength > 0) {
                ...
                if (passwdLength < maxLength) {
                    char *pwd = wptr->password = (char *)realloc(wptr->password,
 ...
 */
    static Boolean
#ifdef _NO_PROTO
     SetValues(current, request, new) HTMLWidget current;
    HTMLWidget request;
    HTMLWidget new;
#else
     SetValues(HTMLWidget current, HTMLWidget request, HTMLWidget new)
#endif
    {
/*	int reformatted;*/
        ...
        if ((request->html.raw_text != current->html.raw_text) ||
            ...
            FreeColors(XtDisplay(current),
                       (installed_colormap ? installed_cmap : DefaultColormapOfScreen(XtScreen(current))));
            FreeImages(current);
             ...
             */
            new->html.html_objects = HTMLParse(current->html.html_objects, request->html.raw_text, new);
            CallLinkCallbacks(new);
            new->html.html_header_objects =
                HTMLParse(current->html.html_header_objects, request->html.header_text, new);
            new->html.html_footer_objects =
                HTMLParse(current->html.html_footer_objects, request->html.footer_text, new);
 ...
 */
    static void
#ifdef _NO_PROTO
     CallLinkCallbacks(hw) HTMLWidget hw;
#else
...
#endif
    {
        ...
        while (mptr != NULL) {
            if (mptr->type == M_BASE) {
                l_info.href = ParseMarkTag(mptr->start, MT_BASE, "HREF");
                l_info.role = ParseMarkTag(mptr->start, MT_BASE, "ROLE");
                XtCallCallbackList((Widget) hw, hw->html.link_callback, (XtPointer) & l_info);
                if (l_info.href != NULL) {
                    free(l_info.href);
                }
                if (l_info.role != NULL) {
 ...
 */
    static void
#ifdef _NO_PROTO
     RecolorInternalHRefs(hw, href) HTMLWidget hw;
    char *href;
#else
...
#endif
    {
        ...
        while (start != NULL) {
             ...
             */
            if ((start->internal == True) && (start->anchorHRef != NULL) && (strcmp(start->anchorHRef, href) == 0)) {
                start->fg = fg;
    ...
    int *format;
    {
        ...
        if (*target == XA_TARGETS(d)) {
            ...
            XmuConvertStandardSelection(w, hw->html.selection_time,
                                        selection, target, type, (caddr_t *) & std_targets, &std_length, format);
...
*/
        ...
        if (*target == XA_STRING || *target == XA_TEXT(d) || *target == XA_COMPOUND_TEXT(d)) {
...
*/
        ...
        if (XmuConvertStandardSelection(w, hw->html.selection_time, selection, target, type, value, length, format)) {
            return True;
        }
    ...
    static void LoseSelection(w, selection) Widget w;
    Atom *selection;
    {
        HTMLWidget hw = (HTMLWidget) w;
    ...
    static void SelectionDone(w, selection, target) Widget w;
 ...
 */
    char *
#ifdef _NO_PROTO
     HTMLGetText(w, pretty, url, time_str) Widget w;
    int pretty;
    char *url;
    char *time_str;
#else
...
#endif
    {
        ...
        if (pretty >= 2) {
            ...
            tptr = ParseTextToPrettyString(hw, start, start, end,
                                           0, 0, hw->html.font->max_bounds.width, hw->html.margin_width);
        } else {
            tptr = ParseTextToString(start, start, end, 0, 0, hw->html.font->max_bounds.width, hw->html.margin_width);
        }
        if (tptr != NULL) {
            if (text == NULL) {
                text = tptr;
 ...
 */
    int
#ifdef _NO_PROTO
     HTMLPositionToId(w, x, y) Widget w;
    int x, y;
#else
 ...
 */
    int
#ifdef _NO_PROTO
     HTMLIdToPosition(w, element_id, x, y) Widget w;
    int element_id;
    int *x, *y;
#else
 ...
 */
    void
#ifdef _NO_PROTO
     HTMLGotoId(w, element_id, correction) Widget w;
    int element_id;
    int correction;
#else
...
#endif
    {
        ...
        setScrollBar(hw->html.vbar, newy, hw->html.doc_height, hw->html.view_height);
#endif
    }
 ...
 */
    int
#ifdef _NO_PROTO
     HTMLAnchorToPosition(w, name, x, y) Widget w;
    char *name;
    int *x, *y;
#else
 ...
 */
    int
#ifdef _NO_PROTO
     HTMLAnchorToId(w, name) Widget w;
    char *name;
#else
 ...
 */
    char **
#ifdef _NO_PROTO
     HTMLGetHRefs(w, num_hrefs) Widget w;
    int *num_hrefs;
#else
 ...
 */
    char **
#ifdef _NO_PROTO
     HTMLGetImageSrcs(w, num_srcs) Widget w;
    int *num_srcs;
#else
...
#endif
    {
        ...
        while (mptr != NULL) {
            ...
            } else /****** temporary until figure support */ if (mptr->type ==
                                                                 M_FIGURE) {
        ...
        if (cnt == 0) {
            ...
            *num_srcs = cnt;
            ...
            while (mptr != NULL) {
                ...
                } else /****** temporary until figure support */ if (mptr->
                                                                     type == M_FIGURE) {
                    tptr = ParseMarkTag(mptr->start, MT_FIGURE, "SRC");
                    if ((tptr != NULL) && (*tptr != '\0')) {
                        harray[cnt] = tptr;
 ...
 */
    LinkInfo *
#ifdef _NO_PROTO
     HTMLGetLinks(w, num_links) Widget w;
    int *num_links;
#else
...
#endif
    {
        ...
        FreeColors(XtDisplay(w), (installed_colormap ? installed_cmap : DefaultColormapOfScreen(XtScreen(w))));
        FreeImages(hw);
    }

    void
#ifdef _NO_PROTO
     HTMLFreeWidgetInfo(ptr) void *ptr;
#else
 ...
 */
    void
#ifdef _NO_PROTO
     HTMLRetestAnchors(w, testFunc) Widget w;
 ...
 */
    void
#ifdef _NO_PROTO
     HTMLSetSelection(w, start, end) Widget w;
    ElementRef *start;
    ElementRef *end;
#else
 ...
 */
    char *
#ifdef _NO_PROTO
     HTMLGetTextAndSelection(w, startp, endp, insertp) Widget w;
    char **startp;
    char **endp;
    char **insertp;
#else
...
#endif
    {
        ...
        if (SwapElements(hw->html.select_start, hw->html.select_end, hw->html.sel_start_pos, hw->html.sel_end_pos)) {
 ...
 */
    void
#ifdef _NO_PROTO
     HTMLSetText(w, text, header_text, footer_text, element_id, target_anchor, ptr) Widget w;
    char *text;
    char *header_text;
    char *footer_text;
    int element_id;
    ...
    void *ptr;
#else
     HTMLSetText(Widget w, char *text, char *header_text,
                 char *footer_text, int element_id, char *target_anchor, void *ptr)
#endif
    {
        HTMLWidget hw = (HTMLWidget) w;
        WidgetInfo *wptr = (WidgetInfo *) ptr;
        ...
        if (text != NULL) {
             ...
             * Free any old colors and pixmaps
             */
            FreeColors(XtDisplay(hw), (installed_colormap ? installed_cmap : DefaultColormapOfScreen(XtScreen(hw))));
            FreeImages(hw);

            /*
             * Parse the raw text with the HTML parser
             */
            hw->html.html_objects = HTMLParse(hw->html.html_objects, hw->html.raw_text, hw);
        ...
        if (header_text != NULL) {
             ...
             */
            hw->html.html_header_objects = HTMLParse(hw->html.html_header_objects, hw->html.header_text, hw);
        }
        if (footer_text != NULL) {
            if (*footer_text == '\0') {
                footer_text = NULL;
             ...
             */
            hw->html.html_footer_objects = HTMLParse(hw->html.html_footer_objects, hw->html.footer_text, hw);
        }
 ...
 */
    int
#ifdef _NO_PROTO
     HTMLSearchNews(w, m_start, m_end) Widget w;
    ElementRef *m_start;
    ElementRef *m_end;
#else
 ...
 */
    int
#ifdef _NO_PROTO
     HTMLSearchText(w, pattern, m_start, m_end, backward, caseless) Widget w;
    char *pattern;
    ElementRef *m_start;
    ElementRef *m_end;
    int backward;
    int caseless;
#else
     HTMLSearchText(Widget w, char *pattern, ElementRef * m_start, ElementRef * m_end, int backward, int caseless)
#endif
    {
        HTMLWidget hw = (HTMLWidget) w;
        int found, equal;
         ...
         * If bad parameters are passed, just fail the search
         */
        if ((pattern == NULL) || (*pattern == '\0') || (m_start == NULL) || (m_end == NULL)) {
            return (-1);
        }
                ...
                if (eptr->type == E_TEXT) {
                    ...
                    while (tptr >= eptr->edata) {
                        if (equal) {
                            ...
                            while ((mptr >= match) && (tptr >= eptr->edata) && (cval == *mptr)) {
                                tptr--;
                                mptr--;
                                if (tptr >= eptr->edata) {
                                    if (caseless) {
...
/* ???? Should this be cast to a HTMLWidget and XtWindow changed to html.view*/
        sens_win = XCreateWindow(XtDisplay((Widget) hw), XtWindow(hw), x,
                                 y, width, height, 0, CopyFromParent, InputOnly, CopyFromParent, 0, NULL);
 ...
 */
    void HTMLDrawBackgroundImage(Widget wid, int x, int y, int width, int height) {
        ...
        int w_whole = 0, h_whole = 0,
            ...
            w_whole_tiles = 0, h_whole_tiles = 0, end_width = 0, end_height = 0, w, h, destx = 0, desty = 0;
        HTMLWidget hw = (HTMLWidget) wid;

        if (!hw || (x < 0) || (y < 0) || width <= 0 || height <= 0 ||
            !hw->html.bg_width || !hw->html.bg_height || !hw->html.bgmap_SAVE || !hw->html.bg_image) {
                ...
                XCopyArea(XtDisplay(wid),
                          ...
                          (!h ?
                           (start_height ?
                            ...
                            hw->html.bg_height) :
                           ((h + 1) == h_whole ?
                            (end_height ? end_height : hw->html.bg_height) : hw->html.bg_height)), destx, desty);
                destx += (!w ? (start_width ? start_width : hw->html.bg_width) : hw->html.bg_width);
            }
            desty += (!h ? (start_height ? start_height : hw->html.bg_height) : hw->html.bg_height);
        }
