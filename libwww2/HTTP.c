...
PUBLIC int HTLoadHTTP ARGS4(char *, arg, HTParentAnchor *, anAnchor, HTFormat, format_out, HTStream *, sink)
{
...
*/
        ...
        *(addr + i) = 0;
        ...
        if (lsocket != -1)
            NETCLOSE(lsocket);  /* no socket leaks here */
...
*/
    ...
    if (extensions) {
         ...
         */
        if (reloading)
            StrAllocCat(command, "Pragma: no-cache\r\nCache-Control: no-cache\r\n");
...
#endif
        sprintf(line, "Content-type: %s%c%c", post_content_type ? post_content_type : "lose", CR, LF);
        StrAllocCat(command, line);
        {
            int content_length;
            if (!post_data)
...
#endif
            /* Do nothing. */
        } else if ((errno == ENOTCONN || errno == ECONNRESET || errno == EPIPE) && !already_retrying &&
                   /* Don't retry if we're posting. */ !do_post) {
            /* Arrrrgh, HTTP 0/1 compability problem, maybe. */
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "HTTP: BONZO ON WRITE Trying again with HTTP0 request.\n");
#endif
/*
...

    {
        ...
        do {
            ...
            if (buffer_length - length < LINE_EXTEND_THRESH) {
                ...
                line_buffer = (char *)realloc(line_buffer, buffer_length * sizeof(char));
            }
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "HTTP: Trying to read %d\n", buffer_length - length - 1);
#endif
            status = NETREAD(s, line_buffer + length, buffer_length - length - 1);
#ifndef DISABLE_TRACE
            if (www2Trace)
            ...
            if (status <= 0) {
                ...
                if (status == HT_INTERRUPTED) {
                    ...
                    (status < 0 &&
                     (errno == ENOTCONN || errno == ECONNRESET || errno == EPIPE) && !already_retrying && !do_post) {
                    /* Arrrrgh, HTTP 0/1 compability problem, maybe. */
#ifndef DISABLE_TRACE
                    if (www2Trace)
...
#endif
                    HTProgress("Unexpected network read error; connection aborted.");
...
*/
...

    {
        ...
        if (fields < 2 || !server_version[0] || server_version[0] != 'H' ||
            ...
            server_version[3] != 'P' || server_version[4] != '/' || server_version[6] != '.') {
            /* HTTP0 reply */
            HTAtom *encoding;
    ...
    if (!target) {
        char buffer[1024];      /* @@@@@@@@ */
        sprintf(buffer, "Sorry, no known way of converting %s to %s.", HTAtom_name(format_in), HTAtom_name(format_out));
        HTProgress(buffer);
        status = -1;
        NETCLOSE(s);
        lsocket = -1;
