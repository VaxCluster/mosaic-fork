...
PRIVATE void output_text_record ARGS3(HTStream *, target, WAISDocumentText *, record, boolean, quote_string_quotes)
{
    if (record->DocumentText->size) {
        ...
        PUTBLOCK((unsigned char *)record->DocumentText->bytes, record->DocumentText->size);
    }
}                               /* output text record */
 ...
 */
void display_search_response ARGS4(HTStructured *, target,
                                   SearchResponseAPDU *, response, char *, database, char *, keywords)
{
    ...
    sprintf(line,
            ...
            database,
            (int)(response->NumberOfRecordsReturned), response->NumberOfRecordsReturned == 1 ? "" : "s", keywords);
...
*/
                ...
                if (www2Trace)
                    fprintf(stderr,
                            ...
                            i,
                            (long int)(info->DocHeaders[k]->Score), (long int)(info->DocHeaders[k]->Lines), headline);
#endif
                START(HTML_LI);
                sprintf(line, "%4ld  %4ld  ", head->Score, head->Lines);
                PUTS(line);
                ...
                if (archie) {
                    ...
                    if (docname) {
                        ...
                        sprintf(line, "%s/%s/%d/%s", dbname, types_array, (int)(head->DocumentLength), docname);

                        PUTS("<A HREF=\"");
                        if (head->Types && head->Types[0] && strcmp(head->Types[0], "URL") == 0) {
                            /* The real URL, maybe? */
#ifndef DISABLE_TRACE
                            if (www2Trace)
...
*/
PUBLIC int HTLoadWAIS ARGS4(WWW_CONST char *, arg, HTParentAnchor *, anAnchor, HTFormat, format_out, HTStream *, sink)
{
...
#ifndef DISABLE_TRACE
                    if (www2Trace)
                        fprintf(stderr, "WAIS: doctype '%s', doclength\n~~~~\n%s\n~~~~\n", doctype, doclength);
#endif
                    /* Multitype! */
                    if (strchr(doctype, ',')) {
...
#ifndef DISABLE_TRACE
                        if (www2Trace) {
                            fprintf(stderr, "WAIS: Hey boss, got multitype.\n");
                            fprintf(stderr, "WAIS: names is '%s'\n", names);
                        }
#endif
...
#endif
        status = mosaic_connect_to_server(server_name, atoi(service), &connection);
        if (status == 0) {
#ifndef DISABLE_TRACE
            if (www2Trace)
    ...
    if (key && !*key) {         /* I N D E X */
        ...
        if (NULL == generate_search_apdu(request_message + HEADER_LENGTH,
                                         &request_buffer_length, keywords, wais_database, NULL, MAXDOCS)) {
            HTProgress("WAIS request too large; something went wrong.");
        }

        if (!interpret_message(request_message, MAX_MESSAGE_LEN - request_buffer_length, response_message, MAX_MESSAGE_LEN, connection, false   /* true verbose */
            )) {
            ...
            display_search_response(target, query_response, wais_database, keywords);
            if (query_response) {
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "===WAIS=== Doing HTAtom_exists on '%s'\n", tmp_doctype);
#endif
        ...
        if (!format_in) {
            ...
            format_in = HTFileFormat(dummy, &pencoding, WWW_PLAINTEXT, &compressed);
            ...
            while (1) {
                ...
                if (generate_retrieval_apdu
                    (request_message + HEADER_LENGTH,
                     ...
                     docid, CT_byte, count * CHARS_PER_PAGE, (count + 1) * CHARS_PER_PAGE, type, wais_database) == 0) {
                ...
                if (0 == interpret_message(request_message, MAX_MESSAGE_LEN - request_buffer_length, response_message, MAX_MESSAGE_LEN, connection, false   /* true verbose */
                    )) {
                    HTProgress("WAIS error condition; retrieval may be unsuccessful.");
                    goto no_more_data;
                }
                 ...
                 */
                readSearchResponseAPDU(&retrieval_response, response_message + HEADER_LENGTH);

                response = (WAISSearchResponse *) retrieval_response->DatabaseDiagnosticRecords;
                ...
                if (NULL == response->Text) {
                    ...
                    output_text_record(target, ((WAISSearchResponse *)
                                                retrieval_response->DatabaseDiagnosticRecords)->Text[0], false);
                }               /* If text existed */
                else {
#ifndef DISABLE_TRACE
                    if (www2Trace)
                ...
                if (diag && diag[0] && diag[0]->ADDINFO != NULL && !strcmp(diag[0]->DIAG, D_PresentRequestOutOfRange)) {
#ifndef DISABLE_TRACE
                    if (www2Trace)
