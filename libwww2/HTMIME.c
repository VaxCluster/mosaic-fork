...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Got C at beginning of line; checking for 'ontent-'\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME] Got E at beginning of line; checking for 'X'\n");
#endif
            }
            break;
        case 'l':
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Got L at beginning of line\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Got W at beginning of line; checking for 'ww-authenticate'\n");
#endif
            break;
#endif
                ...
                if (www2Trace)
                    fprintf(stderr,
                            ...
                            (!me->compression_encoding
                             || !*me->compression_encoding ? "Undefined" : me->compression_encoding));
#endif
...
#endif
                me->target = HTStreamStack(me->format, me->targetRep, compressed, me->sink, me->anchor);
                if (!me->target) {
#ifndef DISABLE_TRACE
                    if (www2Trace) {
                    ...
                    me->target = HTStreamStack(HTAtom_for("text/html"),
                                               me->targetRep, compressed, me->sink, me->anchor);
                }
                if (me->target) {
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was CONTENT_, found T, state now CONTENT_T\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was CONTENT_, found E, checking for 'ncoding:'\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was CONTENT_, found L, checking for 'ength:'\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was CONTENT_, found nothing; bleah\n");
#endif
            goto bad_field_name;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was CONTENT_T; going to check for ansfer-encoding:\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was CONTENT_T; found nothing; bleah\n");
#endif
            goto bad_field_name;
        }                       /* switch on character */
        break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was L; going to check for st-modified:\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was L; going to check for ocation:\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was L; found nothing; bleah\n");
#endif
            goto bad_field_name;
        }                       /* switch on character */
        break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was EX; going to check for EXP or EXT:\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was E; found nothing; bleah\n");
#endif
            goto bad_field_name;
        }                       /* switch on character */
        break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was EXP; going to check for 'ires'\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was EXT; going to check for 'ension:'\n");
#endif
            break;
...
#ifndef DISABLE_TRACE
            if (www2Trace)
                fprintf(stderr, "[MIME] Was EX; found nothing; bleah\n");
#endif
            goto bad_field_name;
        }                       /* switch on character */
        break;
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME_put_char] Picked up transfer_encoding '%s'\n", me->encoding);
#endif
                break;
            case CONTENT_ENCODING:
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME_put_char] Picked up compression encoding '%s'\n", me->compression_encoding);
#endif
                break;
            case CONTENT_LENGTH:
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME_put_char] Picked up content length '%d'\n", me->content_length);
#endif
                break;
            case EXPIRES:
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME_put_char] Picked up expires '%s'\n", me->value);
#endif
                break;
            case LAST_MODIFIED:
                if (me->value_pointer < me->value + VALUE_SIZE - 1) {
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME_put_char] Picked up last modified '%s'\n", me->value);
#endif
                break;
            case LOCATION:
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME_put_char] Picked up location '%s'\n", me->location);
#endif
                break;
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[MIME put char] picked up Auth. arg '%s'\n", me->value);
#endif
                if (got_kerb) {
...
PRIVATE void HTMIME_free ARGS1(HTStream *, me)
{
    if (!me->target) {
        ...
        me->target = HTStreamStack(me->format, me->targetRep, 0, me->sink, me->anchor);
#ifndef DISABLE_TRACE
        if (www2Trace)
    ...
    if (me->expires) {
        ...
        for (p = HTTP_expires + strlen(HTTP_expires) - 1; p > HTTP_expires && isspace(*p); p--) {
            *p = '\0';
        }
    ...
    if (me->last_modified) {
        ...
        for (p = HTTP_last_modified + strlen(HTTP_last_modified) - 1; p > HTTP_last_modified && isspace(*p); p--) {
            *p = '\0';
        }
...
PUBLIC HTStream *HTMIMEConvert ARGS5(HTPresentation *, pres,
