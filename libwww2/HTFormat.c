...
**	-------------------------------------------------------
*/
PUBLIC void HTSetPresentation ARGS5(WWW_CONST char *, representation,
                                    WWW_CONST char *, command, float, quality, float, secs, float, secs_per_byte)
{
...
**	---------------------------------------------
*/
PUBLIC void HTSetConversion ARGS6(WWW_CONST char *, representation_in,
                                  WWW_CONST char *, representation_out,
                                  HTConverter *, converter, float, quality, float, secs, float, secs_per_byte)
{
...
*/
PUBLIC void HTRemoveConversion ARGS3(WWW_CONST char *, representation_in,
                                     WWW_CONST char *, representation_out, HTConverter *, converter)
{
    ...
    for (x = 0; x < numberOfPresentations; x++) {
        ...
        if (pres) {
            if ((!strcmp(pres->rep->name, rep_in->name)) &&
                (!strcmp(pres->rep_out->name, rep_out->name)) && (pres->converter == converter)) {
...
*/
PUBLIC int HTOutputBinary ARGS2(int, input, FILE *, output)
{
    do {
...
PUBLIC HTStream *HTStreamStack ARGS5(HTFormat, format_in,
                                     HTFormat, rep_out, int, compressed, HTStream *, sink, HTParentAnchor *, anchor)
{
...
#ifndef DISABLE_TRACE
        if (www2Trace)
            fprintf(stderr, "[HTStreamStack] rep_out == WWW_SOURCE | rep_out == format_in; returning sink\n");
#endif
        return sink;
    }
...
#endif
            if (pres->rep == format_in || partial_wildcard_matches(pres->rep, format_in)) {
...
#ifndef DISABLE_TRACE
                    if (www2Trace)
                        fprintf(stderr, "[HTStreamStack] pres->rep_out == rep_out\n");
#endif
...
#ifndef DISABLE_TRACE
                    if (www2Trace)
                        fprintf(stderr, "[HTStreamStack] pres->rep_out == wildcard\n");
#endif
...
*/
PUBLIC float HTStackValue ARGS4(HTFormat, format_in, HTFormat, rep_out, float, initial_value, long int, length)
{
    HTAtom *wildcard = HTAtom_for("*");
...

    {
        ...
        for (i = 0; i < n; i++) {
            pres = HTList_objectAt(HTPresentations, i);
            if (pres->rep == format_in && (pres->rep_out == rep_out || pres->rep_out == wildcard)) {
                float value = initial_value * pres->quality;
                if (HTMaxSecs != 0.0)
                    value = value - (length * pres->secs_per_byte + pres->secs) / HTMaxSecs;
                return value;
            }
        }
    }
...
PUBLIC int HTCopy ARGS3(int, file_number, HTStream *, sink, int, bytes_already_read)
{
...
/*  if(loading_length != -1) left = loading_length;*/
    ...
    for (;;) {
        ...
        if (loading_length == -1) {
            ...
            if (left > 0)
                status = NETREAD(file_number, input_buffer, (left > INPUT_BUFFER_SIZE ? INPUT_BUFFER_SIZE : left));
            else
                status = 0;
        }
...
/*	  left = loading_length - total_read;*/
        ...
        if (loading_length == -1) {
            msg = (loading_inlined_images ? "Read %d bytes of inlined image data." : "Read %d bytes of data.");
            sprintf(line, msg, bytes);
/*	HTMeter(0,NULL);*/
        } else {
            msg = (loading_inlined_images ?
                   "Read %d of %d bytes of inlined image data." : "Read %d of %d bytes of data.");
...
**
*/
PUBLIC void HTFileCopy ARGS2(FILE *, fp, HTStream *, sink)
{
    HTStreamClass targetClass;
...
PUBLIC void HTFileCopyToText ARGS2(FILE *, fp, HText *, text)
{
    for (;;) {
        int status = fread(input_buffer, 1, INPUT_BUFFER_SIZE, fp);
        if (status == 0) {      /* EOF or error */
...
**
*/
PUBLIC int HTParseSocket ARGS6(HTFormat, format_in,
                               HTFormat, format_out,
                               HTParentAnchor *, anchor, int, file_number, HTStream *, sink, int, compressed)
{
    HTStream *stream;
    HTStreamClass targetClass;
    int rv;
    ...
    if (!stream) {
        char buffer[1024];      /* @@@@@@@@ */
        sprintf(buffer, "Sorry, can't convert from %s to %s.", HTAtom_name(format_in), HTAtom_name(format_out));
#ifndef DISABLE_TRACE
        if (www2Trace)
...
**
*/
PUBLIC int HTParseFile ARGS6(HTFormat, format_in,
                             HTFormat, format_out,
                             HTParentAnchor *, anchor, FILE *, fp, HTStream *, sink, int, compressed)
{
    HTStream *stream;
    HTStreamClass targetClass;
