...
*/
PUBLIC void HTSetSuffix ARGS4(WWW_CONST char *, suffix,
                              WWW_CONST char *, representation, WWW_CONST char *, encoding, float, value)
{
...
*/
    ...
    if (!second) {              /* Only one slash */
        ...
        *last = 0;              /* Split dir from filename */
        sprintf(vmsname, "%s%s:[%s]%s", nodename, filename + 1, second + 1, last + 1);
...
PUBLIC char *HTLocalName ARGS1(WWW_CONST char *, name)
{
    ...
    if (0 == strcmp(access, "file")) {
        free(access);
        if (!host || !*host || (0 == my_strcasecmp(host, HTHostName())) || (0 == my_strcasecmp(host, "localhost"))) {
            if (host)
                free(host);
#ifndef DISABLE_TRACE
            if (www2Trace)
...
PUBLIC HTFormat HTFileFormat ARGS4(char *, filename, HTAtom **, pencoding, HTAtom *, default_type, int *, compressed)
{
    HTSuffix *suff;
    int n, i, lf;
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "[HTFileFormat] Got hit on .gz; filename '%s'\n", filename);
#endif
                goto ok_ready;
            }
        }
  ...
  ok_ready:
    ...
    *pencoding = NULL;
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((ls <= lf)
            && 0 == my_strcasecmp(suff->suffix, filename + lf - ls)) {
            int j;
            *pencoding = suff->encoding;
            ...
            for (j = 0; j < n; j++) {   /* Got encoding, need representation */
                ...
                if ((ls <= lf) && 0 == my_strncasecmp(suff->suffix, filename + lf - ls - ls2, ls2))
                    if (suff->rep)
                        goto done;
            }
        }
...
**	------------------------------------------------------
*/
PUBLIC char *HTFileMimeType ARGS2(WWW_CONST char *, filename, WWW_CONST char *, default_type)
{
    HTAtom *pencoding;
    HTFormat format;
    int compressed;

    format = HTFileFormat(filename, &pencoding, HTAtom_for(default_type), &compressed);
    ...
    if (st) {
        ...
        if (www2Trace)
            fprintf(stderr,
                    ...
                    t, (strcmp(t, "Application") == 0 ? " data" : ""), st, host, access);
#endif
...
PUBLIC BOOL HTEditable ARGS1(WWW_CONST char *, filename)
{
    ...
    if (www2Trace) {
        ...
        fprintf(stderr,
                ...
                (unsigned int)fileStatus.st_mode, fileStatus.st_uid, fileStatus.st_gid, myUid, ngroups);
        for (i = 0; i < ngroups; i++)
...
**
*/
PUBLIC void HTDirEntry ARGS3(HTStructured *, target, WWW_CONST char *, tail, WWW_CONST char *, entry)
{
    char *relative;
...
**
*/
PUBLIC int HTLoadFile ARGS4(WWW_CONST char *, addr, HTParentAnchor *, anchor, HTFormat, format_out, HTStream *, sink)
{
    char *filename;
    HTFormat format;
...
/* Assume that the file is in Unix-style syntax if it contains a '/'
   after the leading one @@ */
    {
        char *vmsname = strchr(filename + 1, '/') ? vms_name(nodename, filename) : filename + 1;
...
*/
        if ((strlen(localname) > strlen(MULTI_SUFFIX))
            && (0 == strcmp(localname + strlen(localname) - strlen(MULTI_SUFFIX), MULTI_SUFFIX))) {
            DIR *dp;
            ...
            *base++ = 0;        /* Just got directory name */
            ...
            if (!dp) {
              forget_multi:
                free(localname);
                return HTLoadError(sink, 500, "Multiformat: directory scan failed.");
            }
...
#ifndef DISABLE_TRACE
                        if (www2Trace)
                            fprintf(stderr, "HTFile: value of presenting %s is %f\n", HTAtom_name(rep), value);
#endif
                        if (value > best) {
                            best_rep = rep;
                            best = value;
...
*/
                if (HTDirAccess == HT_DIR_FORBID) {
                    free(localname);
                    return HTLoadError(sink, 403, "Directory browsing is not allowed.");
                }

                if (HTDirAccess == HT_DIR_SELECTIVE) {
                    char *enable_file_name = malloc(strlen(localname) + 1 + strlen(HT_DIR_ENABLE_FILE) + 1);
                    ...
                    if (stat(enable_file_name, &statbuf) != 0) {
                        free(localname);
                        return HTLoadError(sink, 403, "Selective access is not enabled for this directory.");
                    }
                }
...
/* How this for a disgusting loop :) */
                ...
                for (count = 0, dataptr = HTSortFetch(count);
                     dataptr != NULL; free(dataptr), count++, dataptr = HTSortFetch(count)) {
...
/* a compressed file, call it binary no matter what                        */
                    ...
                    if (statbuf.st_mode & S_IFDIR) {
                        ...
                        format = HTFileFormat(dataptr, &pencoding, WWW_SOURCE, &cmpr);
