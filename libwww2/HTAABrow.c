...
*/
PRIVATE HTAAServer *HTAAServer_new ARGS2(WWW_CONST char *, hostname, int, portnumber)
{
    HTAAServer *server;
...
**			NULL, if not found.
*/
PRIVATE HTAAServer *HTAAServer_lookup ARGS2(WWW_CONST char *, hostname, int, portnumber)
{
    if (hostname) {
...
**			
*/
PRIVATE HTAASetup *HTAASetup_lookup ARGS3(WWW_CONST char *, hostname, int, portnumber, WWW_CONST char *, docname)
{
    HTAAServer *server;
    HTAASetup *setup;
    ...
    if (hostname && docname && *hostname && *docname && NULL != (server = HTAAServer_lookup(hostname, portnumber))) {

        HTList *cur = server->setups;

#ifndef DISABLE_TRACE
        if (www2Trace)
            fprintf(stderr, "%s (%s:%d:%s)\n", "HTAASetup_lookup: resolving setup for", hostname, portnumber, docname);
#endif
            ...
            if (HTAA_templateMatch(setup->template, docname)) {
                ...
                if (www2Trace)
                    fprintf(stderr, "%s `%s' %s `%s'\n",
                            "HTAASetup_lookup:", docname, "matched template", setup->template);
#endif
                return setup;
            }
#ifndef DISABLE_TRACE
                ...
                fprintf(stderr, "%s `%s' %s `%s'\n",
                        "HTAASetup_lookup:", docname, "did NOT match template", setup->template);
#endif
        }                       /* while setups remain */
    ...
    if (www2Trace)
        fprintf(stderr, "%s `%s' %s\n",
                "HTAASetup_lookup: No template matched", (docname ? docname : "(null)"), "(so probably not protected)");
#endif
...
PRIVATE HTAASetup *HTAASetup_new ARGS4(HTAAServer *, server,
                                       char *, template, HTList *, valid_schemes, HTAssocList **, scheme_specifics)
{
    HTAASetup *setup;
        ...
        if (r) {
            ...
            if (www2Trace)
                fprintf(stderr, "Clearing %s %s:%s\n",
                        ...
                        r->realmname ? r->username : "NULL", r->realmname ? r->password : "NULL");
...
*/
PRIVATE HTAARealm *HTAARealm_lookup ARGS2(HTList *, realm_table, WWW_CONST char *, realmname)
{
...
PRIVATE HTAARealm *HTAARealm_new ARGS4(HTList *, realm_table,
                                       ...
                                       WWW_CONST char *, username, WWW_CONST char *, password)
{
    HTAARealm *realm;
...
PRIVATE char *compose_auth_string ARGS2(HTAAScheme, scheme, HTAASetup *, setup)
{
    ...
    if ((scheme != HTAA_BASIC && scheme != HTAA_PUBKEY && scheme != HTAA_MD5)
        || !setup ||
        !setup->scheme_specifics || !setup->scheme_specifics[scheme] || !setup->server || !setup->server->realms)
        return "";
    ...
    if (!realm || setup->retry) {
        ...
        if (!realm) {
            ...
            sprintf(msg,
                    ...
                    realm->realmname, setup->server->hostname ? setup->server->hostname : "??");
    ...
    len = strlen(realm->username ? realm->username : "") + strlen(realm->password ? realm->password : "") + 3;
    ...
    /* the following added by DXP */
        ...
        if (!(A1 = (unsigned char *)malloc(strlen(realm->username) +
                                           strlen(realm->realmname) + strlen(realm->password) + 3 + 1)))
            outofmem(__FILE__, "compose_auth_string");
...
*/
PUBLIC char *HTAA_composeAuth ARGS3(WWW_CONST char *, hostname, WWW_CONST int, portnumber, WWW_CONST char *, docname)
{
    ...
    if (current_portnumber != portnumber ||
        ...
        !hostname || !docname || 0 != strcmp(current_hostname, hostname) || 0 != strcmp(current_docname, docname)) {
        ...
        if (strchr(line, ':')) {    /* Valid header line */
            ...
            if (0 == my_strcasecmp(fieldname, "WWW-Authenticate:")) {
                if (HTAA_UNKNOWN != (scheme = HTAAScheme_enum(arg1))) {
                    ...
                    fprintf(stderr, "Unknown scheme `%s' %s\n", (arg1 ? arg1 : "(null)"), "in WWW-Authenticate: field");
                }
#endif
            }
