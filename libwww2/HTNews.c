    ...
    if (add->FirstRef) {
        for (temp = LastArt; temp; temp = temp->prevt)
            if (strmatch(add->FirstRef, temp->FirstRef)
                || strmatch(add->FirstRef, temp->ID))
                break;
    } else {
        for (temp = LastArt; temp; temp = temp->prevt)
            if (strmatch(add->ID, temp->FirstRef))
                break;
    }
...
#ifndef DISABLE_TRACE
    if (www2Trace)
        fprintf(stderr, "HTNews: initialising newsrc for host\n");
#endif
...
NewsArt *is_news_url(char *s)
{
    ...
    if ((strlen(s) > 5) && !strncmp("news:", s, 5)) {
        s = &s[5];
        /* check the obvious */
        if (CurrentArt && strmatch(s, CurrentArt->ID))
            return CurrentArt;
        for (art = FirstArt; art; art = art->nextt) {
            if (strmatch(s, art->ID))
                return art;
            if (art->next)
                for (art2 = art->next; art2; art2 = art2->next)
                    if (strmatch(s, art2->ID))
                        return art2;
        }
    }
    return NULL;
...
NewsArt *firstUnread(NewsArt *art)
{
    ...
    if (!newsUseNewsRC || newsShowAllArticles || !newsNextIsUnread || (!(tempNewsGroupS = findgroup(NewsGroup))))
        return art;
...
NewsArt *prevUnread(NewsArt *art, int probe)
{
    ...
    if (!newsUseNewsRC || newsShowAllArticles || !newsPrevIsUnread || (!(tempNewsGroupS = findgroup(NewsGroup))))
        return art->prev;
...
PRIVATE void read_article ARGS1(char *, artID)
{
    ...
    for (art = FirstArt; art; art = art->nextt) {
        if (!strncmp(art->ID, artID, ll))
            break;
        if (art->next) {
            for (art2 = art->next; art2; art2 = art2->next)
                if (!strncmp(art2->ID, artID, ll))
                    break;
            if (art2) {
                art = art2;
                break;
                    ...
                    if (*p == '>') {
                        *p = 0;
                        ...
                        if (strlen(pp) < LINE_LENGTH) {
                            if (i > 1)
                                PUTS(", ");
                            sprintf(line, "<A HREF=\"news:&lt;%s&gt;\">%d</A>", pp, i);
                            PUTS(line);
                            i++;
                        }
                        break;
            ...
            *p++ = 0;           /* Terminate the string */
            ...
            for (f = 0, l = pp = line; *pp; pp++) {
                ...
                if (*pp == ':') {
                    ...
                    if ((pp - m) > 2) {
                        if (match(m, "HTTP:") || match(m, "FTP:") || match(m, "MAILTO:") || match(m, "NEWS:")
                            || match(m, "GOPHER")) {
                            ll = *m;
                            *m = 0;
                            PUTS(l);
...
PRIVATE void read_list NOARGS {
    char line[LINE_LENGTH + 1], group[LINE_LENGTH], elgroup[LINE_LENGTH], postable, *p;
                ...
                if (line[0] == '.') {
                    ...
                    if (sscanf(line, "%s %d %d %c", group, &last, &first, &postable) == 4) {
                        ...
                        if (n) {
                            ...
                            if (n->attribs & naSUBSCRIBED && (newsShowAllGroups || n->unread > 0 || newsShowReadGroups)) {
                                sprintf(line,
                                        "%s % 7d S <A HREF=\"news:%s\">%s</A> \n",
                                        (lastg == 1) ? "<b>&gt;&gt;&gt;</b>" : "   ", n->unread, n->name, elgroup);
                                PUTS(line);
                                if (lastg == 1)
                                    lastg = 2;
                            }
        ...
        while (n) {
            ...
            if (newsShowAllGroups || n->unread > 0 || newsShowReadGroups) {
                sprintf(line,
                        "%s % 7d %s <A HREF=\"news:%s\">%s</A> \n",
                        (lastg == 1) ? "<b>&gt;&gt;&gt;</b>" : "   ",
                        n->unread, n->attribs & naSUBSCRIBED ? "S" : "U", n->name, elgroup);
                PUTS(line);
                l++;
                if (lastg == 1)
                    mark = 1;
               ...
               char **date, char **msgid, char **ref, char **bytes, char **lines)
{
    *num = x;
    ...
    while (*x && *x != '\t')
        x++;                    /* step to next tab */
    if (!*x)
        return 0;               /* early end of string - bad record */
    *x = 0;                     /* terminate */
...
PRIVATE void XBuildArtList ARGS3(WWW_CONST char *, groupName, int, first_required, int, last_required)
{
    NewsArt *art;
...
#ifndef DISABLE_TRACE
    if (www2Trace)
        fprintf(stderr, "Newsgroup status=%d, count=%d, (%d-%d)", status, count, first, last);
#endif
    Count = 0;

    if (NewsGroup && (strlen(NewsGroup) == strlen(groupName))
        && !strcmp(NewsGroup, groupName)) {
        last_required = last;
...
PRIVATE void BuildArtList ARGS3(WWW_CONST char *, groupName, int, first_required, int, last_required)
{
    NewsArt *art;
...
#ifndef DISABLE_TRACE
    if (www2Trace)
        fprintf(stderr, "[%s]", response_text);
#endif
    sscanf(response_text, "%d %d %d %d", &status, &count, &first, &last);
#ifndef DISABLE_TRACE
    if (www2Trace)
...
*/
    ...
    for (i = 0; i < c; i++) {
        ...
        if (status == 221) {    /* Head follows - parse it: */
            ...
            if (!ReadFirst || artno < ReadFirst)
                ReadFirst = artno;
            if (!ReadLast || artno > ReadLast)
                ReadLast = artno;
...
**
*/
PRIVATE void read_group ARGS3(WWW_CONST char *, groupName, int, first, int, last)
{
    NewsArt *art, *art2, *f;
    char *p;
    BOOL done;
    ...
    for (art = f; art; art = nextUnreadThread(art)) {
        ...
        if (!ConfigView) {      /* Thread view */
            ...
            sprintf(buffer, "%s % 4d <A HREF=\"news:%s\">%s</A>%s %s\n",
                    (mark == 1) ? "<b>&gt;&gt;&gt;</b>" : "   ", i,
                    art->ID, esubj, makespaces(subj, newsSubjWidth), efrom);
            PUTS(buffer);
            if (mark == 1)
                mark = 2;
        } else {
            ...
            for (art2 = art; art2; art2 = nextUnread(art2, 0)) {
                ...
                escapeString(from, efrom);
                sprintf(buffer,
                        "%s      <A HREF=\"news:%s\">%s</A>%s %s\n",
                        (mark == 1) ? "<b>&gt;&gt;&gt;</b>" : "   ",
                        art2->ID, esubj, makespaces(subj, newsSubjWidth), efrom);
                PUTS(buffer);
                if (mark == 1)
                    mark = 2;
            }
...
**		============
*/
PUBLIC int HTLoadNews ARGS4(WWW_CONST char *, arg, HTParentAnchor *, anAnchor, HTFormat, format_out, HTStream *, stream)
{
            ...
            if (status = OpenNNTP()) {
                ...
                case 2:
                    ...
                    sprintf(message,
                            ...
                            "to the name of your news host, and restart Mosaic.", HTNewsHost);
