                ...
                *p++ = 0;       /* Terminate the string */
                ...
                if (continuation == '-' && !multiline_response) {
                    multiline_response = result;
                           ...
                           multiline_response == result && isdigit(response_text[0])) {
            ...
            if (foo == LF || p == &response_text[LINE_LENGTH]) {
                ...
                *p++ = 0;       /* Terminate the string */
                ...
                /* Try just continuing if we couldn't pull out
                   ...
                   whitespace. */
                if (rv < 2 && result == -1 && (*response_text == ' ' || *response_text == '\t')) {
...
*/
            ...
            if (strcmp(un, username)) { /*new username */
                ...
                if (!*(ftpcache.host)
                    || (*(ftpcache.host) && strcmp(host, ftpcache.host))) { /*new host */
                    *password = '\0';
                }
            }
        }
        ...
        if (ftpcache.control != (-1) && *(ftpcache.host)
            ...
            if (*username && !strcmp(ftpcache.username, username) && *password && !strcmp(ftpcache.password, password)) {
                /* For security Icon */
    ...
    /* Now we log in; Look up username, prompt for pw. */
    {
        ...
        if (status == 3) {      /* Send password */
            ...
            if (*password) {
                ...
                sprintf(command, "PASS %s@%s%c%c", user ? user : "WWWuser", host, CR, LF);  /*@@ */
...
#endif
{
    ...
    sprintf(port_command, "PORT %d,%d,%d,%d,%d,%d%c%c",
            ...
            (int)*((unsigned char *)(&sin->sin_port) + 0), (int)*((unsigned char *)(&sin->sin_port) + 1), CR, LF);
...
ARGS4(HTParentAnchor *, parent, WWW_CONST char *, address, HTFormat, format_out, HTStream *, sink)
{
    HTFormat format;
    HTAtom *pencoding;
...
#endif
                    if (compact_string
                        (itemname, ellipsis_string, ftpFilenameLength, ftpEllipsisMode, ftpEllipsisLength)) {
                        strcpy(itemname, ellipsis_string);
                    }
                    sprintf(buffer, "%s", itemname);
#if 0
...
**			<0 if bad.
*/
PUBLIC int HTFTPLoad ARGS4(char *, name, HTParentAnchor *, anchor, HTFormat, format_out, HTStream *, sink)
{
    BOOL isDirectory = NO;
    int status;
    ...
    if (isDirectory) {
        ...
        if (!stream) {
            char buffer[1024];  /* @@@@@@@@ */
            sprintf(buffer, "Sorry, can't convert from %s to %s.", HTAtom_name(format), HTAtom_name(format_out));
            HTProgress(buffer);
#ifndef DISABLE_TRACE
            if (www2Trace)
