...
PRIVATE int parse_menu ARGS2(WWW_CONST char *, arg, HTParentAnchor *, anAnchor)
{
    char gtype;
    char ch;
    char line[BIG];
...
PRIVATE void display_index ARGS2(WWW_CONST char *, arg, HTParentAnchor *, anAnchor)
{
...
PRIVATE void display_cso ARGS2(WWW_CONST char *, arg, HTParentAnchor *, anAnchor)
{
 ...
 **
 */
PRIVATE int parse_cso ARGS2(WWW_CONST char *, arg, HTParentAnchor *, anAnchor)
{
    char ch;
    char line[BIG];
    char *p = line;
...
**
*/
PUBLIC int HTLoadGopher ARGS4(char *, arg, HTParentAnchor *, anAnchor, HTFormat, format_out, HTStream *, sink)
{
...
#ifndef DISABLE_TRACE
        if (www2Trace)
            fprintf(stderr, "Gopher: Interrupted on connect; recovering cleanly.\n");
#endif
...
#ifndef DISABLE_TRACE
        if (www2Trace)
            fprintf(stderr, "HTTPAccess: Unable to connect to remote host for `%s'.\n", arg);
#endif
        free(command);
...
#ifndef DISABLE_TRACE
    if (www2Trace)
        fprintf(stderr, "HTGopher: Connected, writing command `%s' to socket %d\n", command, s);
#endif
    ...
    case GOPHER_BINARY:
        if (!tweak_gopher_types)
            ...
            rv = HTParseSocket(HTFileFormat(arg, &enc, WWW_BINARY, &compressed), format_out, anAnchor, s, sink, 0);
        break;
    ...
    case GOPHER_PLUS_IMAGE:
        if (!tweak_gopher_types)
            rv = HTParseSocket(HTAtom_for("image/gif"), format_out, anAnchor, s, sink, 0);
        else
            rv = HTParseSocket(HTFileFormat
                               (arg, &enc, HTAtom_for("image/gif"), &compressed), format_out, anAnchor, s, sink, 0);
        break;

    case GOPHER_SOUND:
    case GOPHER_PLUS_SOUND:
        if (!tweak_gopher_types)
            rv = HTParseSocket(HTAtom_for("audio/basic"), format_out, anAnchor, s, sink, 0);
        else
            rv = HTParseSocket(HTFileFormat(arg, &enc,
                                            HTAtom_for("audio/basic"), &compressed), format_out, anAnchor, s, sink, 0);
        break;

    case GOPHER_PLUS_MOVIE:
        /* Sigh..... */
        if (!tweak_gopher_types)
            rv = HTParseSocket(HTAtom_for("video/mpeg"), format_out, anAnchor, s, sink, 0);
        else
            rv = HTParseSocket(HTFileFormat(arg, &enc,
                                            HTAtom_for("video/mpeg"), &compressed), format_out, anAnchor, s, sink, 0);
        break;
    ...
    case GOPHER_HTML:
        if (!tweak_gopher_types)
            ...
            rv = HTParseSocket(HTFileFormat(arg, &enc, WWW_HTML, &compressed), format_out, anAnchor, s, sink, 0);
        break;
    ...
    default:                   /* @@ parse as plain text */
        if (!tweak_gopher_types)
            ...
            rv = HTParseSocket(HTFileFormat(arg, &enc, WWW_PLAINTEXT, &compressed), format_out, anAnchor, s, sink, 0);
        break;
    }                           /* switch(gtype) */
