...
PUBLIC char *HTAA_statusMessage NOARGS {
    ...
    case HTAA_NO_AUTH:
        ...
        break;
        case HTAA_NOT_MEMBER:return "Unauthorized to access the document";
        break;

        /* 403 cases */
        case HTAA_BY_RULE:return "Forbidden -- by rule";
        break;
        case HTAA_IP_MASK:return "Forbidden -- server refuses to serve to your IP address";
        break;
        case HTAA_NO_ACL:case HTAA_NO_ENTRY:return "Forbidden -- access to file is never allowed";
        break;
        case HTAA_SETUP_ERROR:return "Forbidden -- server protection setup error";
        break;

        /* 404 cases */
        case HTAA_NOT_FOUND:return "Not found -- file doesn't exist or is read protected";
        break;

        /* Success */
        case HTAA_OK:return "AA: Access should be ok but something went wrong";
        break;
...
**	file twice).
*/
PRIVATE HTAAFailReasonType check_authorization ARGS4(WWW_CONST char *,
                                                     pathname, HTAAMethod,
                                                     method, HTAAScheme, scheme, char *, scheme_specifics)
{
    ...
    if (www2Trace)
        fprintf(stderr, "%s `%s' %s %s\n",
                "HTAA_checkAuthorization: translated path:", pathname, "method:", HTAAMethod_name(method));
#endif
    /*
    ...
    if (!(acl_file = HTAA_openAcl(pathname))) {
        if (prot) {             /* protect rule, but no ACL */
            if (prot->mask_group) {
                 ...
                 */
                GroupDefList *group_def_list = HTAA_readGroupFile(HTAssocList_lookup(prot->values,
                                                                                     "group"));
                /*
                 ** Authenticate if authentication info given
                 */
                if (scheme != HTAA_UNKNOWN && scheme != HTAA_NONE) {
                    htaa_user = HTAA_authenticate(scheme, scheme_specifics, prot);
#ifndef DISABLE_TRACE
                    if (www2Trace)
                        fprintf(stderr,
                                "Authentication returned: %s\n",
                                (htaa_user ? htaa_user->username : "NOT-AUTHENTICATED"));
#endif
                }
                HTAA_resolveGroupReferences(prot->mask_group, group_def_list);
                reason =
                    HTAA_userAndInetInGroup(prot->mask_group, htaa_user ? htaa_user->username : "", HTClientHost, NULL);
#ifndef DISABLE_TRACE
                if (www2Trace) {
                    if (reason != HTAA_OK)
                        fprintf(stderr, "%s %s %s %s\n",
                                "HTAA_checkAuthorization: access denied",
                                "by mask (no ACL, only Protect rule)", "host", HTClientHost);
                    else
                        fprintf(stderr, "%s %s %s %s\n",
                                "HTAA_checkAuthorization: request from",
                                HTClientHost,
                                "accepted by only mask match (no ACL, only", "Protect rule, and only mask enabled)");
                }
#endif
                return reason;
            } else {            /* 403 Forbidden */
                ...
                if (www2Trace)
                    fprintf(stderr, "%s %s\n",
                            "HTAA_checkAuthorization: Protected, but", "no mask group nor ACL -- forbidden");
#endif
                return HTAA_NO_ACL;
            }
    ...
    if (!prot) {                /* Not protected by "protect" rule */
        ...
        if (!prot) {            /* @@ Default protection not set ?? */
            ...
            if (www2Trace)
                fprintf(stderr, "%s %s\n",
                        "HTAA_checkAuthorization: default protection", "not set (internal server error)!!");
#endif
     ...
     */
    {
        GroupDefList *group_def_list = HTAA_readGroupFile(HTAssocList_lookup(prot->values, "group"));
        ...
        if (prot->mask_group) {
            ...
            reason = HTAA_userAndInetInGroup(prot->mask_group,
                                             htaa_user ? htaa_user->username : "", HTClientHost, NULL);
            if (reason != HTAA_OK) {
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "%s %s %s\n",
                            "HTAA_checkAuthorization: access denied", "by mask, host:", HTClientHost);
#endif
                return reason;
            }
#ifndef DISABLE_TRACE
                ...
                fprintf(stderr, "%s %s %s %s %s\n",
                        "HTAA_checkAuthorization: request from",
                        HTClientHost,
                        "accepted by just mask group match", "(no ACL, only Protect rule, and only", "mask enabled)");
#endif
        ...
        if (!allowed_groups) {
            ...
            do {
                ...
                reason =
                    HTAA_userAndInetInGroup(allowed_groups, htaa_user ? htaa_user->username : "", HTClientHost, NULL);
                if (reason == HTAA_OK) {
                    HTAA_closeAcl(acl_file);
                    return HTAA_OK; /* OK */
                }
...
PUBLIC int HTAA_checkAuthorization ARGS4(WWW_CONST char *, url,
                                         ...
                                         WWW_CONST char *, scheme_name, char *, scheme_specifics)
{
    ...
    HTAAFailReason = check_authorization(pathname, method, scheme, scheme_specifics);
    ...
    if (htaa_logfile) {
        ...
        fprintf(htaa_logfile, "%24.24s %s %s %s %s %s\n",
                ...
                method_name,
                url, status_name(HTAAFailReason), htaa_user && htaa_user->username ? htaa_user->username : "");
...
**			auto-freed string.
*/
PRIVATE char *compose_scheme_specifics ARGS2(HTAAScheme, scheme, HTAAProt *, prot)
{
    ...
    case HTAA_PUBKEY:
        {
            ...
            sprintf(result, "realm=\"%s\", key=\"%s\"", (realm ? realm : "UNKNOWN"), "PUBKEY-NOT-IMPLEMENTED");
            return result;
        }
        break;
    default:
...
PUBLIC char *HTAA_composeAuthHeaders NOARGS {
    ...
    if (!prot) {
        ...
        if (www2Trace)
            fprintf(stderr, "%s %s\n",
                    "HTAA_composeAuthHeaders: Document not protected", "-- why was this function called??");
#endif
        return NULL;
    }
#ifndef DISABLE_TRACE
