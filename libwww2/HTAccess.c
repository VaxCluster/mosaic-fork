...
**			HT_OK			Success
**
*/ PRIVATE int get_physical ARGS3(char *, addr,
                                     HTParentAnchor *, anchor, int, bong)
{
...

    {
        ...
        if (!GetNoProxy(tmp_access, tmp_host)) {
            ...
            if (proxy) {
                ...
                if (proxyentry)
                    free(proxyentry);
                access = HTParse(HTAnchor_physical(anchor), "http:", PARSE_ACCESS);
            } else if (gateway) {
                char *gatewayed;
...
**
*/
PRIVATE int HTLoad ARGS4(WWW_CONST char *, addr, HTParentAnchor *, anchor, HTFormat, format_out, HTStream *, sink)
{
    HTProtocol *p;
    ...
    while (1) {
      ...
      retry_proxy:
        ...
        finbuf = (char *)calloc((strlen(host) + strlen(buf1) + strlen(buf2) + 5), sizeof(char));
        sprintf(finbuf, "%s%s?%s", buf1, host, buf2);
        if (HTConfirm(finbuf)) {
            free(finbuf);
            finbuf = (char *)calloc((strlen(host) +
                                     strlen("Disabling proxy server ") +
                                     strlen(" and trying again.") + 5), sizeof(char));
...
PRIVATE int HTLoadDocument ARGS4(WWW_CONST char *, full_address,
                                 HTParentAnchor *, anchor, HTFormat, format_out, HTStream *, sink)
{
    int status;
...
#ifndef DISABLE_TRACE
    if (www2Trace)
        fprintf(stderr, "**** HTAccess: socket or file number %d returned by obsolete load routine!\n", status);
#endif
    return 0;
...
PUBLIC BOOL HTLoadToStream ARGS3(WWW_CONST char *, addr, BOOL, filter, HTStream *, sink)
{
    return HTLoadDocument(addr,
                          HTAnchor_parent(HTAnchor_findAddress(addr)),
...
PUBLIC BOOL HTLoadRelative ARGS2(WWW_CONST char *, relative_name, HTParentAnchor *, here)
{
    char *full_address = 0;
    BOOL result;
    char *mycopy = 0;
    ...
    full_address = HTParse(stripped, current_address, PARSE_ACCESS | PARSE_HOST | PARSE_PATH | PARSE_PUNCTUATION);
