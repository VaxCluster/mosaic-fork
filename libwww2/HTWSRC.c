...
PRIVATE void WSRCParser_put_character ARGS2(HTStream *, me, char, c)
{
...
#ifndef DISABLE_TRACE
                if (www2Trace)
                    fprintf(stderr, "HTWSRC: Unknown field `%s' in source file\n", me->param);
#endif
    ...
    case before_value:
        ...
        me->state = (c == '"') ? quoted_value : (c == '(') ? bracketed_value : value;
    ...
    if (source_file) {
        ...
        if (me->par_value[PAR_IP_NAME] && me->par_value[PAR_DATABASE_NAME]) {
            ...
            sprintf(WSRC_address, "wais://%s:%s/%s",
                    ...
                    me->par_value[PAR_TCP_PORT] ? me->par_value[PAR_TCP_PORT] : "210", www_database);
            ...
            sprintf(WSRC_address,
                    "http://www.ncsa.uiuc.edu:8001/%s:%s/%s",
                    me->par_value[PAR_IP_NAME],
                    me->par_value[PAR_TCP_PORT] ? me->par_value[PAR_TCP_PORT] : "210", www_database);
            PUTS("<A HREF=\"");
            PUTS(WSRC_address);
...
PRIVATE void WSRCParser_write ARGS3(HTStream *, context, WWW_CONST char *, str, int, l)
{
    WWW_CONST char *p;
...
**		--------------------------------------
*/
PUBLIC HTStream *HTWSRCConvert ARGS5(HTPresentation *, pres,
