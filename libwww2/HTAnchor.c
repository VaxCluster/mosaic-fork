...
*/ PRIVATE BOOL equivalent ARGS2(WWW_CONST char *, s, WWW_CONST char *, t)
{
...
PUBLIC HTChildAnchor *HTAnchor_findChild ARGS2(HTParentAnchor *, parent, WWW_CONST char *, tag)
{
    HTChildAnchor *child;
    HTList *kids;
...
HTAnchor *HTAnchor_findAddress ARGS1(WWW_CONST char *, address)
{
    ...
    if (tag && *tag) {
        char *docAddress = HTParse(address, "", PARSE_ACCESS | PARSE_HOST | PARSE_PATH | PARSE_PUNCTUATION);
        HTParentAnchor *foundParent = (HTParentAnchor *) HTAnchor_findAddress(docAddress);
...
char *HTAnchor_address ARGS1(HTAnchor *, me)
{
    char *addr = NULL;
    if (me) {
        if (((HTParentAnchor *) me == me->parent) || !((HTChildAnchor *) me)->tag) {    /* it's an adult or no tag */
...
BOOL HTAnchor_link ARGS3(HTAnchor *, source, HTAnchor *, destination, HTLinkType *, type)
{
    if (!(source && destination))
        return NO;              /* Can't link to/from non-existing anchor */
#ifndef DISABLE_TRACE
    if (www2Trace)
        printf("Linking anchor %p to anchor %p\n", source, destination);
#endif
