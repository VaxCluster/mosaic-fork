    ...
    if (me->end_command) {
        ...
        if (strstr(me->end_command, "mosaic-internal-dump")) {
            ...
            if (imageViewInternal && supportedImageType(me->mime_type)) {
                ...
                buf = (char *)
                    calloc((strlen(currentURL) + strlen(newHTML) + 5), sizeof(char));
                ...
                buf = (char *)
                    calloc((strlen(currentURL) + strlen(me->fnam) + 5), sizeof(char));
             ...
             */
            if (envbuf1) {
                envbuf2 = (char *)calloc((strlen(currentURL) + strlen("MOSAIC_CURRENT_URL=")
                                          + 2), sizeof(char));
                ...
                envbuf1 = (char *)calloc((strlen(currentURL) + strlen("MOSAIC_CURRENT_URL=")
                                          + 2), sizeof(char));
            ...
            if (envbuf1) {
                envbuf2 = (char *)calloc((strlen("MOSAIC_CURRENT_URL=") + 2), sizeof(char));
                sprintf(envbuf2, "MOSAIC_CURRENT_URL=");
                putenv(envbuf2);
                free(envbuf1);
                envbuf1 = NULL;
            } else if (envbuf2) {
                envbuf1 = (char *)calloc((strlen("MOSAIC_CURRENT_URL=") + 2), sizeof(char));
...
*/
PUBLIC HTStream *HTSaveAndExecute ARGS5(HTPresentation *, pres, HTParentAnchor *, anchor,   /* Not used */
...
#endif
            if (strstr(pres->command, "%s") || strstr(pres->command, "mosaic-internal")) {
                /* Make command to process file */
                command = (char *)malloc((strlen(pres->command) + 10 + 3 * strlen(me->fnam)) * sizeof(char));
                ...
                me->end_command = (char *)malloc((strlen(command) + 32 + strlen(me->fnam)) * sizeof(char));
                sprintf(me->end_command, "(%s ; /bin/rm -f %s) &", command, me->fnam);

                free(command);
            } else {
                /* Make command to process file -- but we have to cat
                   to the viewer's stdin. */
                me->end_command = (char *)malloc((strlen(pres->command) + 64 + (2 * strlen(me->fnam))) * sizeof(char));
                sprintf(me->end_command, "((cat %s | %s); /bin/rm -f %s) &", me->fnam, pres->command, me->fnam);
            }
        } else {
            /* Overload me->end_command to be what we should write out as text
               to communicate back to client code. */
            me->end_command = (char *)malloc(strlen("mosaic-internal-reference") + strlen(me->fnam) + 32);
