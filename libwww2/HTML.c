...
**	-------------
*/
PRIVATE void HTML_start_element ARGS4(HTStructured *, me,
                                      int, element_number, WWW_CONST BOOL *, present, WWW_CONST char **, value)
{
    if (!me->text) {
        me->text = HText_new();
        HText_beginAppend(me->text);
...
*/
PUBLIC HTStructured *HTML_new ARGS3(HTParentAnchor *, anchor, HTFormat, format_out, HTStream *, stream)
{
...
PUBLIC int HTLoadError ARGS3(HTStream *, sink, int, number, WWW_CONST char *, message)
{
    HTAlert(message);           /* @@@@@@@@@@@@@@@@@@@ */
#if 0
    return -number;
