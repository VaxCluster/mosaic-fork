...
void XmxAddEventHandler(Widget w, EventMask event_mask, XtEventHandler proc, int client_data)
{
    XtAddEventHandler(w, event_mask, False, proc, (XtPointer) _XmxMakeClientData(client_data));

    return;
}

void XmxRemoveEventHandler(Widget w, EventMask event_mask, XtEventHandler proc, int client_data)
{
    XtRemoveEventHandler(w, event_mask, False, proc, (XtPointer) _XmxMakeClientData(client_data));
...
static void XmxCenteringResize(CompositeWidget w, XConfigureEvent *event, String args[], int *num_args)
{
    WidgetList children;
    ...
    XtVaGetValues(children[0], XmNheight, &w_height, XmNwidth, &w_width, NULL);
    XtVaSetValues(children[0], XmNx, width / 2 - w_width / 2, XmNy, height / 2 - w_height / 2, NULL);
...
/* args work */
Widget XmxMakePushButton(Widget parent, String name, XtCallbackProc cb, int cb_data)
{
    XmString xmstr = NULL;
    ...
    Xmx_w = XtCreateManagedWidget("pushbutton", xmPushButtonWidgetClass, parent, Xmx_wargs, Xmx_n);
    XtAddCallback(Xmx_w, XmNactivateCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
...
/* args work */
Widget XmxMakeNamedPushButton(Widget parent, String name, String wname, XtCallbackProc cb, int cb_data)
{
    XmString label;
    ...
    Xmx_w = XtCreateManagedWidget(wname, xmPushButtonWidgetClass, parent, Xmx_wargs, Xmx_n);
    XtAddCallback(Xmx_w, XmNactivateCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
    if (name)
        XmStringFree(label);
...
{
    Xmx_w = XtCreateManagedWidget("blankbutton", xmPushButtonWidgetClass, parent, Xmx_wargs, Xmx_n);
    XtAddCallback(Xmx_w, XmNactivateCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
...
/* args work */
Widget XmxMakeCommand(Widget parent, String prompt, XtCallbackProc cb, int cb_data)
{
    XmString xmstr = XmxMakeXmstrFromString(prompt);

    XmxSetArg(XmNpromptString, (XtArgVal) xmstr);
    Xmx_w = XtCreateManagedWidget("command", xmCommandWidgetClass, parent, Xmx_wargs, Xmx_n);
    XtAddCallback(Xmx_w, XmNcommandEnteredCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
...
Widget XmxMakeScrolledList(Widget parent, XtCallbackProc cb, int cb_data)
{
    ...
    /* defaultAction gets triggered on double click and sends item
       along with it... */
    XtAddCallback(Xmx_w, XmNdefaultActionCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
...
Widget XmxMakeDrawingArea(Widget parent, int width, int height)
{
    ...
    Xmx_w = XtCreateManagedWidget("drawingarea", xmDrawingAreaWidgetClass, parent, Xmx_wargs, Xmx_n);
...
Widget XmxMakeOptionBox(Widget parent)
{
    ...
    Xmx_w = XtCreateManagedWidget("optionbox", xmRowColumnWidgetClass, parent, Xmx_wargs, Xmx_n);
    Xmx_n = 0;
    return Xmx_w;
}

/* args work */
Widget XmxMakeToggleButton(Widget parent, String name, XtCallbackProc cb, int cb_data)
{
    XmString label = XmStringCreateLtoR(name, XmSTRING_DEFAULT_CHARSET);
    XmxSetArg(XmNlabelString, (XtArgVal) label);
    XmxSetArg(XmNmarginHeight, (XtArgVal) 0);
    Xmx_w = XtCreateManagedWidget("togglebutton", xmToggleButtonWidgetClass, parent, Xmx_wargs, Xmx_n);
    /* Used to be XmNarmCallback --- probably not right. */
    if (cb)
        XtAddCallback(Xmx_w, XmNvalueChangedCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
                    ...
                    String label, int min, int max, int start, int dec_adj)
{
    ...
    Xmx_w = XtCreateManagedWidget("scale", xmScaleWidgetClass, parent, Xmx_wargs, Xmx_n);

    XtAddCallback(Xmx_w, XmNvalueChangedCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
    XtAddCallback(Xmx_w, XmNdragCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
...
{
    assert(shadow == XmxShadowIn || shadow == XmxShadowOut
           || shadow == XmxShadowEtchedIn || shadow == XmxShadowEtchedOut);
    switch (shadow) {
    case XmxShadowIn:
    ...
    Xmx_w = XtCreateManagedWidget("frame", xmFrameWidgetClass, parent, Xmx_wargs, Xmx_n);
...
{
    Xmx_w = XtCreateManagedWidget("form", xmFormWidgetClass, parent, Xmx_wargs, Xmx_n);
    Xmx_n = 0;
    return Xmx_w;
}
...
void XmxSetConstraints(Widget w,
                       int top, int bottom, int left, int right, Widget topw, Widget botw, Widget lefw, Widget rigw)
{
...
{
    Xmx_w = XtCreateManagedWidget("rowcolumn", xmRowColumnWidgetClass, parent, Xmx_wargs, Xmx_n);
    Xmx_n = 0;
    return Xmx_w;
}
...
{
    Xmx_w = XtCreateManagedWidget("bboard", xmBulletinBoardWidgetClass, parent, Xmx_wargs, Xmx_n);
    Xmx_n = 0;
    return Xmx_w;
}
...
Widget XmxMakeCenteringBboard(Widget parent, XtAppContext app)
{
    ...
    /* This does, for sure... */
    XtOverrideTranslations(Xmx_w, XtParseTranslationTable("<Configure>: resize()"));
...
Widget XmxMakeLabel(Widget parent, String name)
{
    ...
    XmxSetArg(XmNlabelString, (XtArgVal) xmstr);
    Xmx_w = XtCreateManagedWidget("label", xmLabelWidgetClass, parent, Xmx_wargs, Xmx_n);
    XmStringFree(xmstr);
    Xmx_n = 0;
    return Xmx_w;
}
...
Widget XmxMakeNamedLabel(Widget parent, String name, String wname)
{
    XmString xmstr = XmStringCreateLtoR(name, XmSTRING_DEFAULT_CHARSET);
    XmxSetArg(XmNlabelString, (XtArgVal) xmstr);
    Xmx_w = XtCreateManagedWidget(wname, xmLabelWidgetClass, parent, Xmx_wargs, Xmx_n);
    XmStringFree(xmstr);
    Xmx_n = 0;
    return Xmx_w;
}
                             ...
                             XtCallbackProc cb, int yes_token, int no_token)
{
    ...
    XtAddCallback(Xmx_w, XmNcancelCallback, cb, (XtPointer) _XmxMakeClientData(no_token));
    XtAddCallback(Xmx_w, XmNokCallback, cb, (XtPointer) _XmxMakeClientData(yes_token));
...
XmString XmxMakeXmstrFromFile(String filename)
{
    ...
    while (!feof(_f)) {
        ...
        _xmstr = XmStringConcat(_xmstr, XmStringCreateLtoR(_mstr, XmSTRING_DEFAULT_CHARSET));
    }
...
/* args work */
Widget XmxMakeFileSBDialog(Widget parent, String title, String selection_txt, XtCallbackProc cb, int cb_data)
{
    ...
    XtAddCallback(Xmx_w, XmNokCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
    XtAddCallback(Xmx_w, XmNcancelCallback, (XtCallbackProc) _XmxCancelCallback, 0);

    /* Set selection label to specified selection_txt. */
    Xmx_n = 0;
    _selection_label = XmFileSelectionBoxGetChild(Xmx_w, XmDIALOG_SELECTION_LABEL);
    XmxSetArg(XmNlabelString, (XtArgVal) label);
...
/* args work */
Widget XmxMakeHelpTextDialog(Widget parent, String str, String title, Widget *text_w)
{
    ...
    XmxSetConstraints(_outer_frame, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
    ...
    /* Make an OK button. */
    _buttons_form = XmxMakeFormAndOneButton(_form, _XmxHelpTextCancelCallback, "OK", 0);

    /* Constraints for _form. */
    XmxSetConstraints
        (XtParent(_scr_text), XmATTACH_FORM, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, NULL, _sep, NULL, NULL);
    XmxSetConstraints
        (_sep, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, NULL, _buttons_form, NULL, NULL);
    XmxSetConstraints
        (_buttons_form, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
...
Widget XmxMakeHorizontalSpacer(Widget parent)
{
    ...
    XmxSetArg(XmNlabelString, (XtArgVal) label);
    Xmx_w = XtCreateManagedWidget("label", xmLabelGadgetClass, parent, Xmx_wargs, Xmx_n);
...
{
    XtAddCallback(text, XmNactivateCallback, cb, (XtPointer) _XmxMakeClientData(cb_data));
    return;
}
...
Widget
    ...
    (Widget parent, int width, int height,
     GLXconfig * glxConfig, XtCallbackProc redraw_cb, XtCallbackProc resize_cb, XtCallbackProc ginit_cb) {
    XmxSetArg(XmNwidth, (XtArgVal) width);
    XmxSetArg(XmNheight, (XtArgVal) height);
    ...
    XtAddCallback(Xmx_w, GlxNexposeCallback, redraw_cb, (XtPointer) _XmxMakeClientData(0));
    XtAddCallback(Xmx_w, GlxNresizeCallback, resize_cb, (XtPointer) _XmxMakeClientData(0));
    XtAddCallback(Xmx_w, GlxNginitCallback, ginit_cb, (XtPointer) _XmxMakeClientData(0));
...
void XmxInstallColormapsWithOverlay(Widget toplevel, Widget glw)
{
    ...
    i = 0;
    XtSetArg(args[i], GlxNoverlayWindow, &overlay);
    i++;
    XtSetArg(args[i], GlxNpopupWindow, &popup);
    i++;
    XtSetArg(args[i], GlxNunderlayWindow, &underlay);
    i++;
...
Widget
    XmxMakeDrawingVolume
    (Widget parent, int width, int height,
     XtCallbackProc redraw_cb, XtCallbackProc resize_cb, XtCallbackProc ginit_cb) {
    XmxSetArg(XmNwidth, (XtArgVal) width);
    XmxSetArg(XmNheight, (XtArgVal) height);

    Xmx_w = XtCreateWidget("drawingvolume", glibWidgetClass, parent, Xmx_wargs, Xmx_n);
    XtManageChild(Xmx_w);

    XtAddCallback(Xmx_w, XglNexposeCallback, redraw_cb, (XtPointer) _XmxMakeClientData(0));
    XtAddCallback(Xmx_w, XglNresizeCallback, resize_cb, (XtPointer) _XmxMakeClientData(0));
    XtAddCallback(Xmx_w, XglNgconfigCallback, ginit_cb, (XtPointer) _XmxMakeClientData(0));
...
void XmxApplyBitmapToLabelWidget(Widget label, String data, unsigned int width, unsigned int height)
{
    Display *_disp;
    Pixel _fg, _bg;
    Pixmap _pix;
...
/* args ignored and reset */
Pixmap XmxCreatePixmapFromBitmap(Widget label, String data, unsigned int width, unsigned int height)
{
    Display *_disp;
    Pixel _fg, _bg;
    Pixmap _pix;
...
/* args apply to form */
Widget XmxMakeFormAndOneButton(Widget parent, XtCallbackProc cb, String name1, int cb_data1)
{
    ...
    XmxSetConstraints(_button1, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
...
Widget XmxMakeFormAndTwoButtons
    (Widget parent, XtCallbackProc cb, String name1, String name2, int cb_data1, int cb_data2) {
    ...
    XmxSetConstraints(_button1, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button2, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
...
Widget XmxMakeFormAndThreeButtonsSqueezed
    (Widget parent, XtCallbackProc cb,
     String name1, String name2, String name3, int cb_data1, int cb_data2, int cb_data3) {
    ...
    XmxSetConstraints(_button1, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button2, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button3, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetPositions(_button1, XmxNoPosition, XmxNoPosition, 1, 2);
    XmxSetPositions(_button2, XmxNoPosition, XmxNoPosition, 3, 4);
...
Widget XmxMakeFormAndFourButtons
    (Widget parent, XtCallbackProc cb,
     String name1, String name2, String name3, String name4, int cb_data1, int cb_data2, int cb_data3, int cb_data4) {
    ...
    XmxSetConstraints(_button2, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button3, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button4, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetPositions(_button1, XmxNoPosition, XmxNoPosition, 0, 1);
    XmxSetPositions(_button2, XmxNoPosition, XmxNoPosition, 1, 2);
    XmxSetPositions(_button3, XmxNoPosition, XmxNoPosition, 2, 3);
...
Widget XmxMakeFormAndFiveButtons
    ...
    XmxSetConstraints(_button3, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button4, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetConstraints(_button5, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    XmxSetPositions(_button1, XmxNoPosition, XmxNoPosition, 0, 1);
    XmxSetPositions(_button2, XmxNoPosition, XmxNoPosition, 1, 2);
    XmxSetPositions(_button3, XmxNoPosition, XmxNoPosition, 2, 3);
    XmxSetPositions(_button4, XmxNoPosition, XmxNoPosition, 3, 4);
...
int XmxModalYesOrNo(Widget parent, XtAppContext app, char *questionstr, char *yesstr, char *nostr)
{
    Widget dialog;
...
/*  startbouncetimer();*/
    ...
    XtAddCallback(dialog, XmNokCallback, (XtCallbackProc) _XmxYesOrNoResponse, &answer);
    XtAddCallback(dialog, XmNcancelCallback, (XtCallbackProc) _XmxYesOrNoResponse, &answer);
...
/*SWP -- 7/6/95*/
static void _XmxActivate(Widget w, char **answer, XmSelectionBoxCallbackStruct *cbs)
{
    *answer = XMX_NO_ANSWER;
}

static void _XmxPromptForStringResponse(Widget w, char **answer, XmSelectionBoxCallbackStruct *cbs)
{
    if (!XmStringGetLtoR(cbs->value, XmSTRING_DEFAULT_CHARSET, answer))
        *answer = XMX_NO_ANSWER;
}

static void _XmxPromptForStringCancel(Widget w, char **answer, XmSelectionBoxCallbackStruct *cbs)
{
    *answer = XMX_NO_ANSWER;
}

/*SWP -- 7/4/95*/
void XmxMakeInfoDialogWait(Widget parent, XtAppContext app, char *infostr, char *titlestr, char *yesstr)
{
    Widget dialog;
...
/*SWP -- 4/15/96*/
void XmxMakeErrorDialogWait(Widget parent, XtAppContext app, char *infostr, char *titlestr, char *yesstr)
{
    Widget dialog;
    XmString info, yes, title;
    char *answer = NULL;
...
char *XmxModalPromptForString(Widget parent, XtAppContext app, char *questionstr, char *yesstr, char *nostr)
{
    Widget dialog;
    XmString question, yes, no, title;
    char *answer = NULL;
...
/*  startbouncetimer();*/
    ...
    XtAddCallback(dialog, XmNokCallback, (XtCallbackProc) _XmxPromptForStringResponse, &answer);
    XtAddCallback(dialog, XmNcancelCallback, (XtCallbackProc) _XmxPromptForStringCancel, &answer);
...
static void _XmxPromptForPasswordResponse(Widget w, char **answer, XmSelectionBoxCallbackStruct *cbs)
{
    if (!XmStringGetLtoR(cbs->value, XmSTRING_DEFAULT_CHARSET, answer))
        *answer = XMX_NO_ANSWER;
}

static void _XmxPromptForPasswordCancel(Widget w, char **answer, XmSelectionBoxCallbackStruct *cbs)
{
    *answer = XMX_NO_ANSWER;
}

static void _XmxPromptForPasswordVerify(Widget text_w, XtPointer unused, XmTextVerifyCallbackStruct *cbs)
{
    char *new;
    int len;
...
char *XmxModalPromptForPassword(Widget parent, XtAppContext app, char *questionstr, char *yesstr, char *nostr)
{
    Widget dialog;
    XmString question, yes, no, title;
    char *answer = NULL;
...
/*  startbouncetimer();*/
    ...
    XtAddCallback(dialog, XmNokCallback, (XtCallbackProc) _XmxPromptForPasswordResponse, &answer);
    XtAddCallback(dialog, XmNcancelCallback, (XtCallbackProc) _XmxPromptForPasswordCancel, &answer);
