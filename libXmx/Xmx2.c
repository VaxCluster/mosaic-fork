...
private void _XmxMenuAddEntryToRecord(XmxMenuRecord *rec, Widget w, int token)
{
    XmxMenuEntry *_ent;
...
private XmxMenuEntry *_XmxMenuGetEntryFromRecord(XmxMenuRecord *rec, int token)
{
...
private XmxMenuRecord *_XmxMenuCreateRecord(Widget base)
{
    XmxMenuRecord *_rec;
...
public void XmxRSetSensitive(XmxMenuRecord *rec, int token, int state)
{
    XmxMenuEntry *_entry;
...
/* args not used */
public void XmxRSetToggleState(XmxMenuRecord *rec, int token, int state)
{
    XmxMenuEntry *_entry;
    ...
    _entry = _XmxMenuGetEntryFromRecord(rec, XmxExtractToken(token));
    if (_entry)
        XmToggleButtonGadgetSetState(_entry->w, (state == XmxSet) ? True : False, False);
...
/* args used on Widget */
public void XmxRSetOptionMenuHistory(XmxMenuRecord *rec, int token)
{
    XmxMenuEntry *_entry;
...
/* args used on Widget */
public void XmxRSetValues(XmxMenuRecord *rec, int token)
{
    XmxMenuEntry *_entry;
...
/* args irrelevant */
public Widget XmxRGetWidget(XmxMenuRecord *rec, int token)
{
    XmxMenuEntry *_entry;
...
public XmxMenuRecord *XmxRMakeOptionMenu(Widget parent, String name, XtCallbackProc cb, XmxOptionMenuStruct *opts)
{
    XmxMenuRecord *_rec;
    ...
    while (opts[_i].namestr) {
        Xmx_n = 0;
        XmxSetArg(XmNlabelString, (XtArgVal) XmStringCreateLtoR(opts[_i].namestr, XmSTRING_DEFAULT_CHARSET));
        _button = XmCreatePushButtonGadget(_pulldown, "pushbutton", Xmx_wargs, Xmx_n);
        XtManageChild(_button);
        XtAddCallback(_button, XmNactivateCallback, cb, (XtPointer) _XmxMakeClientData(opts[_i].data));
    ...
    if (name) {
        XmxSetArg(XmNlabelString, (XtArgVal) XmStringCreateLtoR(name, XmSTRING_DEFAULT_CHARSET));
...
public XmxMenuRecord *XmxRMakeToggleMenu(Widget parent, int behavior, XtCallbackProc cb, XmxToggleMenuStruct *opts)
{
    XmxMenuRecord *_rec;
    Widget _box;
    int _i;
...
/* Possible deficiency: will not be able to grey out a submenu
   (cascade button). */
private void _XmxRCreateMenubar(Widget menu, XmxMenubarStruct *menulist, XmxMenuRecord *rec)
{
    int _i;
    Widget *_buttons;
    ...
    for (_i = 0; _i < _nitems; _i++) {
        ...
        if (strcmp(menulist[_i].namestr, "----") == 0) {
            XtCreateManagedWidget("separator", xmSeparatorGadgetClass, menu, NULL, 0);
            _separators++;
        }
        /* A function means it's an ordinary entry with callback. */
            ...
            if (menulist[_i].namestr[0] == '#' || menulist[_i].namestr[0] == '<') { /* option/toggle button */
                XmString xmstr;
                ...
                _buttons[_i - _separators] = XtCreateManagedWidget
                    ("togglebutton", xmToggleButtonGadgetClass, menu, Xmx_wargs, Xmx_n);
                XmStringFree(xmstr);

                XtAddCallback
                    (_buttons[_i - _separators],
                     XmNvalueChangedCallback, menulist[_i].func, (XtPointer) _XmxMakeClientData(menulist[_i].data));

                /* Add thie button to the menu record. */
                _XmxMenuAddEntryToRecord(rec, _buttons[_i - _separators], menulist[_i].data);
            } else {            /* regular button */
                XmString xmstr = XmStringCreateLtoR(menulist[_i].namestr, XmSTRING_DEFAULT_CHARSET);
                XmxSetArg(XmNlabelString, (XtArgVal) xmstr);
                _buttons[_i - _separators] = XtCreateManagedWidget
                    ("pushbutton", xmPushButtonGadgetClass, menu, Xmx_wargs, Xmx_n);
                XmStringFree(xmstr);
                XtAddCallback
                    (_buttons[_i - _separators], XmNactivateCallback,
                     menulist[_i].func, (XtPointer) _XmxMakeClientData(menulist[_i].data));
        ...
        /* No function and no submenu entry means it's just a label. */
            ...
            Xmx_n = 0;
            XmxSetArg(XmNlabelString, (XtArgVal) XmStringCreateLtoR(menulist[_i].namestr, XmSTRING_DEFAULT_CHARSET));
            _buttons[_i - _separators] = XtCreateManagedWidget("label", xmLabelGadgetClass, menu, Xmx_wargs, Xmx_n);
        }
        ...
        /* If all else fails, it's a submenu. */
            ...
            if (menulist[_i].mnemonic)
                XmxSetArg(XmNmnemonic, (XtArgVal) (menulist[_i].mnemonic));
            xmstr = XmStringCreateLtoR(menulist[_i].namestr, XmSTRING_DEFAULT_CHARSET);
            XmxSetArg(XmNlabelString, (XtArgVal) xmstr);
            _buttons[_i - _separators] = XtCreateWidget
                ("cascadebutton", xmCascadeButtonGadgetClass, menu, Xmx_wargs, Xmx_n);
            XmStringFree(xmstr);
...
public XmxMenuRecord *XmxRMakeMenubar(Widget parent, XmxMenubarStruct *mainmenu)
{
    Widget _menubar;
    XmxMenuRecord *_rec;
