...
Boolean *ctd;
{
    ...
    if (have_popup && (BuEvent->button == Button3)) {
        ...
        if (!popup) {
            ...
            popup = _PopupMenuBuilder((Widget) w, XmMENU_POPUP, "popup", 0, popup_items);
        }
        ...
        if (eptr) {
            type = eptr->type;
            if ((type == E_IMAGE) && (eptr->anchorHRef)) {  /* turn on anchor and off text */
                type |= E_ANCHOR;
                if (type & E_TEXT)
                    type -= E_TEXT;
            }
        ...
        for (i = 0; popup_items[i].class != LastItem; i++) {
            if (popup_items[i]._w) {    /* anything is possible in Mosaic */
                ...
                if (popup_items[i].acst.act_code == -2)
                    XtVaSetValues(popup_items[i]._w, XmNsubMenuId, current_win->session_menu, NULL);
                ...
                if (good) {
                    if (popup_items[i].class == Separator)
                        ...
                        case moMODE_PLAIN:
                            if (!(type & E_IMAGE)) {
                                if (!XtIsSensitive(popup_items[i]._w)
                                    && !sens)
                                    del += 24;
                                else
                                    sens = True;
...
void mo_make_popup(Widget view)
{
    ...
    XtInsertEventHandler(view, ButtonPressMask, False, (XtEventHandler) ThirdButtonMenu, NULL, XtListHead);
}
...
Widget _PopupMenuBuilder(Widget parent, int type, char *title, char mnem, PopupItem *items)
{
    Widget menu, cascade;
    XmString str;
    int i, mapping_del;
    ...
    if (type == XmMENU_POPUP) {
        ...
        cascade = XtVaCreateManagedWidget(title, xmCascadeButtonGadgetClass,
                                          ...
                                          XmNlabelString, str, XmNmnemonic, mnem, XmNmappingDelay, mapping_del, NULL);
        XmStringFree(str);
    } else
    ...
    for (i = 0; (items[i].class != LastItem); i++) {
        ...
        case PushButton:
            {
                items[i]._w = XtVaCreateManagedWidget(items[i].label, xmPushButtonGadgetClass, menu, NULL);
                if (items[i].mnemonic)
                    XtVaSetValues(items[i]._w, XmNmnemonic, items[i].mnemonic, NULL);
                if (items[i].accel) {
                    XtVaSetValues(items[i]._w, XmNaccelerator, items[i].accel, NULL);
                }
                ...
                if (items[i].cbfp)
                    XtAddCallback(items[i]._w, XmNactivateCallback, items[i].cbfp, &(items[i].acst));

                XtSetSensitive(items[i]._w, items[i].startup);

                if (items[i].acst.str && items[i].acst.act_code == 69) {
                    XtAddCallback(items[i]._w, XmNarmCallback, rbm_ballonify, items[i].acst.str);
                    XtAddCallback(items[i]._w, XmNdisarmCallback, rbm_ballonify, NULL);
                }
            }
            break;
        case Separator:
            {
                items[i]._w = XtVaCreateManagedWidget(items[i].label, xmSeparatorGadgetClass, menu, NULL);
            }
            break;
        case ToggleButton:
            {
                items[i]._w = XtVaCreateManagedWidget(items[i].label, xmToggleButtonGadgetClass, menu, NULL);
            }
            break;
        case CascadeButton:
            {
                if (items[i].sub_items && (items[i].acst.act_code != -2))
                    items[i]._w = _PopupMenuBuilder(menu, XmMENU_PULLDOWN,
                                                    items[i].label, items[i].mnemonic, items[i].sub_items);
                else {
                    int mapping_del;
                    ...
                    if (get_pref_boolean(eSESSION_HISTORY_ON_RBM)) {
                        mapping_del = get_pref_int(ePOPUPCASCADEMAPPINGDELAY);
                        items[i]._w =
                            XtVaCreateManagedWidget("Session History",
                                                    xmCascadeButtonGadgetClass,
                                                    menu,
                                                    XmNsubMenuId,
                                                    current_win->session_menu, XmNmappingDelay, mapping_del, NULL);
                    }
                }
            }
        }
    ...
    case M_LinkData:
        {
            ...
            xurl = mo_url_canonicalize(eptr->anchorHRef, strdup(current_win->current_node->url));
            do_meta = 2;
            mo_load_window_text(current_win, xurl, NULL);
            do_meta = 0;
...
void fsb(char *src)
{
    ...
    if (!dialog) {
        last_src = strdup(src);
        dialog = XmCreateFileSelectionDialog(current_win->view, "Save Image File", NULL, 0);
...
void hot_cb(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    case 1:                    /* add item */
        mo_add_item_to_hotlist(acst->str, mo_t_url,
                               ...
                               current_win->current_node->url, 0, get_pref_boolean(eADD_RBM_ADDS_RBM));
        break;
    case 2:                    /* add list */
        if (xurl = XmxModalPromptForString(current_win->base,
                                           XtWidgetToApplicationContext
                                           (current_win->base), "Enter New List Name:", "Okay", "Dismiss")) {
            mo_add_item_to_hotlist(acst->str, mo_t_list, xurl, NULL, 0, get_pref_boolean(eADD_RBM_ADDS_RBM));
        }
...
void mo_destroy_hot_menu(PopupItem *pmenu)
{
    ...
    for (i = 0; pmenu[i].class != LastItem; i++) {
        if ((pmenu[i].class != Separator) && (pmenu[i].acst.act_code != 1) && (pmenu[i].acst.act_code != 2))
            free(pmenu[i].label);
...
PopupItem *mo_assemble_hot_menu(mo_hotlist *list)
{
    ...
    for (i = 0, item = list->nodelist; item; item = item->any.next) {
        if ((item->type == mo_t_url && item->hot.rbm) || (item->type == mo_t_list && item->list.rbm)) {
            i++;
        }
    }
...
*/
    ...
    popup = _PopupMenuBuilder(current_win->view, XmMENU_POPUP, "popup", 0, popup_items);
...
{
    char *pt, *end = NULL, *bptr, *select = (char *)value, *str = (char *)client_data, *nselect, *begin;
    char mode;
    ...
    if (*type != XA_STRING || !select || !*select || !begin || !*begin || *length <= 0)
        return;
    ...
    if (pt && *pt && end && nselect) {
        ...
        if (mode == 'P') {
            ...
            mo_post_access_document(current_win, bptr, "application/x-www-form-urlencoded", pt);
            free(pt);
        } else if (mode == 'G') {
            pt[0] = '\0';
...
void user_defs_cb(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    XtGetSelectionValue(current_win->scrolled_win, XA_PRIMARY, XA_STRING, select_cb, str, cbs->event->xbutton.time);
}
...
void copy_link_cb(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    if (!acst || !acst->eptr || !acst->eptr->anchorHRef || !*acst->eptr->anchorHRef)
        return;
    ...
    if (XtOwnSelection((Widget) current_win->scrolled_win, XA_PRIMARY,
        ...
        for (i = 0; popup_items[i].class != LastItem; i++) {
            if (!strcmp(popup_items[i].label, COPY_URL_LABEL) && url) {
                char *copy_str = malloc((strlen(url) + strlen("URL:   has been copied  ")) * sizeof(char));
...
static Boolean convert_selection(Widget w, Atom *sel, Atom *tar,
                                 Atom *typ_ret, XtPointer *val_ret, unsigned long *val_len, int *format)
{
    char *url;
    int i;
...
void mo_add_to_rbm_history(mo_window *win, char *url, char *title)
{
    ...
    if (!get_pref_boolean(eSESSION_HISTORY_ON_RBM))
        ...
        win->session_menu = XmCreatePulldownMenu(win->view, "session_menu", NULL, 0);

    compact_string(title, label, 31, 3, 3);

    if (win->num_session_items < max) {
        win->session_items[win->num_session_items] =
            XtVaCreateManagedWidget(label, xmPushButtonGadgetClass, win->session_menu, NULL);

        XtAddCallback(win->session_items[win->num_session_items], XmNactivateCallback, session_cb, url);
        XtAddCallback(win->session_items[win->num_session_items], XmNarmCallback, rbm_ballonify, url);
        XtAddCallback(win->session_items[win->num_session_items], XmNdisarmCallback, rbm_ballonify, " ");
        ...
        win->session_items[max - 1] = XtVaCreateManagedWidget(label, xmPushButtonGadgetClass, win->session_menu, NULL);
        XtAddCallback(win->session_items[max - 1], XmNactivateCallback, session_cb, url);
        XtAddCallback(win->session_items[max - 1], XmNarmCallback, rbm_ballonify, url);
        XtAddCallback(win->session_items[max - 1], XmNdisarmCallback, rbm_ballonify, " ");
    }
}
