...
static XmxCallback(save_print_a4_cb)
{
    ...
    XmxSetToggleButton(win->print_a4_toggle_save, !XmToggleButtonGetState(win->print_a4_toggle_save));
    XmxSetToggleButton(win->print_us_toggle_save, !XmToggleButtonGetState(win->print_us_toggle_save));
...
static XmxCallback(print_url_cb)
{
    ...
    XmxSetToggleButton(win->print_doc_only, !XmToggleButtonGetState(win->print_doc_only));
    XmxSetToggleButton(win->print_url_only, i = !XmToggleButtonGetState(win->print_url_only));
}
...
static XmxCallback(mail_print_a4_cb)
{
    ...
    XmxSetToggleButton(win->print_a4_toggle_mail, !XmToggleButtonGetState(win->print_a4_toggle_mail));
    XmxSetToggleButton(win->print_us_toggle_mail, !XmToggleButtonGetState(win->print_us_toggle_mail));
...
static XmxCallback(print_print_a4_cb)
{
    ...
    XmxSetToggleButton(win->print_a4_toggle_print, !XmToggleButtonGetState(win->print_a4_toggle_print));
    XmxSetToggleButton(win->print_us_toggle_print, !XmToggleButtonGetState(win->print_us_toggle_print));
...
mo_status mo_save_window(mo_window *win, char *fname, mo_format_token save_format)
{
    ...
    if (!fp) {
        ...
        final_len = 30 + ((!efname || !*efname ? 3 : strlen(efname)) + 13) + 15 + (strlen(buf) + 3);
        ...
        if (save_format == mo_plaintext) {
            char *text = HTMLGetText(win->scrolled_win, 0, win->current_node->url,
                                     0);
            if (text) {
                fputs(text, fp);
                free(text);
            ...
            char *text = HTMLGetText(win->scrolled_win, 1, win->current_node->url,
                                     0);
            if (text) {
                fputs(text, fp);
                free(text);
...
static XmxCallback(save_win_cb)
{
    ...
    XmStringGetLtoR(((XmFileSelectionBoxCallbackStruct *) call_data)->value, XmSTRING_DEFAULT_CHARSET, &fname);
...
void format_sensitive(mo_window *win, int format)
{
    ...
    if (format == mo_plaintext) {
         /*PLAIN*/ XmxSetToggleButton(win->print_header_toggle_save, XmxNotSet);
    ...
    } else if (format == mo_formatted_text) {
         /*FORMATTED*/ XmxSetToggleButton(win->print_header_toggle_save, XmxNotSet);
        XmxSetToggleButton(win->print_footer_toggle_save, XmxNotSet);
        XmxSetToggleButton(win->print_a4_toggle_save, XmxNotSet);
        XmxSetToggleButton(win->print_us_toggle_save, XmxNotSet);
        n = 0;
        ...
        XtSetValues(win->print_a4_toggle_save, args, n);
        XtSetValues(win->print_us_toggle_save, args, n);
    } else if (format == mo_postscript) {
         /*POSTSCRIPT*/ XmxSetToggleButton(win->print_header_toggle_save, get_pref_boolean(ePRINT_BANNERS));
        ...
        XtSetValues(win->print_a4_toggle_save, args, n);
        XtSetValues(win->print_us_toggle_save, args, n);
    } else if (format == mo_html) {
         /*HTML*/ XmxSetToggleButton(win->print_header_toggle_save, XmxNotSet);
        XmxSetToggleButton(win->print_footer_toggle_save, XmxNotSet);
        XmxSetToggleButton(win->print_a4_toggle_save, XmxNotSet);
        XmxSetToggleButton(win->print_us_toggle_save, XmxNotSet);
        n = 0;
...
mo_status mo_post_save_window(mo_window *win)
{
    ...
    if (!win->save_win) {
        ...
        win->save_win = XmxMakeFileSBDialog
            (win->base, "NCSA Mosaic: Save Document", "Name for saved document:", save_win_cb, 0);
        ...
        /*swp */
        win->print_header_toggle_save = XmxMakeToggleButton(workarea, "Include Banners", save_print_header_cb, 0);
        XmxSetToggleButton(win->print_header_toggle_save, get_pref_boolean(ePRINT_BANNERS));
        win->print_footer_toggle_save = XmxMakeToggleButton(workarea, "Include Footnotes", save_print_footer_cb, 0);
        XmxSetToggleButton(win->print_footer_toggle_save, get_pref_boolean(ePRINT_FOOTNOTES));

        paper_size_toggle_box = XmxMakeRadioBox(workarea);
        win->print_a4_toggle_save = XmxMakeToggleButton(paper_size_toggle_box, "A4 Paper Size", save_print_a4_cb, 0);
        ...
        win->format_optmenu = XmxRMakeOptionMenu(workarea, "", format_optmenu_cb, format_opts);
        XmxSetArg(XmNtopOffset, 7);
        XmxSetConstraints
            (format_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
        XmxSetConstraints
            (win->format_optmenu->base, XmATTACH_FORM, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, format_label, NULL);
        /*swp */
        ...
        XmxSetConstraints
            (win->print_footer_toggle_save, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_FORM, XmATTACH_NONE, win->print_header_toggle_save, NULL, NULL, NULL);
        XmxSetConstraints
            (paper_size_toggle_box, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, XmATTACH_NONE, win->print_footer_toggle_save, NULL, NULL, NULL);
...
/*  char *cmd;*/
    ...
    if (unlink(temp_binary_fnam) < 0) {
        ...
        final_len =
            30 +
            ((!temp_binary_fnam || !*temp_binary_fnam ? 3 : strlen(temp_binary_fnam)) + 13) + 15 + (strlen(buf) + 13);
        final = (char *)calloc(final_len, sizeof(char));
...
static mo_status mo_post_savebinary_window(mo_window *win)
{
    ...
    if (!win->savebinary_win) {
        ...
        win->savebinary_win =
            XmxMakeFileSBDialog(win->base,
                                ...
                                "Name for binary file on local disk:", savebinary_win_cb, 0);
        XmxAddCallback(win->savebinary_win, XmNcancelCallback, savebinary_cancel_cb, 0);
    } else {
        XmFileSelectionDoSearch(win->savebinary_win, NULL);
    }

    /*SWP -- 9.21.95 -- Save File now goes to a specific filename */
    XtVaGetValues(win->savebinary_win, XmNdirSpec, &fbfn, NULL);
    if (!XmStringGetLtoR(fbfn, XmSTRING_DEFAULT_CHARSET, &fileBoxFileName)) {
#ifndef DISABLE_TRACE
        if (srcTrace) {
    ...
    if (*fileBoxFileName && saveFileName && *saveFileName) {
        ...
        sfn = XmStringCreateLtoR(fileBuf, XmSTRING_DEFAULT_CHARSET);
        XtVaSetValues(win->savebinary_win, XmNdirSpec, sfn, NULL);
    }
...
/*      Widget frame, workarea, format_label;*/
        ...
        win->open_local_win = XmxMakeFileSBDialog
            (win->base, "NCSA Mosaic: Open Local Document", "Name of local document to open:", open_local_win_cb, 0);
    } else {
...
mo_status mo_post_open_window(mo_window *win)
{
    if (!win->open_win) {
        ...
        XmxSetUniqid(win->id);
        win->open_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Open Document");
        ...
        buttons_form = XmxMakeFormAndFourButtons
            (open_form, open_win_cb, "Open", "Clear", "Dismiss", "Help...", 0, 3, 1, 2);

        /* Constraints for open_form. */
        XmxSetOffsets(label, 14, 0, 10, 0);
        XmxSetConstraints(label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
        XmxSetOffsets(win->open_text, 10, 0, 5, 10);
        XmxSetConstraints
            (win->open_text, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, label, NULL);
        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, win->open_text, buttons_form, NULL, NULL);
        XmxSetConstraints
...
mo_status mo_post_dtmout_window(mo_window *win)
{
    if (!win->dtmout_win) {
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->dtmout_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Open DTM Outport");
        ...
        buttons_form = XmxMakeFormAndThreeButtons(dtmout_form, dtmout_win_cb, "Open", "Dismiss", "Help...", 0, 1, 2);
        ...
        XmxSetConstraints
            (win->dtmout_text, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, label, NULL);
        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, win->dtmout_text, buttons_form, NULL, NULL);
        XmxSetConstraints
            (buttons_form, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
    }
...
static XmxCallback(mail_win_cb)
{
    ...
    case 0:
        ...
        if (win->mail_format == mo_plaintext) {
            ...
            text =
                HTMLGetText(win->scrolled_win, 2 + win->font_family,
        ...
        if (text)
            mo_send_mail_message(text, to, subj,
                                 ...
                                 "url_only" : content_type, win->current_node ? win->current_node->url : NULL);
...
void mail_sensitive(mo_window *win, int format)
{
    ...
    if (format == mo_plaintext) {
         /*PLAIN*/ XmxSetToggleButton(win->print_header_toggle_mail, XmxNotSet);
        ...
        XtSetValues(win->print_us_toggle_mail, args, n);
         ...
         /*FORMATTED*/ XmxSetToggleButton(win->print_header_toggle_mail, XmxNotSet);
        XmxSetToggleButton(win->print_footer_toggle_mail, XmxNotSet);
        XmxSetToggleButton(win->print_a4_toggle_mail, XmxNotSet);
        XmxSetToggleButton(win->print_us_toggle_mail, XmxNotSet);
        n = 0;
        ...
        XtSetValues(win->print_us_toggle_mail, args, n);
         ...
         /*POSTSCRIPT*/ XmxSetToggleButton(win->print_header_toggle_mail, get_pref_boolean(ePRINT_BANNERS));
        ...
        XtSetValues(win->print_us_toggle_mail, args, n);
         ...
         /*HTML*/ XmxSetToggleButton(win->print_header_toggle_mail, XmxNotSet);
        XmxSetToggleButton(win->print_footer_toggle_mail, XmxNotSet);
        XmxSetToggleButton(win->print_a4_toggle_mail, XmxNotSet);
        XmxSetToggleButton(win->print_us_toggle_mail, XmxNotSet);
        n = 0;
...
mo_status mo_post_mail_window(mo_window *win)
{
    ...
    if (!win->mail_win) {
...

        {
            ...
            /*swp */
            win->print_header_toggle_mail = XmxMakeToggleButton(workarea, "Include Banners", mail_print_header_cb, 0);
            XmxSetToggleButton(win->print_header_toggle_mail, get_pref_boolean(ePRINT_BANNERS));

            win->print_footer_toggle_mail = XmxMakeToggleButton(workarea, "Include Footnotes", mail_print_footer_cb, 0);
            XmxSetToggleButton(win->print_footer_toggle_mail, get_pref_boolean(ePRINT_FOOTNOTES));

            paper_size_toggle_box = XmxMakeRadioBox(workarea);
            win->print_a4_toggle_mail = XmxMakeToggleButton
                (paper_size_toggle_box, "A4 Paper Size", mail_print_a4_cb, 0);
            win->print_us_toggle_mail =
                XmxMakeToggleButton(paper_size_toggle_box, "US Letter Paper Size", mail_print_us_cb, 0);
            XmxSetToggleButton(win->print_a4_toggle_mail, !get_pref_boolean(ePRINT_PAPER_SIZE_US));
            XmxSetToggleButton(win->print_us_toggle_mail, get_pref_boolean(ePRINT_PAPER_SIZE_US));
            ...
            /* SWP -- 10/23/95 -- Set the default mode */
            if (!(get_pref_string(eMAIL_MODE))
                || !*(get_pref_string(eMAIL_MODE))) {
                char tbuf[BUFSIZ];
            ...
            if (!my_strcasecmp(get_pref_string(eMAIL_MODE), MODE_HTML)) {
                ...
                sprintf(tbuf,
                        ...
                        "mail", get_pref_string(eMAIL_MODE), "plain text mail");
            ...
            win->mail_fmtmenu = XmxRMakeOptionMenu(workarea, "", mail_fmtmenu_cb, format_opts);
            ...
            XmxSetArg(XmNtopOffset, 15);
            XmxSetConstraints
                (win->print_header_toggle_mail, XmATTACH_WIDGET,
                 XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, format_label, NULL, NULL, NULL);
        ...
        win->print_doc_only = XmxMakeToggleButton(url_toggle_box, "Mail Entire Document", print_url_cb, 0);
        win->print_url_only = XmxMakeToggleButton(url_toggle_box, "Mail URL Only", print_url_cb, 0);
        ...
        buttons_form = XmxMakeFormAndThreeButtons(mail_form, mail_win_cb, "Mail", "Dismiss", "Help...", 0, 1, 2);

        /* Constraints for mail_form. */
        XmxSetOffsets(to_label, 14, 0, 10, 0);
        XmxSetConstraints(to_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
        XmxSetOffsets(win->mail_to_text, 10, 0, 5, 10);
        XmxSetConstraints
            (win->mail_to_text, XmATTACH_FORM, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, to_label, NULL);

        XmxSetOffsets(subj_label, 14, 0, 10, 0);
        XmxSetConstraints
            (subj_label, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
             XmATTACH_NONE, win->mail_to_text, NULL, NULL, NULL);
        XmxSetOffsets(win->mail_subj_text, 10, 0, 5, 10);
        XmxSetConstraints
            (win->mail_subj_text, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, win->mail_to_text, NULL, subj_label, NULL);

        XmxSetOffsets(frame, 10, 0, 10, 10);
        XmxSetConstraints
            (frame, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
             XmATTACH_FORM, win->mail_subj_text, NULL, NULL, NULL);

        XmxSetOffsets(frame2, 10, 0, 10, 10);
        XmxSetConstraints
            (frame2, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, frame, NULL, NULL, NULL);

        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, frame2, buttons_form, NULL, NULL);
        XmxSetConstraints
            (buttons_form, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
    }
...
mo_status mo_print_window(mo_window *win, mo_format_token print_format, char *lpr)
{
    char *fnam, *cmd;
    FILE *fp;
    int retValue;
...
void print_sensitive(mo_window *win, int format)
{
    ...
    if (format == mo_plaintext) {
         /*PLAIN*/ XmxSetToggleButton(win->print_header_toggle_print, XmxNotSet);
        ...
        XtSetValues(win->print_us_toggle_print, args, n);
         ...
         /*FORMATTED*/ XmxSetToggleButton(win->print_header_toggle_print, XmxNotSet);
        XmxSetToggleButton(win->print_footer_toggle_print, XmxNotSet);
        XmxSetToggleButton(win->print_a4_toggle_print, XmxNotSet);
        XmxSetToggleButton(win->print_us_toggle_print, XmxNotSet);
        n = 0;
        ...
        XtSetValues(win->print_us_toggle_print, args, n);
         ...
         /*POSTSCRIPT*/ XmxSetToggleButton(win->print_header_toggle_print, get_pref_boolean(ePRINT_BANNERS));
        ...
        XtSetValues(win->print_us_toggle_print, args, n);
         ...
         /*HTML*/ XmxSetToggleButton(win->print_header_toggle_print, XmxNotSet);
        XmxSetToggleButton(win->print_footer_toggle_print, XmxNotSet);
        XmxSetToggleButton(win->print_a4_toggle_print, XmxNotSet);
        XmxSetToggleButton(win->print_us_toggle_print, XmxNotSet);
        n = 0;
...
mo_status mo_post_print_window(mo_window *win)
{
    ...
    if (!win->print_win) {
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->print_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Print Document");
...

        {
            ...
            /*swp */
            win->print_header_toggle_print = XmxMakeToggleButton(workarea, "Include Banners", print_print_header_cb, 0);
            ...
            win->print_us_toggle_print =
                XmxMakeToggleButton(paper_size_toggle_box, "US Letter Paper Size", print_print_us_cb, 0);
            XmxSetToggleButton(win->print_a4_toggle_print, !get_pref_boolean(ePRINT_PAPER_SIZE_US));
            XmxSetToggleButton(win->print_us_toggle_print, get_pref_boolean(ePRINT_PAPER_SIZE_US));
            ...
            /* SWP -- 10/23/95 -- Set the default mode */
            if (!(get_pref_string(ePRINT_MODE))
                || !*(get_pref_string(ePRINT_MODE))) {
                char tbuf[BUFSIZ];
            ...
            if (!my_strcasecmp(get_pref_string(ePRINT_MODE), MODE_HTML)) {
                ...
                sprintf(tbuf,
                        "You have set the default %s mode to:\n     [%s], which is not valid. Defaulting to %s mode.\n\nPlease use one of the following:\n     plain, formatted, postscript, or html.",
                        "print", get_pref_string(ePRINT_MODE), "plain text print");
            ...
            win->print_fmtmenu = XmxRMakeOptionMenu(workarea, "", print_fmtmenu_cb, format_opts);
            ...
            XmxSetConstraints
                (win->print_fmtmenu->base, XmATTACH_FORM, XmATTACH_NONE,
                 XmATTACH_NONE, XmATTACH_FORM, NULL, NULL, NULL, NULL);
            /*swp */
            XmxSetArg(XmNtopOffset, 15);
            XmxSetConstraints
                (win->print_header_toggle_print, XmATTACH_WIDGET,
                 XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, format_label, NULL, NULL, NULL);
            XmxSetConstraints(win->print_footer_toggle_print,
                              XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
                              XmATTACH_NONE, win->print_header_toggle_print, NULL, NULL, NULL);
            XmxSetConstraints(paper_size_toggle_box, XmATTACH_WIDGET,
                              XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE,
        ...
        buttons_form = XmxMakeFormAndThreeButtons(print_form, print_win_cb, "Print", "Dismiss", "Help...", 0, 1, 2);

        /* Constraints for print_form. */
        XmxSetOffsets(print_label, 14, 0, 10, 0);
        XmxSetConstraints
            (print_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
        ...
        XmxSetOffsets(frame, 10, 0, 10, 10);
        XmxSetConstraints
            (frame, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, win->print_text, NULL, NULL, NULL);

        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, frame, buttons_form, NULL, NULL);
...
mo_status mo_source_search_window(mo_window *win, char *str, int backward, int caseless)
{

    int searchlen, start, str_len = strlen(str);
    char *ptr = NULL, *tptr = NULL, *my_str = NULL;

    if (!(win) || !(win->current_node) || !(win->current_node->text) || !*(win->current_node->text)) {
    ...
    if (!backward) {
        ...
        source_position(win->source_text, win->src_search_pos - str_len, win->src_search_pos);
    } else {
        win->src_search_pos = (ptr - my_str);
        source_position(win->source_text, win->src_search_pos, win->src_search_pos + str_len);
    }
...
mo_status mo_post_source_search_window(mo_window *win)
{
    ...
    if (!win->src_search_win) {
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->src_search_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Search in Source View");
        dialog_frame = XmxMakeFrame(win->src_search_win, XmxShadowOut);

        /* Constraints for base. */
        XmxSetConstraints(dialog_frame,
                          XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
        ...
        XmxAddCallbackToText(win->src_search_win_text, source_search_win_cb, 0);

        win->src_search_caseless_toggle = XmxMakeToggleButton(search_form, "Caseless Search", NULL, 0);
        XmxSetToggleButton(win->src_search_caseless_toggle, XmxSet);
        win->src_search_backwards_toggle = XmxMakeToggleButton(search_form, "Backwards Search", NULL, 0);

        dialog_sep = XmxMakeHorizontalSeparator(search_form);

        buttons_form =
            XmxMakeFormAndFourButtons(search_form,
                                      source_search_win_cb, "Find", "Reset", "Dismiss", "Help...", 0, 1, 2, 3);
        ...
        XmxSetConstraints(win->src_search_win_text,
                          XmATTACH_FORM, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, label, NULL);
        ...
        XmxSetConstraints(win->src_search_caseless_toggle,
                          XmATTACH_WIDGET,
                          XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_NONE, win->src_search_win_text, NULL, label, NULL);
        ...
        XmxSetConstraints(win->src_search_backwards_toggle,
                          XmATTACH_WIDGET,
                          XmATTACH_NONE,
                          XmATTACH_WIDGET, XmATTACH_NONE, win->src_search_caseless_toggle, NULL, label, NULL);
        ...
        XmxSetConstraints(dialog_sep,
                          XmATTACH_WIDGET,
                          XmATTACH_WIDGET,
                          XmATTACH_FORM, XmATTACH_FORM, win->src_search_backwards_toggle, buttons_form, NULL, NULL);
        XmxSetConstraints(buttons_form,
                          XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
    }
...
mo_status mo_post_source_window(mo_window *win)
{
    ...
    if (!win->source_win) {
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->source_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Document Source View");
        ...
        buttons_form =
            XmxMakeFormAndThreeButtonsSqueezed(source_form, source_win_cb, "Search...", "Dismiss", "Help...", 2, 0, 1);
        ...
        XmxSetConstraints(dlabel,
                          XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, label, NULL, NULL, NULL);
        ...
        XmxSetConstraints(win->source_date_text,
                          XmATTACH_WIDGET,
                          XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, win->source_url_text, NULL, dlabel, NULL);
        XmxSetConstraints(XtParent(win->source_text),
                          XmATTACH_WIDGET,
                          XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, win->source_date_text, dialog_sep, NULL, NULL);
        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints(dialog_sep,
                          XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, NULL, buttons_form, NULL, NULL);
        XmxSetConstraints(buttons_form,
                          XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
    }
...
mo_status mo_search_window(mo_window *win, char *str, int backward, int caseless, int news)
{
    ...
    if (!backward) {
        ...
        if (((ElementRef *) win->search_start)->id) {
            ((ElementRef *) win->search_start)->id = ((ElementRef *) win->search_end)->id;
            ((ElementRef *) win->search_start)->pos = ((ElementRef *) win->search_end)->pos;
        }
    }

    if (news) {
        rc = HTMLSearchNews(win->scrolled_win, (ElementRef *) win->search_start, (ElementRef *) win->search_end);
    } else {
        rc = HTMLSearchText(win->scrolled_win, str,
                            (ElementRef *) win->search_start, (ElementRef *) win->search_end, backward, caseless);
    }
    ...
    if (rc == -1) {
        ...
        /* Now search_start and search_end are starting and ending
           points of the match. */
        HTMLGotoId(win->scrolled_win, ((ElementRef *) win->search_start)->id, (news ? (-1) : 0));

        /* Set the selection. */
        if (!news) {
            HTMLSetSelection(win->scrolled_win, (ElementRef *) win->search_start, (ElementRef *) win->search_end);
        }
    }                           /* found a target */
...
mo_status mo_post_search_window(mo_window *win)
{
    if (!win->search_win) {
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->search_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Find In Document");
        ...
        win->search_caseless_toggle = XmxMakeToggleButton(search_form, "Caseless Search", NULL, 0);
        XmxSetToggleButton(win->search_caseless_toggle, XmxSet);
        win->search_backwards_toggle = XmxMakeToggleButton(search_form, "Backwards Search", NULL, 0);

        dialog_sep = XmxMakeHorizontalSeparator(search_form);

        buttons_form = XmxMakeFormAndFourButtons
            (search_form, search_win_cb, "Find", "Reset", "Dismiss", "Help...", 0, 1, 2, 3);
        ...
        /* search_win_text attaches top to form, bottom to nothing,
           left to label, right to form. */
        XmxSetConstraints
            (win->search_win_text, XmATTACH_FORM, XmATTACH_NONE,
        ...
        XmxSetConstraints
            (win->search_caseless_toggle, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_NONE, win->search_win_text, NULL, label, NULL);
        ...
        XmxSetConstraints
            (win->search_backwards_toggle, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_NONE, win->search_caseless_toggle, NULL, label, NULL);
        ...
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, win->search_backwards_toggle, buttons_form, NULL, NULL);
        XmxSetConstraints
            (buttons_form, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
    }
...
*/
    ...
    if (!(fp = fopen(sourceFileName, "w"))) {
        ...
        final_len =
            30 + ((!sourceFileName || !*sourceFileName ? 3 : strlen(sourceFileName)) + 13) + 15 + (strlen(buf) + 3);
        final = (char *)calloc(final_len, sizeof(char));
    ...
    sprintf(editorTitle, "(NCSA Mosaic) Editing Copy of: %s", win->current_node->url);

    argCount = 0;
    if (get_pref_boolean(eEDIT_COMMAND_USE_XTERM)) {
        sprintf(execString, "%s -T %s -e %s", get_pref_string(eXTERM_COMMAND), editorTitle, editorCommand);
