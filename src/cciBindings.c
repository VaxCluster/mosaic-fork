    ...
    if (!get_pref_int(eLOAD_LOCAL_FILE))
        if (!my_strncasecmp(url, "file:", 5)) {
            *retCode = MCCIR_GET_FAILED;
            strcpy(retText, " Can't get local file (for CCI security reasons)");
            return;
        }
...
void MCCIRequestPost(client, retCode, retText, url, contentType, postData, dataLength, output)
MCCIPort client;
int *retCode;
char *retText;
char *url;
...
int output;
{
    ...
    *retCode = MCCIR_POST_OK;
    ...
    case MCCI_OUTPUT_NONE:
        ...
        response = mo_post_pull_er_over(url, contentType, postData, &textHead);

        /* send response back through cci */
        if (response && (length = strlen(response))) {
            MCCISendResponseLine(client, MCCIR_POST_OUTPUT, "POST output");
...
void MCCIRequestDisplay(client, retCode, retText, url, contentType, displayData, dataLength, output)
MCCIPort client;
int *retCode;
char *retText;
char *url;
    ...
    default:
        ...
        if (strcmp(contentType, "text/html") == 0)
            mo_do_window_text(current_win, new_url,
                              ...
                              current_win->current_node->last_modified, current_win->current_node->expires);
        else
