...
static void mo_read_global_history(char *filename)
{
    ...
    if (strncmp(line, NCSA_HISTORY_FORMAT_COOKIE_ONE, strlen(NCSA_HISTORY_FORMAT_COOKIE_ONE))) {
        if (strncmp(line, NCSA_HISTORY_FORMAT_COOKIE_TWO, strlen(NCSA_HISTORY_FORMAT_COOKIE_TWO))) {
            goto screwed_with_file;
        } else {
            format = 2;
        }
...
mo_status mo_setup_global_history(void)
{
    ...
    filename = (char *)malloc((strlen(home) + strlen(get_pref_string(eHISTORY_FILE)) + 8) * sizeof(char));
        ...
        if (cached_cd_array[i])
            if (cacheTrace) {
                fprintf(stderr, "  %02d data 0x%08x last_access %d\n", i,
                        cached_cd_array[i]->image_data, cached_cd_array[i]->last_access);
            } else if (cacheTrace) {
...
/*  int i;*/
    ...
    cached_cd_array = (cached_data **) malloc(sizeof(cached_data *) * CHUNK_OF_IMAGES);
...
/*  bzero ((char *)cached_cd_array, 
    ...
    memset((char *)cached_cd_array, 0, CHUNK_OF_IMAGES * sizeof(cached_cd_array[0]));
...
/*  int i;*/
    ...
    cached_cd_array = (cached_data **) realloc
        (cached_cd_array, sizeof(cached_data *) * (size_of_cached_cd_array + CHUNK_OF_IMAGES));

#ifndef DISABLE_TRACE
    if (cacheTrace)
        fprintf(stderr,
                "[grow] cached_cd_array 0x%08x, size_of_cached_cd_array 0x%08x, sum 0x%08x\n",
                cached_cd_array, size_of_cached_cd_array, cached_cd_array + size_of_cached_cd_array);
#endif
/*  bzero ((char *)(cached_cd_array + size_of_cached_cd_array), 
         CHUNK_OF_IMAGES * sizeof (cached_cd_array[0]));*/
    memset((char *)(cached_cd_array + size_of_cached_cd_array), 0, CHUNK_OF_IMAGES * sizeof(cached_cd_array[0]));
...
static mo_status mo_sort_cached_cd_array(void)
{
    ...
    qsort((void *)cached_cd_array, size_of_cached_cd_array, sizeof(cached_cd_array[0]), mo_sort_cd_for_qsort);
            ...
            if (cacheTrace)
                fprintf
                    (stderr,
                     ...
                     (kbytes_cached + kbytes_in_new_image), get_pref_int(eIMAGE_CACHE_SIZE));
#endif

            /* Try to remove one -- we rely on the fact that NULL
               entries in cached_cd_array are at the end of the array. */
            if (num_to_remove < size_of_cached_cd_array && cached_cd_array[num_to_remove] != NULL) {
#ifndef DISABLE_TRACE
                if (cacheTrace)
                    fprintf(stderr,
                            "        ** going to try to remove %d; last_access %d < dont_nuke_after_me %d??\n",
