...
struct pixload_info {
    ...
    EASTER_ARRAY { unlock_none_xpm, &securityNone, 0 },
...
static Pixmap PixmapFromData(Widget wid, unsigned char *data, int width, int height, XColor * colrs, int gray);
...
static Pixmap PixmapFromData(Widget wid, unsigned char *data, int width, int height, XColor *colrs, int gray)
{
    ...
    vinfo.visualid = XVisualIDFromVisual(DefaultVisual(XtDisplay(wid), DefaultScreen(XtDisplay(wid))));
        ...
        if ((Vclass == TrueColor) || (Vclass == DirectColor)) {
            ...
            if (hash_ptr == NULL) {
                FindIconColor(XtDisplay(wid),
                              (installed_colormap ? installed_cmap : DefaultColormapOfScreen(XtScreen(wid))), &tmpcolr);
                PixAddHash(colrs[i].red, colrs[i].green, colrs[i].blue, tmpcolr.pixel);
...
/*		bcopy(data, bit_data, size);*/
        ...
        newimage = XCreateImage(XtDisplay(wid),
                                DefaultVisual(XtDisplay(wid),
                                              DefaultScreen(XtDisplay
                                                            (wid))), depth,
                                ZPixmap, 0, (char *)bit_data, width, height, 8, bytesperline);
        break;
    case 1:
    case 2:
    case 4:
        ...
        *bitp = 0;
        ...
        newimage = XCreateImage(XtDisplay(wid),
                                DefaultVisual(XtDisplay(wid),
                                              DefaultScreen(XtDisplay
                                                            (wid))), depth,
                                ZPixmap, 0, (char *)bit_data, (width + linepad), height, 8, bytesperline);
        break;
        /*
    ...
    case 16:
        ...
        theVisual = DefaultVisual(XtDisplay(wid), DefaultScreen(XtDisplay(wid)));
        ...
        for (w = size; w > 0; w--) {
            temp = (((colrs[(int)*datap].red >> rshift) &
                    ...
                    ((colrs[(int)*datap].green >> gshift) &
                     theVisual->green_mask) | ((colrs[(int)*datap].blue >> bshift) & theVisual->blue_mask));
        ...
        newimage = XCreateImage(XtDisplay(wid),
                                DefaultVisual(XtDisplay(wid),
                                              DefaultScreen(XtDisplay
                                                            (wid))), depth,
                                ZPixmap, 0, (char *)bit_data, width, height, 16, 0);
        break;
    case 24:
        ...
        datap = data;
        for (w = size; w > 0; w--) {
            c = (((colrs[(int)*datap].red >> 8) & 0xff) << rshift) |
        ...
        newimage = XCreateImage(XtDisplay(wid),
                                DefaultVisual(XtDisplay(wid),
                                              DefaultScreen(XtDisplay
                                                            (wid))), depth,
                                ZPixmap, 0, (char *)bit_data, width, height, 32, 0);
        break;
    default:
        newimage = NULL;
    }
    ...
    if (newimage != NULL) {
        ...
        pix = XCreatePixmap(XtDisplay(wid), XtWindow(wid), width, height, depth);
        drawGC = XCreateGC(XtDisplay(wid), XtWindow(wid), 0, NULL);
        XSetFunction(XtDisplay(wid), drawGC, GXcopy);

        XPutImage(XtDisplay(wid), pix, drawGC, newimage, 0, 0, 0, 0, width, height);
...
{
    XFreeColors(dsp, (installed_colormap ? installed_cmap : DefaultColormapOfScreen(XtScreen(wid))), p, j, 0);
}
...
Pixmap LoadSplashXPM(Widget wid, int *colorcount)
{
    ...
    if (XAllocColorCells(dsp, (installed_colormap ?
                               installed_cmap :
                               DefaultColormapOfScreen(XtScreen(wid))), False, NULL, 0, p, *colorcount)) {
        XFreeColors(dsp, (installed_colormap ?
                          installed_cmap : DefaultColormapOfScreen(XtScreen(wid))), p, *colorcount, 0);
        *colorcount = 1;
    } else {
...
*/
    ...
    XtVaSetValues(wid, XmNlabelPixmap, pix, XmNlabelType, XmPIXMAP, NULL);
...
void AnimatePixmapInWidget(Widget wid, Pixmap pix)
{
    ...
    XCopyArea(XtDisplay(wid), pix, XtWindow(wid), DrawGC, 0, 0, IconWidth, IconHeight, x, y);
}
