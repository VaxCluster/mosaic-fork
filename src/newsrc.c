...
void dumpgroup(newsgroup_t *ng)
{
    ...
    printf("Name: %s [%ld %ld] %ld %c%c%c\n",
           ...
           (ng->attribs & naSUBSCRIBED) ? 'S' : 's', (ng->attribs & naNEWGROUP) ? 'N' : 'n');
...
newsgroup_t *addgroup(char *nline, long min, long max, int dri)
{
    ...
    /* Pull out the name */
    name = nline;
    while (*nline && *nline != ' ' && *nline != '\t' && *nline != ':' && *nline != '!' && *nline != '\n')
        nline++;
...
int newsrc_init(char *newshost)
{
    ...
    if ((newsrc_fp = fopen(newsrc_filename, "r")) == NULL) {
        sprintf(buf, "News file %s/%s%s%s does not exist", home, npref, spref ? "" : "-", spref ? "" : newshost);
        HTProgress(buf);
        newsNoNewsRC = 1;
        newsrc_active = 1;
        return 0;
    ...
    if (get_pref_boolean(eUSEBACKGROUNDFLUSH)) {
        newsrc_ti = XtAppAddTimeOut(app_context, 1000L * get_pref_int(eBACKGROUNDFLUSHTIME), newsrc_flushcb, NULL);
        newsrc_timer = 1;
    }
    newsrc_active = 1;
    return 0;
