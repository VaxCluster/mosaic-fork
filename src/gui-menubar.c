...
{
    XmxRSetToggleState(win->menubar, mo_fancy_selections, win->pretty ? XmxSet : XmxNotSet);
    return mo_succeed;
}
...
mo_status mo_set_underlines(mo_window *win, int choice)
{
    if (!win->underlines_snarfed) {
        XtVaGetValues(win->scrolled_win,
                      ...
                      &(win->visited_underlines), WbNdashedAnchorUnderlines,
                      &(win->dashed_underlines),
                      WbNdashedVisitedAnchorUnderlines, &(win->dashed_visited_underlines), NULL);
    ...
    case mo_default_underlines:
        ...
        XmxSetArg(WbNdashedVisitedAnchorUnderlines, win->dashed_visited_underlines);
        XmxSetValues(win->scrolled_win);
        break;
...
static mo_status mo_do_delete_annotation(mo_window *win)
{
    ...
    if (win->current_node->annotation_type == mo_annotation_private) {
        mo_grok_pan_pieces(win->current_node->url, win->current_node->text, &title, &author, &text, &id, &fname);
...
XmxCallback(menubar_cb)
{
    ...
    case mo_reload_document:
        if (cci_event)
            MoCCISendEventOutput(MOSAIC_RELOAD_CURRENT);
        mo_reload_window_text(win, 0);
        break;
    case mo_reload_document_and_images:
        if (cci_event)
            MoCCISendEventOutput(FILE_RELOAD_IMAGES);
        mo_reload_window_text(win, 1);
        break;
    case mo_refresh_document:
        if (cci_event)
            MoCCISendEventOutput(FILE_REFRESH_CURRENT);
        mo_refresh_window_text(win);
        break;
    case mo_re_init:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_RELOAD_CONFIG_FILES);
        mo_re_init_formats();
        break;
    case mo_clear_image_cache:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FLUSH_IMAGE_CACHE);
    ...
    case mo_kerberosv4_login:
        if (cci_event)
            MoCCISendEventOutput(FILE_KERBEROS_V4_LOGIN);
    ...
    case mo_kerberosv5_login:
        if (cci_event)
            MoCCISendEventOutput(FILE_KERBEROS_V5_LOGIN);
    ...
    case mo_home_document:
        if (cci_event)
            MoCCISendEventOutput(MOSAIC_HOME_DOCUMENT);
    ...
    case mo_mosaic_demopage:
        if (cci_event)
            MoCCISendEventOutput(HELP_DEMO);
        mo_open_another_window(win, DEMO_PAGE_DEFAULT, NULL, NULL);
        break;
    case mo_mosaic_manual:
        if (cci_event)
            MoCCISendEventOutput(HELP_MANUAL);
        mo_open_another_window(win, mo_assemble_help_url("mosaic-docs.html"), NULL, NULL);
        break;

    case mo_back:
        if (cci_event)
            MoCCISendEventOutput(MOSAIC_BACK);
        mo_back_node(win);
        break;
    case mo_forward:
        if (cci_event)
            MoCCISendEventOutput(MOSAIC_FORWARD);
        mo_forward_node(win);
        break;
    case mo_history_list:
        if (cci_event)
            MoCCISendEventOutput(NAVIGATE_WINDOW_HISTORY);
        mo_post_history_win(win);
        break;
    case mo_clear_global_history:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_CLEAR_GLOBAL_HISTORY);
        XmxSetUniqid(win->id);
    ...
    case mo_hotlist_postit:
        if (cci_event)
            MoCCISendEventOutput(NAVIGATE_HOTLIST);
...
/* removed 5/17/96 - bjs 
    ...
    case mo_expand_images_current:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_LOAD_IMAGES_IN_CURRENT);
    ...
    case mo_large_fonts:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_TL);
        mo_set_fonts(win, i);
        break;
    case mo_regular_fonts:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_TR);
        mo_set_fonts(win, i);
        break;
    case mo_small_fonts:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_TS);
        mo_set_fonts(win, i);
        break;
    case mo_large_helvetica:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_HL);
        mo_set_fonts(win, i);
        break;
    case mo_regular_helvetica:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_HR);
        mo_set_fonts(win, i);
        break;
    case mo_small_helvetica:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_HS);
        mo_set_fonts(win, i);
        break;
    case mo_large_newcentury:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_NCL);
        mo_set_fonts(win, i);
        break;
    case mo_regular_newcentury:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_NCR);
        mo_set_fonts(win, i);
        break;
    case mo_small_newcentury:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_NCS);
        mo_set_fonts(win, i);
        break;
    case mo_large_lucidabright:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_LBL);
        mo_set_fonts(win, i);
        break;
    case mo_regular_lucidabright:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_LBR);
        mo_set_fonts(win, i);
        break;
    case mo_small_lucidabright:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_FONTS_LBS);
        mo_set_fonts(win, i);
        break;

    case mo_default_underlines:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_ANCHOR_UNDERLINES_DU);
        mo_set_underlines(win, i);
        break;
    case mo_l1_underlines:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_ANCHOR_UNDERLINES_LU);
        mo_set_underlines(win, i);
        break;
    case mo_l2_underlines:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_ANCHOR_UNDERLINES_MU);
        mo_set_underlines(win, i);
        break;
    case mo_l3_underlines:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_ANCHOR_UNDERLINES_HU);
        mo_set_underlines(win, i);
        break;
    case mo_no_underlines:
        if (cci_event)
            MoCCISendEventOutput(OPTIONS_ANCHOR_UNDERLINES_NU);
        mo_set_underlines(win, i);
        break;
    case mo_help_about:
        if (cci_event)
            MoCCISendEventOutput(HELP_ABOUT);
        mo_open_another_window(win, mo_assemble_help_url("help-about.html"), NULL, NULL);
        break;
    case mo_help_onwindow:
        if (cci_event)
            MoCCISendEventOutput(HELP_ON_WINDOW);
        mo_open_another_window(win, mo_assemble_help_url("help-on-docview-window.html"), NULL, NULL);
        break;
    case mo_whats_new:
        if (cci_event)
            MoCCISendEventOutput(HELP_WHATS_NEW);
        mo_open_another_window(win, WHATSNEW_PAGE_DEFAULT, NULL, NULL);
        break;
    case mo_help_onversion:
        if (cci_event)
            MoCCISendEventOutput(HELP_ON_VERSION);
        mo_open_another_window(win, MO_HELP_ON_VERSION_DOCUMENT, NULL, NULL);
        break;
    case mo_help_faq:
        if (cci_event)
            MoCCISendEventOutput(HELP_ON_FAQ);
        mo_open_another_window(win, mo_assemble_help_url("mosaic-faq.html"), NULL, NULL);
        break;
    case mo_help_html:
        if (cci_event)
            MoCCISendEventOutput(HELP_ON_HTML);
        mo_open_another_window(win, HTMLPRIMER_PAGE_DEFAULT, NULL, NULL);
        break;
    case mo_help_url:
        if (cci_event)
            MoCCISendEventOutput(HELP_ON_URLS);
        mo_open_another_window(win, URLPRIMER_PAGE_DEFAULT, NULL, NULL);
        break;
#ifndef PRERELEASE
    case mo_cc:
        if (cci_event)
            MoCCISendEventOutput(HELP_COMMENT_CARD);
        do_comment = 1;
        CommentCard(win);
        break;
#endif
    case mo_techsupport:
        if (cci_event)
            MoCCISendEventOutput(HELP_MAIL_TECH_SUPPORT);
...
/*      mo_post_techsupport_win (win);*/
    ...
    case mo_annotate:
        if (cci_event)
            MoCCISendEventOutput(ANNOTATE_ANNOTATE);
        mo_post_annotate_win(win, 0, 0, NULL, NULL, NULL, NULL);
        break;
    case mo_news_prev:
        if (cci_event)
            MoCCISendEventOutput(NEWS_PREV);
        gui_news_prev(win);
        break;
    case mo_news_next:
        if (cci_event)
            MoCCISendEventOutput(NEWS_NEXT);
        gui_news_next(win);
        break;
    case mo_news_prevt:
        if (cci_event)
            MoCCISendEventOutput(NEWS_PREV_THREAD);
        gui_news_prevt(win);
        break;
    case mo_news_nextt:
        if (cci_event)
            MoCCISendEventOutput(NEWS_NEXT_THREAD);
        gui_news_nextt(win);
        break;
    case mo_news_index:
        if (cci_event)
            MoCCISendEventOutput(NEWS_INDEX);
        gui_news_index(win);
        break;
    case mo_news_flush:
        gui_news_flush(win);
    ...
    case mo_news_list:
        if (cci_event)
            MoCCISendEventOutput(NEWS_LIST_GROUPS);
        gui_news_list(win);
        break;
    case mo_news_fmt0:
        if (cci_event)
            MoCCISendEventOutput(NEWS_FORMAT_TV);
    ...
    case mo_news_fmt1:
        if (cci_event)
            MoCCISendEventOutput(NEWS_FORMAT_GV);
    ...
    case mo_news_post:
        if (cci_event)
            MoCCISendEventOutput(NEWS_POST);
        mo_post_news_win(win);
        break;
    case mo_news_follow:
        if (cci_event)
            MoCCISendEventOutput(NEWS_FOLLOW_UP);
    ...
    case mo_ftp_put:
        if (cci_event)
            MoCCISendEventOutput(FTP_PUT);
        mo_handle_ftpput(win);
        break;
    case mo_ftp_mkdir:
        if (cci_event)
            MoCCISendEventOutput(FTP_MKDIR);
    ...
    case mo_audio_annotate:
        if (cci_event)
            MoCCISendEventOutput(ANNOTATE_AUDIO_ANNOTATE);
    ...
    case mo_annotate_edit:
        ...
        if (cci_event)
            MoCCISendEventOutput(ANNOTATE_EDIT_THIS_ANNOTATION);
        if (win->current_node) {
            char *author, *title, *text, *fname;
            int id;
            ...
            if (win->current_node->annotation_type == mo_annotation_private) {
                mo_grok_pan_pieces(win->current_node->url,
                                   win->current_node->text, &title, &author, &text, &id, &fname);
    ...
    case mo_annotate_delete:
        if (cci_event)
            MoCCISendEventOutput(ANNOTATE_DELETE_THIS_ANNOTATION);
...
/* --------------------------- format options ------------------------------ */
    ALLOC_OPTIONS(format_opts, 5)
        ...
        DEFINE_OPTIONS("HTML", mo_html, XmxNotSet) NULL_OPTIONS()
/* -------------------------- annotation options --------------------------- */
        ALLOC_OPTIONS(pubpri_opts, 4)
        DEFINE_OPTIONS("Personal Annotation", mo_annotation_private, XmxSet)
        DEFINE_OPTIONS("Workgroup Annotation", mo_annotation_workgroup,
                       XmxNotSet) DEFINE_OPTIONS("Public Annotation", mo_annotation_public, XmxNotSet) NULL_OPTIONS()
/* ----------------------- full menubar interface -------------------------- */
        ...
        SPACER()DEFINE_MENUBAR("Open URL...", "O", menubar_cb,
                               mo_open_document,
                               NULL) DEFINE_MENUBAR("Open Local...", "L", menubar_cb, mo_open_local_document, NULL)
        SPACER()DEFINE_MENUBAR("Reload Current", "R", menubar_cb,
                               mo_reload_document,
                               NULL) DEFINE_MENUBAR("Reload Images", "a",
                                                    menubar_cb, mo_reload_document_and_images, NULL)
        DEFINE_MENUBAR("Refresh Current", "f", menubar_cb,
                       mo_refresh_document,
                       NULL) SPACER()DEFINE_MENUBAR("Find In Current", "I", menubar_cb, mo_search, NULL)
        DEFINE_MENUBAR("View Source...", "V", menubar_cb, mo_document_source,
                       NULL) DEFINE_MENUBAR("Edit Source...", "E", menubar_cb, mo_document_edit, NULL)
        SPACER()DEFINE_MENUBAR("Save As...", "S", menubar_cb,
                               mo_save_document,
                               NULL) DEFINE_MENUBAR("Print...", "P", menubar_cb, mo_print_document, NULL)
        DEFINE_MENUBAR("Mail To...", "M", menubar_cb, mo_mail_document,
                       NULL) SPACER()DEFINE_MENUBAR("CCI...", "D", menubar_cb, mo_cci, NULL)
#ifdef HAVE_DTM
        SPACER()DEFINE_MENUBAR("Open DTM Outport...", "u", menubar_cb,
                               mo_dtm_open_outport,
                               NULL) DEFINE_MENUBAR("Broadcast Over DTM", "B", menubar_cb, mo_dtm_send_document, NULL)
#endif                          /* HAVE_DTM */
/*SWP -- 7/17/95*/
        ...
        SPACER()DEFINE_MENUBAR("Proxy List...", "0", menubar_cb, mo_proxy,
                               NULL) DEFINE_MENUBAR("No Proxy List...", "1", menubar_cb, mo_no_proxy, NULL)
        SPACER()DEFINE_MENUBAR("Close", "W", menubar_cb, mo_close_window,
                               NULL) DEFINE_MENUBAR("Exit Program...", "x",
                                                    menubar_cb, mo_exit_program, NULL) NULL_MENUBAR()
        /* Fonts Sub-Menu */
    ...
    DEFINE_MENUBAR("<Times Regular", "T", menubar_cb, mo_regular_fonts, NULL)
        ...
        SPACER()DEFINE_MENUBAR("<Helvetica Regular", "H", menubar_cb,
                               mo_regular_helvetica,
                               NULL) DEFINE_MENUBAR("<Helvetica Small", "e", menubar_cb, mo_small_helvetica, NULL)
        DEFINE_MENUBAR("<Helvetica Large", "v", menubar_cb,
                       mo_large_helvetica,
                       NULL) SPACER()DEFINE_MENUBAR("<New Century Regular",
                                                    "N", menubar_cb, mo_regular_newcentury, NULL)
        DEFINE_MENUBAR("<New Century Small", "w", menubar_cb,
                       mo_small_newcentury,
                       NULL) DEFINE_MENUBAR("<New Century Large", "C", menubar_cb, mo_large_newcentury, NULL)
        SPACER()DEFINE_MENUBAR("<Lucida Bright Regular", "L", menubar_cb,
                               mo_regular_lucidabright,
                               NULL) DEFINE_MENUBAR("<Lucida Bright Small",
                                                    "u", menubar_cb, mo_small_lucidabright, NULL)
        DEFINE_MENUBAR("<Lucida Bright Large", "i", menubar_cb, mo_large_lucidabright, NULL) NULL_MENUBAR()
        /* Underline Sub-Menu */
        ALLOC_MENUBAR(undr_menuspec, 6)
        DEFINE_MENUBAR("<Default Underlines", "D", menubar_cb,
                       mo_default_underlines,
                       NULL) DEFINE_MENUBAR("<Light Underlines", "L", menubar_cb, mo_l1_underlines, NULL)
        DEFINE_MENUBAR("<Medium Underlines", "M", menubar_cb,
                       mo_l2_underlines,
                       NULL) DEFINE_MENUBAR("<Heavy Underlines", "H", menubar_cb, mo_l3_underlines, NULL)
        DEFINE_MENUBAR("<No Underlines", "N", menubar_cb, mo_no_underlines, NULL) NULL_MENUBAR()
...
/*	DEFINE_MENUBAR("#Fancy Selections" ,"S",menubar_cb,mo_fancy_selections,NULL)
	SPACER()
        */
        DEFINE_MENUBAR("#Load to Local Disk", "T", menubar_cb,
                       mo_binary_transfer,
                       NULL) SPACER()DEFINE_MENUBAR("#Table Support", "b", menubar_cb, mo_table_support, NULL)
        SPACER()DEFINE_MENUBAR("#Body Color", "y", menubar_cb, mo_body_color, NULL)
        DEFINE_MENUBAR("#Body (Background) Images", "k", menubar_cb,
                       mo_body_images,
                       NULL) SPACER()DEFINE_MENUBAR("#View Images Internally",
                                                    "V", menubar_cb, mo_image_view_internal, NULL)
        DEFINE_MENUBAR("#Delay Image Loading", "D", menubar_cb,
                       mo_delay_image_loads,
                       NULL) DEFINE_MENUBAR("Load Images In Current", "L", menubar_cb, mo_expand_images_current, NULL)
        SPACER()DEFINE_MENUBAR("Reload Config Files", "R", menubar_cb, mo_re_init, NULL)
        SPACER()DEFINE_MENUBAR("Flush Image Cache", "I", menubar_cb,
                               mo_clear_image_cache,
                               NULL) DEFINE_MENUBAR("Flush Password Cache",
                                                    "P", menubar_cb, mo_clear_passwd_cache, NULL)
        DEFINE_MENUBAR("Clear Global History...", "C", menubar_cb,
                       mo_clear_global_history, NULL) SPACER()DEFINE_MENUBAR("Fonts", "F", NULL, 0, fnts_menuspec)
        DEFINE_MENUBAR("Anchor Underlines", "A", NULL, 0,
                       undr_menuspec) DEFINE_MENUBAR("Agent Spoofs", "g", NULL, 0, agent_menuspec)
        NULL_MENUBAR()
        /* Navigation Menu */
        ...
        SPACER()DEFINE_MENUBAR("Home Document", "D", menubar_cb,
                               mo_home_document,
                               NULL) DEFINE_MENUBAR("Window History...", "W", menubar_cb, mo_history_list, NULL)
        DEFINE_MENUBAR("Document Links...", "L", menubar_cb, mo_links_window,
                       NULL) SPACER()DEFINE_MENUBAR("Hotlist...", "H", menubar_cb, mo_hotlist_postit, NULL)
        DEFINE_MENUBAR("Add Current To Hotlist", "A", menubar_cb,
                       mo_register_node_in_default_hotlist,
                       NULL) DEFINE_MENUBAR("Add All Hotlist Entries to RBM",
                                            "E", menubar_cb, mo_all_hotlist_to_rbm, NULL);
    DEFINE_MENUBAR("Remove All Hotlist Entries from RBM", "R", menubar_cb, mo_all_hotlist_from_rbm, NULL);
    SPACER()DEFINE_MENUBAR("Internet Starting Points", "I", menubar_cb, mo_network_starting_points, NULL)
        DEFINE_MENUBAR("Internet Resource Meta-Index", "M", menubar_cb, mo_internet_metaindex, NULL) NULL_MENUBAR()
        /* Help Menu */
        ...
        SPACER()DEFINE_MENUBAR("What's New...", "W", menubar_cb, mo_whats_new,
                               NULL) DEFINE_MENUBAR("Demo...", "D", menubar_cb, mo_mosaic_demopage, NULL)
        SPACER()DEFINE_MENUBAR("Help on Version 2.7b5...", "V", menubar_cb,
                               mo_help_onversion,
                               NULL) DEFINE_MENUBAR("On Window...", "O", menubar_cb, mo_help_onwindow, NULL)
        DEFINE_MENUBAR("On FAQ...", "F", menubar_cb, mo_help_faq,
                       NULL) SPACER()DEFINE_MENUBAR("On HTML...", "H", menubar_cb, mo_help_html, NULL)
        DEFINE_MENUBAR("On URLS...", "U", menubar_cb, mo_help_url,
                       NULL) SPACER()DEFINE_MENUBAR("Mail Tech Support...", "M", menubar_cb, mo_techsupport, NULL)
#ifndef PRERELEASE
        SPACER()DEFINE_MENUBAR("Comment Card...", "C", menubar_cb, mo_cc, NULL)
#endif
        NULL_MENUBAR()
        /* Annotation Menu */
...
#endif
        SPACER()DEFINE_MENUBAR("Edit This Annotation...", "E", menubar_cb, mo_annotate_edit, NULL)
        DEFINE_MENUBAR("Delete This Annotation...", "D", menubar_cb, mo_annotate_delete, NULL) NULL_MENUBAR()
        ...
        SPACER()DEFINE_MENUBAR("Post", "o", menubar_cb, mo_news_post, NULL)
        DEFINE_MENUBAR("Followup", "F", menubar_cb, mo_news_follow, NULL)
        SPACER()DEFINE_MENUBAR("Subscribe to Group", "s", menubar_cb,
                               mo_news_sub,
                               NULL) DEFINE_MENUBAR("Unsubscribe Group", "u", menubar_cb, mo_news_unsub, NULL)
        SPACER()DEFINE_MENUBAR("<Show All Groups", "A", menubar_cb, mo_news_grp0, NULL)
        DEFINE_MENUBAR("<Show Unread Subscribed Groups", "S", menubar_cb,
                       mo_news_grp1,
                       NULL) DEFINE_MENUBAR("<Show All Subscribed Groups", "R", menubar_cb, mo_news_grp2, NULL)
        SPACER()DEFINE_MENUBAR("<Show All Articles", "l", menubar_cb, mo_news_art0, NULL)
        DEFINE_MENUBAR("<Show Only Unread Articles", "n", menubar_cb,
                       mo_news_art1,
                       NULL) SPACER()DEFINE_MENUBAR("Mark Group Read", "e", menubar_cb, mo_news_mread, NULL)
        DEFINE_MENUBAR("Mark Group Unread", "d", menubar_cb, mo_news_munread,
                       NULL) DEFINE_MENUBAR("Mark Article Unread", "M", menubar_cb, mo_news_maunread, NULL)
        SPACER()DEFINE_MENUBAR("Flush News Data", "F", menubar_cb,
                               mo_news_flush, NULL) DEFINE_MENUBAR("Thread Style", "T", NULL, 0, newsfmt_menuspec)
        NULL_MENUBAR()
        /* The Menubar */
        ...
        /* Dummy submenu. */
        NULL_MENUBAR()NULL_MENUBAR()
/* ----------------------- simple menubar interface ----------------------- */
        /* File Menu */
        ALLOC_MENUBAR(file_simple_menuspec, 7)
        DEFINE_MENUBAR("Clone", "C", menubar_cb, mo_clone_window, NULL)
        SPACER()DEFINE_MENUBAR("Find In Current", "I", menubar_cb, mo_search,
                               NULL) SPACER()DEFINE_MENUBAR("Close", "W", menubar_cb, mo_close_window, NULL)
        DEFINE_MENUBAR("Exit Program...", "x", menubar_cb, mo_exit_program, NULL) NULL_MENUBAR()
        /* Options Menu */
        ALLOC_MENUBAR(opts_simple_menuspec, 2)
        DEFINE_MENUBAR("#Load to Local Disk", "T", menubar_cb, mo_binary_transfer, NULL) NULL_MENUBAR()
        /* Navigation Menu */
        ALLOC_MENUBAR(navi_simple_menuspec, 12)
        DEFINE_MENUBAR("Back", "B", menubar_cb, mo_back, NULL)
        DEFINE_MENUBAR("Forward", "F", menubar_cb, mo_forward, NULL)
        SPACER()DEFINE_MENUBAR("Home Document", "D", menubar_cb, mo_home_document, NULL)
        SPACER()DEFINE_MENUBAR("Window History...", "W", menubar_cb,
                               mo_history_list,
                               NULL) SPACER()DEFINE_MENUBAR("Hotlist...", "H", menubar_cb, mo_hotlist_postit, NULL)
        DEFINE_MENUBAR("Add Current To Hotlist", "A", menubar_cb,
                       mo_register_node_in_default_hotlist,
...
/*    goto screwed_no_file;*/
    ...
    /* Make the menu. */
    menu = (XmxMenubarStruct *) malloc(MAX_DOCUMENTS_MENU_ITEMS * sizeof(XmxMenubarStruct));
    count = 0;
...
XmxMenuRecord *mo_make_document_view_menubar(Widget form)
{
    ...
    toBeReturned = XmxRMakeMenubar(form, get_pref_boolean(eSIMPLE_INTERFACE) ? simple_menuspec : menuspec);
