...
static XmxCallback(links_win_cb)
{
    ...
    case 0:                    /* GOTO */
        if (XmListGetSelectedPos(win->links_list, &posns, &pcount)) {
            if (pcount && XmStringGetLtoR(win->links_items[posns[0] - 1], XmSTRING_DEFAULT_CHARSET, &text)) {
                if (strncmp(text, "===", 3))
    ...
    case 1:                    /* DISMISS */
        ...
        break;
    case 2:                    /* HELP */
        mo_open_another_window(win, mo_assemble_help_url("help-on-links.html"), NULL, NULL);
        break;
    }
...
static void links_list_cb(Widget w, XtPointer client, XtPointer call)
{
    ...
    if (XmStringGetLtoR(win->links_items[cs->item_position - 1], XmSTRING_DEFAULT_CHARSET, &text)) {
        if (strncmp(text, "===", 3))
            mo_access_document(win, text);
        XtFree(text);
    }
...
mo_status mo_post_links_window(mo_window *win)
{
    ...
    if (!win->links_win) {
        ...
        Xmx_n = 0;
        win->links_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Document Links");
        ...
        buttons_form = XmxMakeFormAndFourButtons
            (links_form, links_win_cb, "Goto URL", "Save", "Dismiss", "Help...", 0, 3, 1, 2);
        ...
        label = XtVaCreateManagedWidget("Document Links & Images ...",
                                        ...
                                        XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 2, NULL);

        scroller = XtVaCreateWidget("scroller", xmScrolledWindowWidgetClass, links_form, XmNheight, 100,
                                    /* form attachments */
                                    XmNleftAttachment, XmATTACH_FORM,
                                    XmNrightAttachment, XmATTACH_FORM,
                                    XmNtopAttachment, XmATTACH_WIDGET,
                                    XmNtopWidget, label,
                                    XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, dialog_sep,
                                    /* offsets */
                                    XmNtopOffset, 10, XmNbottomOffset, 10, XmNleftOffset, 8, XmNrightOffset, 8, NULL);

        list = XtVaCreateManagedWidget("list", xmListWidgetClass,
                                       scroller,
                                       XmNvisibleItemCount, 10,
                                       XmNresizable, False,
                                       XmNscrollBarDisplayPolicy, XmSTATIC, XmNlistSizePolicy, XmCONSTANT, NULL);
...
mo_status mo_update_links_window(mo_window *win)
{
    ...
    if (!count) {
        XtVaSetValues(win->links_list, XmNitemCount, 0, NULL);
    } else {
        if (hrefs)
            count++;
        if (imgs)
        ...
        if (hrefs) {
            xmstr[p++] = XmStringCreateLtoR("=== Links ===", XmSTRING_DEFAULT_CHARSET);
            for (i = 0; i < hcount; i++, p++) {
                xmstr[p] = XmStringCreateLtoR(hrefs[i], XmSTRING_DEFAULT_CHARSET);
                free(hrefs[i]);
            }
            free(hrefs);
        }
        if (imgs) {
            xmstr[p++] = XmStringCreateLtoR("=== Images ===", XmSTRING_DEFAULT_CHARSET);
            for (i = 0; i < icount; i++, p++) {
                xmstr[p] = XmStringCreateLtoR(imgs[i], XmSTRING_DEFAULT_CHARSET);
                free(imgs[i]);
            }
            free(imgs);
        }
        XtVaSetValues(win->links_list, XmNitems, xmstr, XmNitemCount, count, NULL);
    }
...
/* tech support */
    ...
    { "yingxian",
     "<title>Not sure...</title><p>She's our new tech support manager...but we're not sure what her website is..." },
/* weird guy */
...
/* help */
    { "cci",
     "0http://www.ncsa.uiuc.edu/SDG/Software/XMosaic/CCI/cci-spec.html" },
