...
mo_status mo_add_node_to_history(mo_window *win, mo_node *node)
{
    ...
    if (win->history_list) {
        XmString xmstr = XmxMakeXmstrFromString(get_pref_boolean(eDISPLAY_URLS_NOT_TITLES) ? node->url : node->title);
...
/*              bcopy (foo1, server, (foo2 - foo1));*/
    ...
    if (!strncmp(url, "wais:", 5) ||
        ...
        !strncmp(url, "http://info.cern.ch.:8001/", 26) || !strncmp(url, "http://www.ncsa.uiuc.edu:8001/", 30)) {
        /* It's a WAIS server. */
        /* Do we have a ref? */
        if (ref) {
            title = strdup(ref);
...
mo_status mo_dump_history(mo_window *win)
{
    ...
    if (srcTrace) {
        ...
        fprintf(stderr, "CURRENT NODE %d %s\n", win->current_node->position, win->current_node->url);
        fprintf(stderr, "----------------- history -------------- \n");
...
static XmxCallback(mailhist_win_cb)
{
    ...
    case 1:
        ...
        break;
    case 2:
        mo_open_another_window(win, mo_assemble_help_url("help-on-nested-hotlists.html"), NULL, NULL);
        break;
    }
...
static mo_status mo_post_mailhist_win(mo_window *win)
{
    ...
    if (!win->mailhist_win) {
        ...
        XmxSetUniqid(win->id);
        win->mailhist_win = XmxMakeFormDialog(win->history_win, "NCSA Mosaic: Mail Window History");
        ...
        XmxSetConstraints
            (win->mailhist_to_text, XmATTACH_FORM, XmATTACH_NONE,
        ...
        XmxSetConstraints
            (subj_label, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
             XmATTACH_NONE, win->mailhist_to_text, NULL, NULL, NULL);
        XmxSetOffsets(win->mailhist_subj_text, 10, 0, 5, 10);
        XmxSetConstraints
            (win->mailhist_subj_text, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, win->mailhist_to_text, NULL, subj_label, NULL);

        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, win->mailhist_subj_text, buttons_form, NULL, NULL);
        XmxSetConstraints
...
static XmxCallback(history_win_cb)
{
    ...
    case 1:
        ...
        break;
    case 2:
        mo_open_another_window(win, mo_assemble_help_url("docview-menubar-navigate.html#history"), NULL, NULL);
        break;
    }
...
mo_status mo_post_history_win(mo_window *win)
{
    if (!win->history_win) {
        ...
        static char listTranslations[] = "~Shift ~Ctrl ~Meta ~Alt <Btn2Down>: ListKbdSelectAll() ListBeginSelect() \n\
	 ~Shift ~Ctrl ~Meta ~Alt <Btn2Up>:   ListEndSelect() ListKbdActivate()";
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->history_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Window History");
        ...
        win->history_list = XmxMakeScrolledList(history_form, history_list_cb, 0);
        ...
        buttons_form = XmxMakeFormAndThreeButtons(history_form,
                                                  history_win_cb, "Mail To...", "Dismiss", "Help...", 1, 0, 2);

        /* Constraints for history_form. */
        XmxSetOffsets(history_label, 8, 0, 10, 10);
        XmxSetConstraints
            (history_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
        /* History list is stretchable. */
        XmxSetOffsets(XtParent(win->history_list), 0, 10, 10, 10);
        XmxSetConstraints
            (XtParent(win->history_list),
             XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, history_label, dialog_sep, NULL, NULL);
        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
