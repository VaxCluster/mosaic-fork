...
int close_connection;
{
    ...
    if (status) {
        ...
        while (sendForm) {
            if ((sendForm->client == client) && (!strcmp(sendForm->actionID, actionID))) {
                sendForm->valid = 1;
                registered = 1;
            }
...
int MoCCITerminateAllConnections()
{
    ...
    while (con) {
        ...
        while (sendOutput) {    /* remove sendOutputs */
            HTRemoveConversion(sendOutput->contentType, "www/present", CCIPresent);
...
MCCIPort client;
{
    ...
    while (con) {
        if (con->client == client) {
            ...
            ListDeleteEntry(listOfConnections, con);
            con = (struct Connection *)ListCurrent(listOfConnections);
        } else {
...
static XmxCallback(MoCCIWindowCallBack)
{
    ...
    case 1:                    /*Cancel */
        ...
        break;
    case 2:                    /*Help */
        mo_open_another_window(win, mo_assemble_help_url("docview-menubar-file.html#open"), NULL, NULL);
        ...
        if (cciAccepting) {
            ...
            if ((newPort < 1024) || (newPort > 65535)) {
                XmxMakeErrorDialog(win->cci_win,
                                   "CCI port address must be between 1024 and 65535", "CCI port address error");
                XtManageChild(Xmx_w);
                cciAccepting = False;
                listenPortNumber = 0;
                XmxSetToggleButton(win->cci_accept_toggle, cciAccepting);
            ...
            if (MoCCIInitialize(newPort)) {
                ...
                connectInputID = XtAppAddInput(app_context, MCCIReturnListenPortSocketDescriptor(), (XtPointer)
                                               XtInputReadMask, (XtInputCallbackProc)
                                               MoCCINewConnection, app_context);
...
/*Widget buttonsForm;*/
    ...
    if (!win->cci_win) {
        XmxSetUniqid(win->id);
        win->cci_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Common Client Interface");
        dialogFrame = XmxMakeFrame(win->cci_win, XmxShadowOut);
        XmxSetConstraints
            (dialogFrame, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
        /* Main form. */
        ...
        XmxAddCallbackToText(win->cci_win_text, MoCCIWindowCallBack, 0);

        toggleBox = XmxMakeRadioBox(cciForm);

        win->cci_accept_toggle = XmxMakeToggleButton(toggleBox, "Accept requests", NULL, 0);

        win->cci_off_toggle = XmxMakeToggleButton(toggleBox, "Interface off", NULL, 0);

        dialogSeparator = XmxMakeHorizontalSeparator(cciForm);
        buttonBox = XmxMakeFormAndThreeButtons(cciForm, MoCCIWindowCallBack, "Ok", "Dismiss", "Help...", 0, 1, 2);
        ...
        XmxSetConstraints
            (win->cci_win_text, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, label, NULL);

        XmxSetConstraints(toggleBox, XmATTACH_WIDGET,
                          XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_NONE, win->cci_win_text, NULL, label, NULL);
        XmxSetOffsets(toggleBox, 8, 0, 2, 0);

        XmxSetConstraints(dialogSeparator, XmATTACH_WIDGET,
                          XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, toggleBox, NULL, NULL, NULL);

        XmxSetConstraints(buttonBox, XmATTACH_WIDGET,
                          XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, dialogSeparator, NULL, NULL, NULL);
    }
...
char *fileName;                 /* data stored here */
{
    ...
    while (sendOutput) {
        if (!strcmp(sendOutput->contentType, contentType)) {
            ...
            MCCISendOutputFile(sendOutput->client, contentType, fileName);
        }
...
int status;
{
    ...
    if (send) {
        ...
        while (sendForm) {
            ...
            {
                if (sendForm->valid)
                    MCCIFormQueryToClient(sendForm->client, actionID, query, contentType, post_data);
                found = 1;
            }
            sendForm = (struct FormSubmit *)ListNext(listOfForm);
        }
...
void MoCCISendOutput(MCCIPort client, Boolean sendIt, char *contentType)
{
    ...
    if (sendIt) {
        ...
        HTSetConversion(contentType, "www/present", CCIPresent, 1.0, 0.0, 0.0);
...
*/
        ...
        sendOutput = (struct SendWhatToWhom *)ListHead(listOfSendOutput);
        while (sendOutput) {
            if ((sendOutput->client == client) && (!strcmp(sendOutput->contentType, contentType))) {
                HTRemoveConversion(contentType, "www/present", CCIPresent);
                ListDeleteEntry(listOfSendOutput, sendOutput);
                FREE(sendOutput->contentType);
                FREE(sendOutput);
...
int dataLength;
{
    ...
    while (client) {
        MCCISendBrowserViewOutput(client, url, contentType, data, dataLength);
...
char *fileName;
{
    ...
    while (fileURL) {
        if (!strncmp(fileURL->fileName, fileName, strlen(fileURL->fileName))) {
            return (fileURL->url);
        }
