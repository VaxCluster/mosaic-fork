...
char *slab_words[] = { "MENU", "TITLE", "URL", "TOOLS", "STATUS", "VIEW", "GLOBE",
    "SMALLGLOBE", "TEXTTOOLS", NULL
};
...
extern Pixmap toolbarBack, toolbarForward, toolbarHome, toolbarReload,
    ...
    toolbarNewsFRevGRAY, toolbarNewsIndexGRAY, toolbarFTPput, toolbarFTPmkdir;

extern Pixmap securityKerberos4, securityBasic, securityMd5, securityNone,
    securityUnknown, securityKerberos5, securityDomain, securityLogin, enc_not_secure;

extern char *HTDescribeURL(char *);
extern mo_status mo_post_access_document(mo_window * win, char *url, char *content_type, char *post_data);
...
char *mo_assemble_help_url(char *file)
{
    ...
    tmp = (char *)malloc((strlen(file) + strlen(docs_directory) + 4) * sizeof(char));
...
*/
    ...
    XAllocNamedColor(dsp, (installed_colormap ?
                           installed_cmap : DefaultColormapOfScreen(XtScreen(bob))), "black", &ccell1, &ccell_fg);
    XAllocNamedColor(dsp, (installed_colormap ?
                           installed_cmap : DefaultColormapOfScreen(XtScreen(bob))), "white", &ccell1, &ccell_bg);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_1_bits, busy_1_width, busy_1_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_2_bits, busy_2_width, busy_2_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_3_bits, busy_3_width, busy_3_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_4_bits, busy_4_width, busy_4_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_5_bits, busy_5_width, busy_5_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_6_bits, busy_6_width, busy_6_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_7_bits, busy_7_width, busy_7_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_8_bits, busy_8_width, busy_8_height, 1, 0, 1);
    ...
    pmap = XCreatePixmapFromBitmapData
        (XtDisplay(bob), DefaultRootWindow(XtDisplay(bob)), busy_9_bits, busy_9_width, busy_9_height, 1, 0, 1);
...
int animateCursor()
{
    ...
    while (win = mo_next_window(win)) {
        ...
        if (win->history_win) {
            XDefineCursor(dsp, XtWindow(win->history_win), busyCursor[cursorAnimCnt]);
        }
        if (win->hotlist_win) {
            XDefineCursor(dsp, XtWindow(win->hotlist_win), busyCursor[cursorAnimCnt]);
        }
        if (win->searchindex_win) {
            XDefineCursor(dsp, XtWindow(win->searchindex_win), busyCursor[cursorAnimCnt]);
        }
    }
...
mo_status mo_set_dtm_menubar_functions(mo_window *win)
{
    if (mo_dtm_out_active_p()) {
        ...
        XmxRSetSensitive(win->menubar, mo_dtm_open_outport, XmxNotSensitive);
        XmxRSetSensitive(win->menubar, mo_dtm_send_document, XmxSensitive);
    } else {
        /* If we don't have an active outport, then we can't send a document
           but we can open an outport. */
        XmxRSetSensitive(win->menubar, mo_dtm_open_outport, XmxSensitive);
        XmxRSetSensitive(win->menubar, mo_dtm_send_document, XmxNotSensitive);
    }
...
/*  char *access;*/
    ...
    mo_window *win = mo_fetch_window_by_id(XmxExtractUniqid((int)client_data));
    XButtonReleasedEvent *event = (XButtonReleasedEvent *) (((WbAnchorCallbackData *) call_data)->event);
    ...
    if (get_pref_boolean(eKIOSK) || get_pref_boolean(eKIOSKNOEXIT) || get_pref_boolean(eDISABLEMIDDLEBUTTON)) {
    ...
    if (referer != NULL) {
     ...
     /*SWP*/ if (!my_strncasecmp(win->current_node->url, "http://", 7)) {
    ...
    if (!force_newwin)
        ...
        char *url = mo_url_canonicalize_keep_anchor(href, win->current_node->url);
        /* @@@ should we be keeping the anchor here??? */
        if (strncmp(url, "telnet:", 7) && strncmp(url, "tn3270:", 7) && strncmp(url, "rlogin:", 7)) {
            /* Not a telnet anchor. */
...
XmxCallback(submit_form_callback)
{
  ...
  foundit:
    ...
    if ((my_strcasecmp(method, "POST") == 0) || (my_strcasecmp(method, "cciPOST") == 0))
    ...
    if (cbdata->attribute_count == 1 && strcmp(cbdata->attribute_names[0], "isindex") == 0) {
    ...
    if (do_post_urlencoded) {
        ...
        mo_post_access_document(win, url, (plaintext ? "text/plain" : "application/x-www-form-urlencoded"), query);
    } else {
...
void mo_gui_done_with_icon(void)
{
    ...
    XtAppAddTimeOut(app_context, 10, (XtTimerCallbackProc) ungrab_the_____ing_pointer, NULL);
}
...
/*  char url[128]; /* buffer for news io */
    ...
    _count = XLookupString(&(event->xkey), _buffer, _bufsize, &_key, &_cs);
    ...
    if (!(get_pref_boolean(eKIOSK) || get_pref_boolean(eKIOSKNOEXIT))) {
        ...
        case XK_f:
            if (event->xkey.state & ControlMask) {
                if (XtIsManaged(win->slab[SLAB_URL]) && !get_pref_boolean(eFOCUS_FOLLOWS_MOUSE)) {
...
/* Not active */
        ...
        case XK_z:
            if (XtIsManaged(win->slab[SLAB_URL]) && !get_pref_boolean(eFOCUS_FOLLOWS_MOUSE)) {
                ...
                XmTextFieldSetSelection(win->url_text, 0, strlen(str), event->xkey.time);
...
static void DrawMeter(Widget meter, XtPointer client, XtPointer call)
{
    ...
    XFillRectangle(XtDisplay(meter), XtWindow(meter), gc, 0, 0, win->meter_width, win->meter_height);

    XSetForeground(XtDisplay(meter), gc, win->meter_fg);
    XFillRectangle(XtDisplay(meter), XtWindow(meter), gc, 0, 0, current_len, win->meter_height);
    ...
    if (!win->meter_notext && (win->meter_level <= 100 || win->meter_text)) {
        ...
        if (!win->meter_font) {
            XtVaGetValues(win->scrolled_win, WbNmeterFont, &(win->meter_font), NULL);
            if (!win->meter_font) {
/*
...
*/
            ...
            XDrawString(XtDisplay(meter), XtWindow(meter), gc,
                        (win->meter_width / 2 -
                         (win->meter_fontW * l) / 2) + 2, ((win->meter_height / 2) + (win->meter_fontH / 2)), ss, l);
            XSetForeground(XtDisplay(meter), gc, win->meter_font_fg);
            XDrawString(XtDisplay(meter), XtWindow(meter), gc,
                        (win->meter_width / 2 -
                         (win->meter_fontW * l) / 2), ((win->meter_height / 2) + (win->meter_fontH / 2)) - 2, ss, l);
        }
    }
        ...
        if (!*p || (*p == ',')) {
            *p = 0;
            ...
            for (j = 0; slab_words[j]; j++) {
                if ((strlen(slab_words[j]) == strlen(s)) && !strcmp(slab_words[j], s)) {
                    if (j == SLAB_TEXTTOOLS) {
                        j = SLAB_TOOLS;
                        stexttools = 1;
                    }
...
struct tool mo_tools[] = {
    { "<-", "Back", "Previous page", mo_back, &toolbarBack, &toolbarBackGRAY,
     moMODE_ALL, 1, NULL },
    { "->", "Forward", "Next page", mo_forward, &toolbarForward,
     &toolbarForwardGRAY, moMODE_ALL, 1, NULL },
    { "Rel", "Reload", "Reload this page", mo_reload_document, &toolbarReload,
     NULL, moMODE_ALL, 0, NULL },
    { "Home", "Home", "Go Home!", mo_home_document, &toolbarHome, NULL,
     moMODE_ALL, 1, NULL },
    { "Open", "Open", "Open a new URL", mo_open_document, &toolbarOpen, NULL,
     moMODE_ALL, 0, NULL },
    { "Save", "Save", "Save current page as ...", mo_save_document,
     &toolbarSave, NULL, moMODE_ALL, 0, NULL },
    { "New", "New", "Create a new Mosaic window", mo_new_window, &toolbarNew,
     NULL, moMODE_ALL, 0, NULL },
    ...
    { "+ Hot", "Add To Hotlist", "Add current page to hotlist",
     mo_register_node_in_default_hotlist, &toolbarAddHotlist, NULL,
     moMODE_PLAIN, 0, NULL },
    { "Find", "Find", "Search this document", mo_search, &toolbarSearch, NULL,
     moMODE_ALL, 0, NULL },
    { "Prt", "Print", "Print this document", mo_print_document, &toolbarPrint,
     NULL, moMODE_ALL, 0, NULL },
    { "Grps", "Groups", "Newsgroups index", mo_news_groups, &toolbarNewsGroups,
     NULL, moMODE_ALL, 0, NULL },
/* News Mode */
    ...
    { "Art>", "Article >", "Go to next article", mo_news_next, &toolbarNewsFwd,
     &toolbarNewsFwdGRAY, moMODE_NEWS, 1, NULL },
    { "Thr>", "Thread >", "Go to next thread", mo_news_nextt, &toolbarNewsFFwd,
     &toolbarNewsFFwdGRAY, moMODE_NEWS, 1, NULL },
    { "Post", "Post", "Post a UseNet Article", mo_news_post, &toolbarPost,
     &toolbarPostGRAY, moMODE_NEWS, 1, NULL },
    { "Foll", "Followup", "Follow-up to UseNet Article", mo_news_follow,
     &toolbarFollow, &toolbarFollowGRAY, moMODE_NEWS, 1, NULL },
/* FTP Mode */
    { "Put", "Put", "Send file to remote host", mo_ftp_put, &toolbarFTPput,
     NULL, moMODE_FTP, 1, NULL },
...
void mo_get_tools_from_res()
{
    ...
    if (get_pref_boolean(eKIOSK)) {
        ...
        ptr =
            "BACK,FORWARD,RELOAD,HOME,OPEN,SAVE,CLONE,CLOSE,FIND,PRINT,GROUPS,INDEX,PREVIOUS_THREAD,PREVIOUS_ARTICLE,NEXT_ARTICLE,NEXT_THREAD,POST,FOLLOW_UP,PUT,MKDIR";
    }
    tools = strdup(ptr);
...
/*             topLevelShellWidgetClass,*/
                                               ...
                                               win->base,
                                               XmNminHeight, h =
                                               win->toolbarorientation ? 640 :
                                               40, XmNminWidth, w =
                                               win->toolbarorientation ? 40 :
                                               640, XmNmaxHeight, h,
                                               XmNmaxWidth, w, XmNheight, h,
                                               XmNwidth, w, XmNallowShellResize, FALSE, NULL);
        XtManageChild(win->toolbarwin);
        win->topform = XtVaCreateWidget
            ("slab_tools",
             xmFormWidgetClass, win->toolbarwin,
             XmNminHeight, h, XmNminWidth, w, XmNmaxHeight, h, XmNmaxWidth, w, XmNheight, h, XmNwidth, w, NULL);
        mo_fill_toolbar(win, win->topform, 0, 0);
        XtManageChild(win->topform);
        WM_DELETE_WINDOW = XmInternAtom(dsp, "WM_DELETE_WINDOW", False);
        XmAddWMProtocolCallback(win->toolbarwin, WM_DELETE_WINDOW, mo_tool_detach_cb, (XtPointer) win);
...
{
    win->security = XmxMakeNamedPushButton(top, NULL, "sec", security_pressed_cb, 0);
    ...
    XtVaSetValues(win->security,
                  ...
                  XmNmarginLeft, 0,
                  XmNmarginRight, 0,
                  XmNuserData, (XtPointer) "Security Stats Information", XmNtraversalOn, False, NULL);
    ...
    win->encrypt = XtVaCreateManagedWidget
        (" ", xmPushButtonWidgetClass,
         ...
         XmNmarginLeft, 0,
         XmNmarginRight, 0,
         XmNuserData, (XtPointer) "Encryption Status (not in this release)", XmNtraversalOn, False, NULL);
...
void mo_make_globe(mo_window *win, Widget parent, int small)
{
    ...
    win->logo = XmxMakeNamedPushButton(parent, NULL, "logo", icon_pressed_cb, 0);
    XmxApplyPixmapToLabelWidget(win->logo, IconPix[0]);
    XtVaSetValues(win->logo,
                  ...
                  XmNleftAttachment, !win->biglogo || (!win->smalllogo
                                                       && win->toolbardetached) ?
                  XmATTACH_FORM : XmATTACH_NONE, XmNrightAttachment, XmATTACH_FORM, XmNtraversalOn, False, NULL);
    ...
    if (win->biglogo) {
        if (win->smalllogo) {
            ...
            XtVaSetValues(win->security,
                          XmNtopAttachment, XmATTACH_FORM,
                          XmNbottomAttachment, XmATTACH_FORM,
                          XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_NONE, NULL);
            XtVaSetValues(win->encrypt,
                          XmNtopAttachment, XmATTACH_FORM,
                          XmNbottomAttachment, XmATTACH_FORM,
                          XmNleftAttachment, XmATTACH_WIDGET,
                          XmNleftWidget, win->security,
                          XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget, win->logo, NULL);
        } else {
            if (!win->toolbardetached) {
                mo_extra_buttons(win, win->slab[SLAB_GLOBE]);
                XtVaSetValues(win->security,
    ...
    XtVaSetValues(win->logo, XmNuserData, (XtPointer) "Logo Button - Abort a Transaction", NULL);
...
Widget mo_fill_toolbar(mo_window *win, Widget top, int w, int h)
{
    ...
    if (!tmpFont) {
        XtVaGetValues(win->scrolled_win, WbNtoolbarFont, &tmpFont, NULL);
        if (!tmpFont) {
            fprintf(stderr,
    ...
    win->button2_rc = XtVaCreateWidget
        ("buttonrc2", xmRowColumnWidgetClass,
         ...
         XmNrightAttachment, XmATTACH_FORM, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, NULL);

    win->button_rc = XtVaCreateWidget
        ("buttonrc", xmRowColumnWidgetClass, win->topform, XmNorientation, vert ? XmVERTICAL : XmHORIZONTAL,
...
*/
         ...
         XmNrightWidget, win->button2_rc, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, NULL);
    ...
    tearbutton = XtVaCreateManagedWidget
        ("|", xmPushButtonWidgetClass,
         ...
         XmNlabelPixmap, vert ? tearh : tearv, XmNfontList, tmpFontList, XmNtraversalOn, False, NULL);

    XtOverrideTranslations(tearbutton, XtParseTranslationTable(xlattab));

    XtAddCallback(tearbutton, XmNactivateCallback, mo_tool_detach_cb, (XtPointer) win);

    for (i = 0; mo_tools[i].label; i++) {
        if (mo_tools[i].action > 0) {
            if (use_tool[i]) {
                win->tools[i].w = XtVaCreateManagedWidget
                    ((long_text ? mo_tools[i].long_text : mo_tools[i].text), xmPushButtonWidgetClass, win->button_rc,
                     XmNuserData, (XtPointer) mo_tools[i].label,
                     XmNmarginWidth, 0, XmNmarginHeight, 0, XmNmarginTop, 0, XmNmarginBottom, 0,
/*
...
*/
                     ...
                     XmNlabelType, textbuttons ? XmSTRING : XmPIXMAP,
                     XmNlabelPixmap, *(mo_tools[i].image), XmNfontList, tmpFontList, XmNtraversalOn, False, NULL);

                XtOverrideTranslations(win->tools[i].w, XtParseTranslationTable(xlattab));
                if (mo_tools[i].greyimage != NULL)
                    XtVaSetValues(win->tools[i].w, XmNlabelInsensitivePixmap, *(mo_tools[i].greyimage), NULL);
                XmxSetSensitive(win->tools[i].w, win->tools[i].gray);
                XmxAddCallback(win->tools[i].w, XmNactivateCallback, menubar_cb, mo_tools[i].action);
            ...
            XtVaCreateManagedWidget
                (" ", xmSeparatorWidgetClass,
                 ...
                 vert ? XmNheight : XmNwidth, vert ? 3 : 4, XmNseparatorType, XmNO_LINE, XmNtraversalOn, False, NULL);
        }
    }
...
static mo_status mo_fill_window(mo_window *win)
{
    ...
    win->toolbarwin = NULL;
      ...
      /*********************** SLAB_GLOBE ****************************/
    if (win->biglogo) {
        win->slab[SLAB_GLOBE] = XtVaCreateWidget("slab_globe", xmFormWidgetClass, form, NULL);
    ...
    XtUnmanageChild(win->slab[SLAB_MENU]);
      ...
      /*********************** SLAB_TITLE ****************************/
    win->slab[SLAB_TITLE] = XtVaCreateWidget("slab_title", xmFormWidgetClass, form, XmNheight, 36, NULL);
    ...
    win->title_text = XtVaCreateManagedWidget("title", xmTextFieldWidgetClass,
                                              ...
                                              title_label, XmNrightAttachment,
                                              XmATTACH_FORM, XmNtopAttachment,
                                              XmATTACH_FORM,
                                              XmNbottomAttachment,
                                              XmATTACH_NONE, XmNeditable, False, XmNcursorPositionVisible, False, NULL);

      /*********************** SLAB_URL ****************************/
    win->slab[SLAB_URL] = XtVaCreateWidget("slab_url", xmFormWidgetClass, form, XmNheight, 36, NULL);
    ...
    win->url_text = XtVaCreateManagedWidget("text", xmTextFieldWidgetClass,
                                            ...
                                            XmNleftAttachment,
                                            XmATTACH_WIDGET, XmNleftWidget,
                                            url_label, XmNrightAttachment,
                                            XmATTACH_FORM, XmNtopAttachment,
                                            XmATTACH_FORM,
                                            XmNbottomAttachment,
                                            XmATTACH_NONE,
                                            XmNcursorPositionVisible, True,
                                            XmNeditable, True, XmNtraversalOn, False, NULL);
    ...
    XtOverrideTranslations(win->url_text, XtParseTranslationTable(text_translations));
    XtOverrideTranslations(win->title_text, XtParseTranslationTable(text_translations));
    XtOverrideTranslations(win->url_text, XtParseTranslationTable(url_translations));
    ...
    win->slab[SLAB_VIEW] = win->scrolled_win = XtVaCreateManagedWidget
        ("view", htmlWidgetClass, form,
         ...
         WbNdelayImageLoads, win->delay_image_loads ? True : False, XmNshadowThickness, 2, NULL);
    ...
    XmxAddCallback(win->scrolled_win, WbNsubmitFormCallback, submit_form_callback, 0);
    ...
    mo_make_popup(win->view);
      ...
      /*********************** SLAB_STATUS ****************************/
    win->slab[SLAB_STATUS] = XtVaCreateWidget("slab_status", xmFormWidgetClass, form, NULL);
    ...
    if (get_pref_boolean(eMETER)) {
        ...
        XtVaSetValues(win->meter_frame, XmNrightOffset, 3, XmNtopOffset, 2,
                      XmNbottomOffset, 2, XmNleftAttachment, XmATTACH_NONE,
                      XmNrightAttachment, XmATTACH_FORM, XmNtopAttachment,
                      XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, NULL);

        win->meter = XtVaCreateManagedWidget
            ("meter", xmDrawingAreaWidgetClass,
             win->meter_frame, XmNuserData, (XtPointer) "Progress Meter", XmNheight, 16, XmNwidth, 96, NULL);
        ...
        /* grab some colors */
        {
            ...
            XAllocNamedColor(dsp, (installed_colormap ?
                                   ...
                                   DefaultColormapOfScreen(XtScreen
                                                           (win->base))),
                             get_pref_string(eMETER_FOREGROUND), &ccell1, &ccell2);
            win->meter_fg = ccell2.pixel;
            XAllocNamedColor(dsp, (installed_colormap ?
                                   installed_cmap :
                                   DefaultColormapOfScreen(XtScreen
                                                           (win->base))),
                             get_pref_string(eMETER_BACKGROUND), &ccell1, &ccell2);
            win->meter_bg = ccell2.pixel;
            XAllocNamedColor(dsp, (installed_colormap ?
                                   installed_cmap :
                                   DefaultColormapOfScreen(XtScreen
                                                           (win->base))),
                             get_pref_string(eMETER_FONT_FOREGROUND), &ccell1, &ccell2);
            win->meter_font_fg = ccell2.pixel;
            XAllocNamedColor(dsp, (installed_colormap ?
                                   installed_cmap :
                                   DefaultColormapOfScreen(XtScreen
                                                           (win->base))),
                             get_pref_string(eMETER_FONT_BACKGROUND), &ccell1, &ccell2);
    ...
    win->tracker_label = XtVaCreateManagedWidget
        (" ", xmLabelWidgetClass,
         ...
         XmNleftAttachment, XmATTACH_FORM,
         XmNrightAttachment,
         get_pref_boolean(eMETER) ? XmATTACH_WIDGET : XmATTACH_NONE,
         XmNrightWidget, get_pref_boolean(eMETER) ? win->meter_frame : NULL,
         XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_NONE, NULL);

      /*********************** SLAB_TOOLS ****************************/
    win->slab[SLAB_TOOLS] = XtVaCreateWidget("slab_tools", xmFormWidgetClass, form, NULL);
...
/* next two slabs have to attach to the globe */
            ...
            if (linkup) {
                XtVaSetValues
                    (win->slab[SLAB_GLOBE],
                     ...
                     XmNtopAttachment,
                     i ? XmATTACH_WIDGET : XmATTACH_FORM, XmNtopWidget,
                     i ? win->slab[win->slabpart[i - 1]] : NULL, XmNbottomAttachment, XmATTACH_NONE, NULL);
            } else {
                XtVaSetValues
                    (win->slab[SLAB_GLOBE],
                     XmNleftAttachment, XmATTACH_NONE,
                     ...
                     XmNbottomAttachment,
                     i + globe + 1 ==
                     win->slabcount ? XmATTACH_FORM : XmATTACH_WIDGET,
                     XmNbottomWidget,
                     i + globe + 1 ==
                     win->slabcount ? NULL : win->slab[win->slabpart[i +
                                                                     globe +
                                                                     1]], XmNtopAttachment, XmATTACH_NONE, NULL);
            }
            ...
            XtVaSetValues(win->slab[win->slabpart[i]],
                          ...
                          XmNleftAttachment, XmATTACH_FORM,
                          XmNrightAttachment,
                          globe ? XmATTACH_WIDGET : XmATTACH_FORM, XmNrightWidget, win->slab[SLAB_GLOBE], NULL);
            if (globe)
                globe--;
        }
    }
...
void mo_sync_windows(mo_window *win, mo_window *parent)
{
    ...
    XmxRSetToggleState(win->menubar, mo_image_view_internal, (win->image_view_internal ? XmxSet : XmxNotSet));

    tableSupportEnabled = win->table_support = parent->table_support;
    XmxRSetToggleState(win->menubar, mo_table_support, win->table_support ? XmxSet : XmxNotSet);

    win->body_color = parent->body_color;
    XtVaSetValues(win->scrolled_win, WbNbodyColors, win->body_color, NULL);
    XmxRSetToggleState(win->menubar, mo_body_color, win->body_color ? XmxSet : XmxNotSet);

    win->body_images = parent->body_images;
    XtVaSetValues(win->scrolled_win, WbNbodyImages, win->body_images, NULL);
    XmxRSetToggleState(win->menubar, mo_body_images, win->body_images ? XmxSet : XmxNotSet);
    ...
    XmxRSetSensitive(win->menubar, mo_expand_images_current, win->delay_image_loads ? XmxSensitive : XmxNotSensitive);
    XmxRSetToggleState(win->menubar, mo_delay_image_loads, win->delay_image_loads ? XmxSet : XmxNotSet);
}
...
*/
    ...
    win->mail_win = win->mailhot_win = win->edithot_win = win->mailhist_win = win->inserthot_win = 0;
    win->print_win = 0;
    ...
    win->save_format = 0;
    if (!parent) {
        win->font_size = mo_get_font_size_from_res(get_pref_string(eDEFAULT_FONT_CHOICE), &(win->font_family));
...
/* Install all the GUI bits & pieces. */
    ...
    win->binary_transfer = 0;
    XmxRSetToggleState(win->menubar, mo_binary_transfer, (win->binary_transfer ? XmxSet : XmxNotSet));
    XmxRSetToggleState(win->menubar, mo_delay_image_loads, (win->delay_image_loads ? XmxSet : XmxNotSet));
    XmxRSetSensitive(win->menubar, mo_expand_images_current, win->delay_image_loads ? XmxSensitive : XmxNotSensitive);
    ...
    if (get_pref_boolean(eSESSION_HISTORY_ON_RBM)) {
        ...
        if (get_pref_boolean(eSESSION_HISTORY_ON_RBM))
            win->session_items = malloc(sizeof(Widget) * get_pref_int(eNUMBER_OF_ITEMS_IN_RBM_HISTORY));
    }
...
static mo_window *mo_make_window(Widget parent, mo_window *parentw)
{
    ...
    base = XtCreatePopupShell("shell", topLevelShellWidgetClass, toplevel, Xmx_wargs, Xmx_n);
    Xmx_n = 0;

#ifdef EDITRES_SUPPORT
    XtAddEventHandler(base, (EventMask) 0, TRUE, (XtEventHandler) _XEditResCheckMessages, NULL);
#endif
...
static mo_window *mo_open_another_window_internal(mo_window *win)
{
    ...
    XtVaGetValues(win->base, XmNx, &oldx, XmNy, &oldy, XmNwidth, &width, XmNheight, &height, NULL);
 ...
 *   
 ****************************************************************************/
mo_window *mo_open_another_window(mo_window *win, char *url, char *ref, char *target_anchor)
{
    mo_window *neww;
...
/* set the geometry values - dxp */
    ...
    win = mo_open_window(toplevel, startup_document ? startup_document : init_document, NULL);
    ...
    XtVaGetValues(win->scrolled_win, WbNbodyColors, &(win->body_color), NULL);

    XtVaGetValues(win->scrolled_win, WbNbodyImages, &(win->body_images), NULL);
...
mo_status mo_open_initial_window(void)
{
    ...
    XtAppAddTimeOut(app_context, 10, (XtTimerCallbackProc) fire_er_up, (XtPointer) True);
...
static int mo_error_handler(Display *dsp, XErrorEvent *event)
{
    ...
    if (event->error_code == BadAlloc || (event->error_code == BadAccess && event->request_code == 88))
        return 0;
    else {
        {
            char ans[10];
...
void setup_imagekill(void)
{
    ...
    if (!(fp = fopen(imageselect_file_pathname, "r"))) {
        ...
        fprintf(stderr, "Error: Can't open .mosaic-imageselect-sites file for reading\n");
        return;
    }
...
void mo_do_gui(int argc, char **argv)
{
    ...
    if (no_defaults) {
        toplevel = XtAppInitialize
            (&app_context, "Mosaic", options, XtNumber(options), &argc, argv, NULL, Xmx_wargs, Xmx_n);
    } else {
        if (use_color) {
            toplevel = XtAppInitialize
                (&app_context, "Mosaic", options, XtNumber(options), &argc, argv, color_resources, Xmx_wargs, Xmx_n);
        } else {
            toplevel = XtAppInitialize
                (&app_context, "Mosaic", options, XtNumber(options), &argc, argv, mono_resources, Xmx_wargs, Xmx_n);
        }
    }
    ...
    XtVaGetApplicationResources(toplevel, (XtPointer) thePrefsStructP->RdataP, resources, XtNumber(resources), NULL);
...
*/
    ...
    if (installed_colormap) {
        ...
        installed_cmap = XCreateColormap(dsp,
                                         RootWindow(dsp,
                                                    DefaultScreen(dsp)),
                                         DefaultVisual(dsp, DefaultScreen(dsp)), AllocNone);
  ...
  splash_goto:
    ...
    if (get_pref_boolean(eSPLASHSCREEN)) {
        ...
        if (!(font = XLoadQueryFont(dsp, "-adobe-helvetica-medium-o-normal-*-*-180-*-*-p-*-iso8859-*"))) {
        ...
        splash = XtVaCreatePopupShell
            ("Hello, World!",
             ...
             toplevel, XmNheight, 320, XmNwidth, 320, XmNx, x, XmNy, y, XmNallowShellResize, FALSE, NULL);
        ...
        sform = XtVaCreateManagedWidget("sform", xmRowColumnWidgetClass,
                                        splash, XmNheight, 320, XmNwidth, 320, XmNx, x, XmNy, y, NULL);
        ...
        if (!XAllocColor(dsp, (installed_colormap ?
                               installed_cmap : DefaultColormapOfScreen(XtScreen(splash))), &ccell_fg))
        ...
        XDrawString(dsp, splashpix, gc, 320 - (fontW * l / 2), 320 - fontH / 2, s, l);

        spixwid = XtVaCreateManagedWidget(" ", xmLabelWidgetClass,
                                          sform,
                                          XmNlabelType, XmPIXMAP,
                                          XmNlabelPixmap, splashpix,
                                          XmNalignment, XmALIGNMENT_CENTER, XmNx, x, XmNy, y, NULL);
    ...
    if (get_pref_boolean(eHTTPTRACE) ||
        ...
        get_pref_boolean(eSRCTRACE) || get_pref_boolean(eCACHETRACE) || get_pref_boolean(eNUTTRACE)) {
    ...
    machine_with_domain = (strlen(machine) > strlen(shortmachine) ? machine : shortmachine);
    ...
    XtVaGetValues(toplevel, XmNwidth, &userWidth, XmNheight, &userHeight, XmNx, &userX, XmNy, &userY, NULL);
    ...
    if (get_pref_boolean(eSPLASHSCREEN) && splash) {
        ...
        if (splash_cc) {
            splashTimer = XtAppAddTimeOut(app_context, 3000, (XtTimerCallbackProc) kill_splash, NULL);
        } else {
            kill_splash();
        }
    }
