...
void URL_Include_Set(mo_window *win, int togval, int sensitive)
{
    ...
    XtVaSetValues(win->hotlist_rbm_toggle, XmNsensitive, sensitive, NULL);
...
/* Given a hotlist and a hotnode, rip the hotnode out of the hotlist.
   ...
   it better be. */
static void mo_remove_hotnode_from_hotlist(mo_hotlist *list, mo_hot_item *hotnode)
{
...
static void mo_insert_item_in_hotlist(mo_hotlist *list, mo_hot_item *node, int position)
{
    if (!position) {
...
static mo_hotlist *mo_copy_hot_hier(mo_hotlist *list)
{
    ...
    for (item = list->nodelist; item; item = item->any.next)
        if (item->type == mo_t_url) {
            ...
            mo_append_item_to_hotlist(hotlist, (mo_hot_item *) mo_copy_hot_hier((mo_hotlist *) item));
        }
    return hotlist;
}
    ...
    while (win = mo_next_window(win))
        if (win->hotlist_list && win->current_hotlist == list) {
            ...
            XmString xmstr =
                ...
                (item->type == mo_t_url ?
                 (get_pref_boolean(eDISPLAY_URLS_NOT_TITLES) ? item->hot.url : item->hot.title) : (highlight =
                                                                                                   mo_highlight_hotlist
                                                                                                   (&item->list)));
...
mo_status mo_add_item_to_hotlist(mo_hotlist *list, mo_item_type type, char *title, char *url, int position, int rbm)
{
    mo_hot_item *item;
    mo_window *win = NULL;
...
static void mo_load_hotlist_list(mo_window *win, Widget list)
{
    ...
    for (node = win->current_hotlist->nodelist; node != NULL; node = node->any.next) {
        ...
        XmString xmstr =
            XmxMakeXmstrFromString
            (node->type == mo_t_url ?
             (get_pref_boolean(eDISPLAY_URLS_NOT_TITLES) ? node->hot.url : node->hot.title) : (highlight =
                                                                                               mo_highlight_hotlist
                                                                                               (&node->list)));
...
static XmxCallback(edit_or_insert_hot_cb)
{
...

        {
            ...
            while (w = mo_next_window(w)) {
                if (w->hotlist_list && w->current_hotlist == win->current_hotlist) {
                    ...
                    XmString xmstr =
                        ...
                        (hotnode->type == mo_t_url ?
                         (get_pref_boolean(eDISPLAY_URLS_NOT_TITLES) ?
                          hotnode->hot.url : title) : (highlight = mo_highlight_hotlist(&hotnode->list)));
                    ...
                    XmListAddItemUnselected(w->hotlist_list, xmstr, hotnode->any.position);
                    XmStringFree(xmstr);
                }
    ...
    case 3:                    /* Commit Insert */
        ...
        title = XmxTextGetString(eht_info->title_text);
        {
            ...
            if (useIns)
                if (XmListGetSelectedPos(win->hotlist_list, &pos_list, &pos_cnt) && pos_cnt) {
                    posi = pos_list[0];
            ...
            if (isUrl)
                addOk = mo_add_item_to_hotlist
                    (win->current_hotlist, mo_t_url, title,
                     XmxTextGetString(eht_info->url_text), posi, get_pref_boolean(eADD_HOTLIST_ADDS_RBM));
            else {
                if (win->hot_cut_buffer &&
                    win->hot_cut_buffer->type == mo_t_list &&
                    (!strcmp(title, win->hot_cut_buffer->any.name)) &&
                    (!mo_is_ancestor((mo_hotlist *) win->hot_cut_buffer, win->current_hotlist))) {
                    mo_insert_item_in_hotlist
                        (win->current_hotlist,
                         (mo_hot_item *) mo_copy_hot_hier((mo_hotlist *) win->hot_cut_buffer), posi);
                    /* Now we've got to update all active hotlist_list's. */
                    mo_gui_add_hot_item(win->current_hotlist, win->current_hotlist->nodelist_last);
                } else
                    addOk =
                        mo_add_item_to_hotlist(win->current_hotlist,
                                               mo_t_list, title, NULL, posi, get_pref_boolean(eADD_HOTLIST_ADDS_RBM));
            }
            if (addOk == mo_succeed)
                mo_write_default_hotlist();
        }
    ...
    case 2:                    /* Help... (Edit) */
    case 5:                    /* Help... (Insert) */
        mo_open_another_window(win, mo_assemble_help_url("help-on-hotlist-view.html"), NULL, NULL);
        break;
    }
...
static mo_status mo_create_ed_or_ins_hot_win(mo_window *win, int isInsert)
{
    ...
    ed_or_ins_w = XmxMakeFormDialog
        (win->hotlist_win, isInsert ? "NCSA Mosaic: Insert Hotlist Entry" : "NCSA Mosaic: Edit Hotlist Entry");
    ...
    if (isInsert) {
        ...
        eht_info->tog_url =
            XtVaCreateManagedWidget("toggle", xmToggleButtonGadgetClass,
                                    togm, XtVaTypedArg, XmNlabelString,
                                    XtRString, "URL", strlen("URL") + 1, XmNmarginHeight, 0, XmNset, True, NULL);
        XtAddCallback(eht_info->tog_url, XmNvalueChangedCallback, url_or_list_cb, (XtPointer) eht_info);
        eht_info->tog_list =
            XtVaCreateManagedWidget("toggle", xmToggleButtonGadgetClass,
                                    togm, XtVaTypedArg, XmNlabelString,
                                    XtRString, "List", strlen("List") + 1, XmNmarginHeight, 0, NULL);
        togm2 = XmxMakeRadioBox(eht_form);
        eht_info->insert_tog = insert_tog =
            XtVaCreateManagedWidget("toggle", xmToggleButtonGadgetClass,
                                    togm2, XtVaTypedArg, XmNlabelString,
                                    XtRString, "Insert", strlen("Insert") + 1, XmNmarginHeight, 0, NULL);
        append_tog =
            XtVaCreateManagedWidget("toggle", xmToggleButtonGadgetClass,
                                    togm2, XtVaTypedArg, XmNlabelString,
                                    XtRString, "Append", strlen("Append") + 1, XmNmarginHeight, 0, XmNset, True, NULL);
        sep2 = XmxMakeHorizontalSeparator(eht_form);
    }

    buttons_form = XmxMakeFormAndThreeButtons
        (eht_form, edit_or_insert_hot_cb, "Save",
         "Dismiss", "Help...", isInsert * 3, isInsert * 3 + 1, isInsert * 3 + 2);

    /* Constraints for eht_form. */
    XmxSetOffsets(title_label, 14, 0, 10, 0);
    XmxSetConstraints(title_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    ...
    XmxSetConstraints
        (url_label, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, title_label, NULL, NULL, NULL);
    XmxSetOffsets(url_val, 8, 10, 5, 10);
    XmxSetConstraints
        (url_val, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, title_label, NULL, url_label, NULL);

    XmxSetArg(XmNtopOffset, 10);
    XmxSetConstraints
        (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
         XmATTACH_FORM, url_val, isInsert ? togm : buttons_form, NULL, NULL);
    if (isInsert) {
        XmxSetConstraints(togm, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_NONE, NULL, sep2, NULL, NULL);
        XmxSetPositions(togm, XmxNoPosition, XmxNoPosition, XmxNoPosition, 50);
        XmxSetConstraints(togm2, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, NULL, sep2, NULL, NULL);
        XmxSetPositions(togm2, XmxNoPosition, XmxNoPosition, 50, XmxNoPosition);
        XmxSetConstraints
            (sep2, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, NULL, buttons_form, NULL, NULL);
    }
    XmxSetConstraints(buttons_form, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);

    return mo_succeed;
}

static mo_status mo_do_edit_hotnode_title_win(mo_window *win, mo_hot_item *item, int position)
{
    edit_or_insert_hot_info *eht_info;
 ...
 * Return status.
 */
static mo_status mo_edit_title_in_current_hotlist(mo_window *win, int position)
{
    mo_hotlist *list = win->current_hotlist;
    mo_hot_item *hotnode;
    ...
    return ((hotnode != NULL) ? mo_do_edit_hotnode_title_win(win, hotnode, position) : mo_fail);
}
...
static mo_status mo_rbm_toggle_in_hotlist(mo_window *win, int position)
{
    ...
    XmxSetToggleButton(win->hotlist_rbm_toggle, (hotnode->type == mo_t_url ? hotnode->hot.rbm : hotnode->list.rbm));
...
static void mo_insert_item_in_current_hotlist(mo_window *win)
{
    if (win->hotlist_win) {
        ...
        if (win->hot_cut_buffer) {
            /* Insert this title as a starting point. */
            XmxTextSetString(eht_info->title_text, win->hot_cut_buffer->any.name);
            if (win->hot_cut_buffer->type == mo_t_url) {
                /* Insert URL */
                XmxTextSetString(eht_info->url_text, win->hot_cut_buffer->hot.url);
                XtManageChild(eht_info->url_lab);
                XtManageChild(eht_info->url_text);
...
static mo_root_hotlist *mo_read_hotlist(char *filename, char *home)
{
    ...
    if (!strncmp(line, NCSA_HOTLIST_FORMAT_COOKIE_ONE, strlen(NCSA_HOTLIST_FORMAT_COOKIE_ONE))) {
        isnew = 0;
    } else {
        status = fgets(line, MO_LINE_LENGTH, fp);
        if (!status || !(*line)) {
        ...
        if (!strncmp(line, NCSA_HOTLIST_FORMAT_COOKIE_TWO, strlen(NCSA_HOTLIST_FORMAT_COOKIE_TWO))) {
            isnew = 1;
        } else if (!strncmp(line, NCSA_HOTLIST_FORMAT_COOKIE_THREE, strlen(NCSA_HOTLIST_FORMAT_COOKIE_THREE))) {
            isnew = 2;
        } else {
            fprintf(stderr,
...
/* New hotlist format... SWP */
        free(filename);
        filename = (char *)malloc((strlen(home) + strlen(hot_filename) + 8) * sizeof(char));
 ...
 * Return status.
 */
static void delete_hot_from_list(mo_hotlist *list, mo_hot_item *hotnode, int position)
{
    mo_window *win = NULL;
    ...
    if (hotnode->type == mo_t_list)
        while (win = mo_next_window(win)) {
            if (win->hotlist_list && mo_is_ancestor(&(hotnode->list), win->current_hotlist)) {
...
static mo_status mo_delete_position_from_current_hotlist(mo_window *win, int position)
{
    mo_hotlist *list = win->current_hotlist;
    mo_hot_item *hotnode;
    ...
    if (hotnode->type == mo_t_list) {
        ...
        buff = (char *)malloc(strlen(question) + strlen(hotnode->list.name) + strlen(endquestion) + 1);

        strcat(strcat(strcpy(buff, question), hotnode->list.name), endquestion);
        XmxSetUniqid(win->id);
        XmxMakeQuestionDialog(win->hotlist_win, buff, "NCSA Mosaic: Remove list", remove_confirm_cb, position, 0);
        free(buff);
        free(question);
        free(endquestion);
        XtManageChild(Xmx_w);
...
static mo_status mo_post_mailhot_win(mo_window *win)
{
    ...
    if (!win->mailhot_win) {
        ...
        XmxSetUniqid(win->id);
        win->mailhot_win = XmxMakeFormDialog(win->hotlist_win, "NCSA Mosaic: Mail Hotlist");
        ...
        buttons_form = XmxMakeFormAndThreeButtons(mailhot_form, mailhot_win_cb, "Mail", "Dismiss", "Help...", 0, 1, 2);
        ...
        XmxSetConstraints
            (win->mailhot_to_text, XmATTACH_FORM, XmATTACH_NONE,
        ...
        XmxSetConstraints
            (subj_label, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
             XmATTACH_NONE, win->mailhot_to_text, NULL, NULL, NULL);
        XmxSetOffsets(win->mailhot_subj_text, 10, 0, 5, 10);
        XmxSetConstraints
            (win->mailhot_subj_text, XmATTACH_WIDGET,
             XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, win->mailhot_to_text, NULL, subj_label, NULL);

        XmxSetArg(XmNtopOffset, 10);
        XmxSetConstraints
            (dialog_sep, XmATTACH_WIDGET, XmATTACH_WIDGET, XmATTACH_FORM,
             XmATTACH_FORM, win->mailhot_subj_text, buttons_form, NULL, NULL);
        XmxSetConstraints
...
static XmxCallback(hotlist_rbm_toggle_cb)
{
    ...
    if (rv && pos_cnt) {
        ...
        URL_Include_Set(win, (hotnode->type == mo_t_url ? hotnode->hot.rbm : hotnode->list.rbm), 1);
    } else {
        XmxMakeErrorDialog
            (win->hotlist_win,
    ...
    case 10:
        ...
        if (!win->save_hotlist_win)
            win->save_hotlist_win = XmxMakeFileSBDialog
                (win->hotlist_win, "NCSA Mosaic: Save Current hotlist", "Name for saved hotlist", save_hot_cb, 0);
        else
    ...
    case 11:
        ...
        if (!win->load_hotlist_win) {
            ...
            win->load_hotlist_win = XmxMakeFileSBDialog
                (win->hotlist_win, "NCSA Mosaic: Load in Current hotlist", "Name of file to open", load_hot_cb, 0);
            ...
            tb = XtVaCreateManagedWidget("toggle",
                                         xmToggleButtonGadgetClass,
                                         workarea, XtVaTypedArg,
                                         XmNlabelString, XtRString,
                                         "Create new hotlist",
                                         strlen("Create new hotlist") + 1, XmNmarginHeight, 0, XmNset, True, NULL);
            XmxSetArg(XmNuserData, (XtArgVal) tb);
            ...
            XtVaCreateManagedWidget("toggle", xmToggleButtonGadgetClass,
                                    ...
                                    strlen("Load in current hotlist") + 1, XmNmarginHeight, 0, NULL);
        } else
...
mo_status mo_post_hotlist_win(mo_window *win)
{
    if (!win->hotlist_win) {
        ...
        static char listTranslations[] = "~Shift ~Ctrl ~Meta ~Alt <Btn2Down>: ListBeginSelect() \n\
	 ...
	 ~Shift ~Ctrl ~Meta ~Alt <Btn2Up>:  ListBeginSelect() ListEndSelect()";
        ...
        win->hotlist_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Hotlist View");
        ...
        buttons1_form = XmxMakeFormAndFourButtons
            (hotlist_form, hotlist_win_cb, "Add Current", "Goto URL", "Remove", "Edit", 3, 4, 5, 6);
        buttons2_form = XmxMakeFormAndThreeButtons(hotlist_form, hotlist_win_cb, "Copy", "Insert", "Up", 7, 8, 9);
        ...
        XtAddCallback(win->hotlist_list, XmNbrowseSelectionCallback, hotlist_rbm_toggle_cb, 0);

        win->hotlist_rbm_toggle = XmxMakeToggleButton(hotlist_form, "Include On Right Button Menu", hotlist_win_cb, 12);
        URL_Include_Set(win, 0, 0);
        ...
        buttons_form = XmxMakeFormAndFiveButtons(hotlist_form,
                                                 hotlist_win_cb,
                                                 "Mail To...", "Save", "Load", "Dismiss", "Help...", 1, 10, 11, 0, 2);
        ...
        XmxSetConstraints
            (win->hotlist_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, NULL, NULL, NULL, NULL);
        XmxSetOffsets(buttons1_form, 0, 0, 0, 0);
        XmxSetConstraints
            (buttons1_form,
             XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, win->hotlist_label, NULL, NULL, NULL);
        XmxSetOffsets(buttons2_form, 0, 2, 0, 0);
        XmxSetConstraints
            (buttons2_form,
             XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, buttons1_form, NULL, NULL, NULL);
        ...
        XmxSetConstraints
            (win->hotlist_rbm_toggle,
             XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_NONE, NULL, dialog_sep, NULL, NULL);
        XmxSetConstraints
...
mo_status mo_add_node_to_current_hotlist(mo_window *win)
{
    ...
    return mo_add_item_to_hotlist(win->current_hotlist, mo_t_url,
                                  ...
                                  win->current_node->url, 0, get_pref_boolean(eADD_HOTLIST_ADDS_RBM));
}
