...
mo_status mo_send_mailto_message(char *text, char *to, char *subj, char *content_type, char *url);
...
static XmxCallback(include_fsb_cb)
{
    ...
    while (1) {
        ...
        XmTextInsert(win->mailto_text, pos = XmTextGetInsertionPosition(win->mailto_text), line);
...
static XmxCallback(mailto_win_cb)
{
    ...
    case 1:                    /* dismiss */
        ...
        break;
    case 2:                    /* help */
        mo_open_another_window(win, mo_assemble_help_url("help-on-mailto.html"), NULL, NULL);
        break;
    ...
    case 3:                    /* insert file */
        if (!win->mail_fsb_win) {
            win->mail_fsb_win = XmxMakeFileSBDialog
                (win->mailto_win,
                 "NCSA Mosaic: Include File In Mail Message", "Name of file to include:", include_fsb_cb, 0);
        } else {
    ...
    case 4:
        if (win->current_node->url) {
            XmTextInsert(win->mailto_text, pos = XmTextGetInsertionPosition(win->mailto_text), win->current_node->url);
            /* move insertion position to past this line
               to avoid inserting the lines in reverse order */
            XmTextSetInsertionPosition(win->mailto_text, pos + strlen(win->current_node->url));
        }
        break;
    }
...
static XmxCallback(mailto_form_win_cb)
{
    ...
    case 1:                    /* dismiss */
        ...
        break;
    case 2:                    /* help */
        mo_open_another_window(win, mo_assemble_help_url("help-on-mailto-form.html"), NULL, NULL);
        break;
    }
...
mo_status mo_post_mailto_win(char *to_address, char *subject)
{
    ...
    if (!win->mailto_win) {
        ...
        XmxSetUniqid(win->id);
        win->mailto_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Mail To Author");
        ...
        XmxSetConstraints
            (fromlabel, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
        XmxSetOffsets(win->mailto_fromfield, 10, 10, 10, 10);
        XmxSetConstraints
            (win->mailto_fromfield, XmATTACH_FORM, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, fromlabel, NULL);

        /* constraints for TO */
        XmxSetOffsets(tolabel, 14, 10, 10, 10);
        XmxSetConstraints
            (tolabel, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
             XmATTACH_NONE, win->mailto_fromfield, NULL, NULL, NULL);
        XmxSetOffsets(win->mailto_tofield, 10, 10, 10, 10);
        XmxSetConstraints
            (win->mailto_tofield, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, win->mailto_fromfield, NULL, tolabel, NULL);
        ...
        XmxSetConstraints
            (win->mailto_subfield, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, win->mailto_tofield, NULL, sublabel, NULL);

        /* create buttons */
        buttons_form = XmxMakeFormAndFiveButtons
            (mailto_form, mailto_win_cb, "Send", "Insert File", "Insert URL", "Dismiss", "Help...", 0, 3, 4, 1, 2);

        XmxSetOffsets(XtParent(win->mailto_text), 3, 0, 3, 3);
        XmxSetConstraints
            (XtParent(win->mailto_text), XmATTACH_WIDGET, XmATTACH_WIDGET,
             XmATTACH_FORM, XmATTACH_FORM, win->mailto_subfield, dialog_sep, NULL, NULL);
    ...
    sprintf(namestr, "%s <%s>", get_pref_string(eDEFAULT_AUTHOR_NAME), get_pref_string(eDEFAULT_AUTHOR_EMAIL));
    ...
    if (get_pref_string(eSIGNATURE)) {
        ...
        if ((fp = fopen(get_pref_string(eSIGNATURE), "r")) != NULL) {
            while (fgets(tmp, sizeof(tmp) - 1, fp)) {
                XmTextInsert(win->mailto_text, pos = XmTextGetInsertionPosition(win->mailto_text), tmp);
...
mo_status mo_post_mailto_form_win(char *to_address, char *subject)
{
    ...
    if (!win->mailto_form_win) {
        ...
        /* Create it for the first time. */
        XmxSetUniqid(win->id);
        win->mailto_form_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Mail Form Results To Author");
        ...
        XmxSetConstraints
            (tolabel, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM,
             XmATTACH_NONE, win->mailto_form_fromfield, NULL, NULL, NULL);
        XmxSetOffsets(win->mailto_form_tofield, 10, 10, 10, 10);
        XmxSetConstraints
            (win->mailto_form_tofield, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, win->mailto_form_fromfield, NULL, tolabel, NULL);
        ...
        XmxSetConstraints
            (win->mailto_form_subfield, XmATTACH_WIDGET, XmATTACH_NONE,
             XmATTACH_WIDGET, XmATTACH_FORM, win->mailto_form_tofield, NULL, sublabel, NULL);

        /* create buttons */
        buttons_form = XmxMakeFormAndThreeButtons
            (mailto_form_form, mailto_form_win_cb, "Send", "Dismiss", "Help...", 0, 1, 2);

        XmxSetOffsets(XtParent(win->mailto_form_text), 3, 0, 3, 3);
        XmxSetConstraints
            (XtParent(win->mailto_form_text), XmATTACH_WIDGET,
             XmATTACH_WIDGET, XmATTACH_FORM, XmATTACH_FORM, win->mailto_form_subfield, dialog_sep, NULL, NULL);
...
void do_mailto_post(mo_window *win, char *to, char *from, char *subject, char *body)
{
    ...
    if (!win || !win->current_node || !win->current_node->url ||
        !*(win->current_node->url) || !to || !*to || !from || !*from || !body || !*body || !subject || !*subject) {
        return;
    }
    ...
    if (!strcmp(post_content_type, "text/plain")) {
        ...
        mo_send_mailto_message(buf, to, subject, post_content_type, win->current_node->url);
        ...
        mo_send_mailto_message(body, to, subject, post_content_type, win->current_node->url);
    }
...
FILE *mo_start_sending_mailto_message(char *to, char *subj, char *content_type, char *url)
{
    char cmd[2048];
/*  char *tmp;*/
    ...
    if (get_pref_string(eMAIL_FILTER_COMMAND)) {
        sprintf(cmd, "%s | %s", get_pref_string(eMAIL_FILTER_COMMAND), get_pref_string(eSENDMAIL_COMMAND));
    } else {
