...
/* return a MCCIPort if connected */
{
    ...
    if (cciTrace) {
        fprintf(stderr,
                ...
                MoCCIMaxNumberOfConnectionsAllowed(), MoCCICurrentNumberOfConnections());
    }
#endif

    if (client && MoCCIMaxNumberOfConnectionsAllowed()) {
        /* if maxNumConnections == 0, then no limit */
        if ((MoCCICurrentNumberOfConnections() + 1) > MoCCIMaxNumberOfConnectionsAllowed()) {
            MCCISendResponseLine(client, MCCIR_MAX_CONNECTIONS, "Maximum number of allowed CCI connections exceeded");
...
/* SEND OUTPUT */
        ...
        if (start && (start != end)) {
            if (!my_strncasecmp(start, MCCI_S_STOP, strlen(MCCI_S_STOP))) {
                ...
                if (start && (start != end)) {
                    *end = '\0';
                    MCCIRequestSendOutput(&retCode, retText, client, 0, start);
                } else {
                    /* no output type... so all types */
                    MCCIRequestSendOutput(&retCode, retText, client, 0, (char *)0);
                }
            } else {
                /* SEND OUTPUT type */
                *end = '\0';
                MCCIRequestSendOutput(&retCode, retText, client, 1, start);
            }

        } else {
            /* "SEND OUTPUT" so send it all */
            MCCIRequestSendOutput(&retCode, retText, client, 1, (char *)0);
        }
    }
...
/* SEND BROWSERVIEW */
        ...
        if (start && (start != end)) {
            if (!my_strncasecmp(start, MCCI_S_STOP, strlen(MCCI_S_STOP))) {
                ...
                MCCIRequestSendBrowserView(&retCode, retText, client, 0);
            } else {
...
/* SEND EVENT */
        ...
        if (start && (start != end)) {
            if (!my_strncasecmp(start, MCCI_S_STOP, strlen(MCCI_S_STOP))) {
                /* SEND EVENT STOP */
                MCCIRequestSendEvent(&retCode, retText, client, 0);
            } else {
...
char *retText;                  /* text to be returned to cci client */
{
    ...
    *tmpend = '\0';             /* terminate the content-type */
    ...
    MCCIRequestPost(client, &retCode, retText, url, mimeType, postData, postDataLength, output);
...
char *retText;                  /* text to be returned to cci client */
{
    ...
    *tmpend = '\0';             /* terminate the content-type */
    ...
    MCCIRequestDisplay(client, &retCode, retText, url, mimeType, displayData, displayDataLength, output);
...
char *retText;                  /* text to be returned to cci client */
{
    ...
    *tmp = '\0';
    ...
    GetWordFromString(s, &next, &end);
    if (next && (next != end)) {
        if (!my_strncasecmp(next, MCCI_S_STOP, strlen(MCCI_S_STOP))) {
            status = 0;
        } else if (!my_strncasecmp(next, MCCI_S_START, strlen(MCCI_S_START))) {
            status = 1;
        } else {
            return (MCCIR_ERROR);
        }
...
static int annoCodes[NUM_ANNO_CODES] = { MCCI_PUBLIC_ANNOTATION, MCCI_GROUP_ANNOTATION,
    MCCI_PRIVATE_ANNOTATION
};
...
int *retDataLength;
{
    ...
    } else if (!my_strncasecmp(type, MCCI_S_GROUP_ANN, strlen(MCCI_S_GROUP_ANN))) {
        annotationType = MCCI_GROUP_ANNOTATION;
    } else if (!my_strncasecmp(type, MCCI_S_PRIVATE_ANN, strlen(MCCI_S_PRIVATE_ANN))) {
        annotationType = MCCI_PRIVATE_ANNOTATION;
    } else if (!my_strncasecmp(type, MCCI_S_ALL_ANN, strlen(MCCI_S_ALL_ANN))) {
    ...
    *end = '\0';                /* terminate url */
    ...
    for (int index = 0; index < NUM_ANNO_CODES; ++index) {
        if (!my_strncasecmp(type, annoStrings[index],
            ...
            MCCIRequestGetAnnotation(&retCode, retText, retData, retDataLength, url, annoCodes[index]);
        } else
...
#else                           /* NEW */
    MCCIRequestGetAnnotation(&retCode, retText, retData, retDataLength, url, annotationType);
...
char *retText;                  /* text to be returned to cci client */
{
    ...
    *end = '\0';                /* terminate url */
    ...
    MCCIRequestPutAnnotation(&retCode, retText, annotationType, url, annotation, annotationLength);
    return (retCode);
}
...
MCCIPort client;
{
    ...
    if (!my_strncasecmp(line, MCCI_S_DISCONNECT, strlen(MCCI_S_DISCONNECT))) {
        MCCISendResponseLine(client, MCCIR_DISCONNECT_OK, "DISCONNECT request received");
        return (0);
    }
/* This has to go ahead of the simple get or else it gets snagged */
    else if (!my_strncasecmp(line, MCCI_S_GETANNOTATION, strlen(MCCI_S_GETANNOTATION))) {
        retDataLength = 0;
 ...
 */
        retCode = MCCIHandleGetAnnotation(client, line, retText, retData, &retDataLength);
/*Original
...
*/
        ...
        MCCISendResponseLine(client, retCode, "DISPLAY request received by Mosaic");
    } else if (!my_strncasecmp(line, MCCI_S_FORM, strlen(MCCI_S_FORM))) {
        retCode = MCCIHandleForm(client, line, retText);
        MCCISendResponseLine(client, MCCIR_FORM_OK, "FORM request received by Mosaic");
    } else if (!my_strncasecmp(line, MCCI_S_QUIT, strlen(MCCI_S_QUIT))) {
        MCCISendResponseLine(client, MCCIR_QUIT_OK, "QUIT request received exiting...");
        MCCIRequestQuit();
        ...
        MCCISendResponseLine(client, retCode, retText);
    } else if (!my_strncasecmp(line, MCCI_S_PUTANNOTATION, strlen(MCCI_S_PUTANNOTATION))) {
        retCode = MCCIHandlePutAnnotation(client, line, retText);
        MCCISendResponseLine(client, retCode, retText);
    } else if (!my_strncasecmp(line, MCCI_S_FILE_TO_URL, strlen(MCCI_S_FILE_TO_URL))) {
         ...
         */
        MCCISendResponseLine(client, MCCIR_UNRECOGNIZED, "Command not recognized");
    }
