...
static void set_annotation_mode(mo_window *win, int mode)
{
    ...
    XmxSetSensitive(win->delete_button, mode == mo_edit_annotation ? XmxSensitive : XmxNotSensitive);
    return;
}
 ...
 * a new annotation.
 */
        if (win->pubpri == mo_annotation_public || win->pubpri == mo_annotation_workgroup) {
            ...
            if (txt && *txt) {
                if (win->annotation_mode == mo_new_annotation) {
                     ...
                     */
                    mo_new_grpan(mo_url_to_unique_document(win->current_node->url), title, author, txt);
                     ...
                     */
                    mo_modify_grpan(mo_url_to_unique_document(win->current_node->url), title, author, txt);
            ...
            if (txt && *txt) {
                if (win->annotation_mode == mo_new_annotation) {
                    mo_new_pan
                        (mo_url_to_unique_document
                         (win->current_node->url),
                         XmxTextGetString(win->annotate_title), XmxTextGetString(win->annotate_author), txt);
    ...
    case 1:
        ...
        break;
    case 2:
        mo_open_another_window(win, mo_assemble_help_url("help-on-annotate-win.html"), NULL, NULL);
        break;
    }
...
static void do_slate(mo_window *win)
{
    ...
    sprintf(namestr, "%s (%s)", get_pref_string(eDEFAULT_AUTHOR_EMAIL), get_pref_string(eDEFAULT_AUTHOR_NAME));

    XmxTextSetString(win->annotate_author, namestr);

    sprintf(namestr, "%s %s", "Annotation by", get_pref_string(eDEFAULT_AUTHOR_NAME));
...
static XmxCallback(include_fsb_cb)
{
    ...
    if (!fp) {
        ...
        XmxMakeErrorDialog(win->annotate_win, final, "Annotation Open Error");
        XtManageChild(Xmx_w);
    ...
    while (1) {
        ...
        XmTextInsert(win->annotate_text, pos = XmTextGetInsertionPosition(win->annotate_text), line);
...
static XmxCallback(include_button_cb)
{
    ...
    if (!win->include_fsb) {
        win->include_fsb = XmxMakeFileSBDialog
            (win->annotate_win,
             "NCSA Mosaic: Include File In Annotation", "Name of file to include:", include_fsb_cb, 0);
    } else {
...
static void make_annotate_win(mo_window *win)
{
    ...
    win->annotate_win = XmxMakeFormDialog(win->base, "NCSA Mosaic: Annotate Window");
    ...
    win->pubpri_menu = XmxRMakeOptionMenu(annotate_form, "", pubpri_opts_cb, pubpri_opts);
    ...
    {
        win->passwd_toggle = XmxMakeToggleButton(passwd_f, "Password Protection", passwd_toggle_cb, 0);
        ...
        XmxSetConstraints
            (win->passwd_toggle, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    ...
    XmxApplyBitmapToLabelWidget(logo, annotate_bits, annotate_width, annotate_height);

    sep = XmxMakeHorizontalSeparator(annotate_form);

    text_label = XmxMakeLabel(annotate_form, "Enter the annotation text:");

    clear_button = XmxMakePushButton(annotate_form, "Clean Slate", clear_button_cb, 0);
    include_button = XmxMakePushButton(annotate_form, "Include File...", include_button_cb, 0);
    win->delete_button = XmxMakePushButton(annotate_form, "Delete", delete_button_cb, 0);
    ...
    buttons_form = XmxMakeFormAndThreeButtonsSqueezed
        (annotate_form, annotate_win_cb, "Save", "Dismiss", "Help...", 0, 1, 2);
    ...
    XmxSetConstraints(author_label, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, NULL, NULL, NULL, NULL);
    ...
    XmxSetConstraints
        (win->annotate_author, XmATTACH_FORM, XmATTACH_NONE,
         XmATTACH_WIDGET, XmATTACH_WIDGET, NULL, NULL, author_label, logo);
    ...
    XmxSetConstraints
        (title_label, XmATTACH_WIDGET, XmATTACH_NONE,
         XmATTACH_FORM, XmATTACH_NONE, win->annotate_author, NULL, NULL, NULL);
    ...
    XmxSetConstraints
        (win->annotate_title, XmATTACH_WIDGET, XmATTACH_NONE,
         XmATTACH_WIDGET, XmATTACH_WIDGET, win->annotate_author, NULL, title_label, logo);
#ifdef GRPAN_PASSWD
    ...
    XmxSetConstraints
        (win->pubpri_menu->base, XmATTACH_WIDGET, XmATTACH_NONE,
         XmATTACH_FORM, XmATTACH_NONE, win->annotate_title, NULL, NULL, NULL);
#ifdef GRPAN_PASSWD
    ...
    XmxSetConstraints
        (passwd_frame, XmATTACH_WIDGET, XmATTACH_NONE,
         XmATTACH_WIDGET, XmATTACH_FORM, win->annotate_title, NULL, win->pubpri_menu->base, NULL);
    XmxSetOffsets(sep, 10, 0, 0, 0);
    XmxSetConstraints
        (sep, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, passwd_frame, NULL, NULL, NULL);
#else                           /* not GRPAN_PASSWD */
    XmxSetOffsets(sep, 10, 0, 0, 0);
    XmxSetConstraints
        (sep, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_FORM, win->pubpri_menu->base, NULL, NULL, NULL);
#endif                          /* not GRPAN_PASSWD */

    /* text_label: top sep, bottom nothing,
       left form, right nothing */
    XmxSetOffsets(text_label, 12, 0, 10, 10);
    XmxSetConstraints(text_label, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_FORM, XmATTACH_NONE, sep, NULL, NULL, NULL);

    /* Buttons */
    XmxSetOffsets(clear_button, 8, 0, 0, 3);
    XmxSetConstraints
        (clear_button, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_NONE, XmATTACH_WIDGET, sep, NULL, NULL, include_button);
    XmxSetOffsets(include_button, 8, 0, 0, 3);
    XmxSetConstraints
        (include_button, XmATTACH_WIDGET, XmATTACH_NONE,
         XmATTACH_NONE, XmATTACH_WIDGET, sep, NULL, NULL, win->delete_button);
    XmxSetOffsets(win->delete_button, 8, 0, 10, 10);
    XmxSetConstraints
        (win->delete_button, XmATTACH_WIDGET, XmATTACH_NONE, XmATTACH_NONE, XmATTACH_FORM, sep, NULL, NULL, NULL);

    /* logo: top form, bottom nothing,
       left nothing, right form. */
    XmxSetOffsets(logo, 5, 10, 10, 5);
    XmxSetConstraints(logo, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_NONE, XmATTACH_FORM, NULL, NULL, NULL, NULL);

    XmxSetOffsets(XtParent(win->annotate_text), 0, 2, 0, 0);
    XmxSetConstraints
        (XtParent(win->annotate_text), XmATTACH_WIDGET, XmATTACH_WIDGET,
         XmATTACH_FORM, XmATTACH_FORM, text_label, dialog_sep, NULL, NULL);
...
mo_status mo_post_annotate_win
    (mo_window * win, int edit_flag, int id, char *title, char *author, char *text, char *fname) {
    if (!win->current_node)
        return mo_fail;
