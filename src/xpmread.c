 ...
 */
void xpmFreeColorTable_(colorTable, ncolors)
char ***colorTable;
int ncolors;
{
    int a, b;
 ...
 */
void xpmInitInternAttrib(attrib)
 ...
 */
void xpmFreeInternAttrib(attrib)
xpmInternAttrib *attrib;
{
...
xpmData *mdata;
{
    ...
    if (!filename) {
        ...
        } else if ((strlen(filename) > 2) && !strcmp(".z", filename + (strlen(filename) - 2))) {
            mdata->type = XPMPIPE;
...
xpmData *mdata;
{
    if (mdata->type == XPMBUFFER) {
        ...
        *s = mdata->Bcmt[0];
        ...
        while (c == *s2 && *s2 != '\0' && c != EOF && c != mdata->Bos);
...
static int ParseValues(data, width, height, ncolors, cpp, x_hotspot, y_hotspot, hotspot, extensions)
xpmData *data;
...
static int ParseColors(data, ncolors, cpp, colorTablePtr, hashtable)
xpmData *data;
...
static int ParsePixels(data, width, height, ncolors, cpp, colorTable, hashtable, pixels)
xpmData *data;
unsigned int width;
unsigned int height;
unsigned int ncolors;
...
xpmData *mdata;
{
    ...
    if (mdata->type) {
        ...
        if ((l == 3 && !strncmp("XPM", buf, 3)) || (l == 4 && !strncmp("XPM2", buf, 4))) {
            if (l == 3)
                 ...
                 */
                while (xpmDataTypes[n].type && strncmp(xpmDataTypes[n].type, buf, l))
                    n++;
            }
...
XpmAttributes *attributes;
{
     ...
     * read values
     */
    ErrorStatus = ParseValues(data, &width, &height, &ncolors, &cpp, &x_hotspot, &y_hotspot, &hotspot, &extensions);
    if (ErrorStatus != XpmSuccess)
        return (ErrorStatus);
     ...
     */
    ErrorStatus = ParsePixels(data, width, height, ncolors, cpp, colorTable, &hashtable, &pixelindex);
...
unsigned char *ReadXpm3Pixmap(FILE *fp, char *datafile, int *w, int *h, XColor *colrs, int *bg)
{
    xpmData mdata;
    XpmAttributes attributes;
    xpmInternAttrib attrib;
    ...
    for (i = 0; i < Colors; i++) {
        ...
        if (strcmp(colorName, TRANSPARENT_COLOR) == 0) {
            ...
            XQueryColor(XtDisplay(view),
                        (installed_colormap ?
                         installed_cmap : DefaultColormap(XtDisplay(view), DefaultScreen(XtDisplay(view)))), &tmpcolr);
            *bg = i;
        } else {
            XParseColor(dsp,
                        (installed_colormap ?
                         installed_cmap : DefaultColormap(dsp, DefaultScreen(dsp))), colorName, &tmpcolr);
        }
...
unsigned char *ProcessXpm3Data(Widget wid, char **xpmdata, int *w, int *h, XColor *colrs, int *bg)
{
    xpmData mdata;
    XpmAttributes attributes;
    xpmInternAttrib attrib;
    ...
    for (i = 0; i < Colors; i++) {
        ...
        if (strcmp(colorName, TRANSPARENT_COLOR) == 0) {
            ...
            XQueryColor(XtDisplay(wid),
                        (installed_colormap ?
                         installed_cmap : DefaultColormap(XtDisplay(wid), DefaultScreen(XtDisplay(wid)))), &tmpcolr);
            *bg = i;
        } else {
            XParseColor(XtDisplay(wid),
                        (installed_colormap ?
                         installed_cmap :
                         DefaultColormap(XtDisplay(wid), DefaultScreen(XtDisplay(wid)))), colorName, &tmpcolr);
        }
        colrs[i].red = tmpcolr.red;
        colrs[i].green = tmpcolr.green;
        colrs[i].blue = tmpcolr.blue;
