...
unsigned char *ReadGIF(FILE *fd, int *w, int *h, XColor *colrs, int *bg)
{
    ...
    if (BitSet(buf[4], LOCALCOLORMAP)) {    /* Global Colormap */
        ...
        if (ReadColorMap(fd, GifScreen.BitPixel, GifScreen.ColorMap, &GifScreen.xGrayScale)) {
...
/*			    unsigned char *tdata;
			    ...
			    tdata =*/ ReadImage(fd,
                           ...
                           LM_to_uint(buf[6], buf[7]), colrs, bitPixel,
                           localColorMap, grayScale, BitSet(buf[8], INTERLACE), imageCount != imageNumber);
            }
        } else {
            /*
             * We only want to set the data for the
            ...
            if (imageCount == imageNumber) {
                image = ReadImage(fd, LM_to_uint(buf[4], buf[5]),
                                  ...
                                  GifScreen.BitPixel,
                                  GifScreen.ColorMap,
                                  GifScreen.xGrayScale, BitSet(buf[8], INTERLACE), imageCount != imageNumber);
            } else {
/*			    unsigned char *tdata;
...
static int ReadColorMap(FILE *fd, int number, unsigned char buffer[3][MAXCOLORMAPSIZE], int *gray)
{
    int i;
    unsigned char rgb[3];
    int flag;
...
static unsigned char *ReadImage(FILE *fd, int len, int height, XColor *colrs,
                                int cmapSize,
                                unsigned char cmap[3][MAXCOLORMAPSIZE], int gray, int interlace, int ignore)
{
    unsigned char *dp, c;
    int v;
