...
*/
...
#ifdef HAVE_HDF
    if (ishdf) {
        img_data = (ImageInfo *) hdfGrokImage(mo_hdf_fetch_local_filename(src), hdfref, &bg);
...
#endif                          /* HAVE_HDF */
    {
...
#ifndef DISABLE_TRACE
            if (srcTrace)
                fprintf(stderr, "[ImageResolve] Got through mo_pull_er_over_virgin, rc %d\n", rc);
#endif
...
#ifndef DISABLE_TRACE
        if (srcTrace)
            fprintf(stderr, "[ImageResolve] Did ReadBitmap, got 0x%08x\n", data);
#endif
        ...
        if ((bg >= 0) && (data != NULL) &&
            ...
            (get_pref_int(eMAX_CLIPPING_SIZE_Y) == (-1) || get_pref_int(eMAX_CLIPPING_SIZE_Y) >= height)) {
    ...
    for (i = 0; i < img_data->height; i++) {
        for (j = 0, bcnt = 0; j < img_data->width; j++) {
            ...
            if (bg >= 0) {
                ...
                if (img_data->transparent && ((bcnt % 8) == 7 || j == (img_data->width - 1))) {
                    cptr++;
                }
                bgptr++;
                bcnt++;
     ...
     */
    if ((cnt > get_pref_int(eCOLORS_PER_INLINED_IMAGE))
        && (Vclass != TrueColor)) {
        MedianCut(img_data->image_data, &img_data->width,
                  &img_data->height, colrs, 256, get_pref_int(eCOLORS_PER_INLINED_IMAGE));
