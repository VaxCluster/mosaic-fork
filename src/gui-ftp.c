...
mo_status mo_handle_ftpput(mo_window *win)
{
    ...
    if ((strlen(win->current_node->url) > 4)
        && strncmp("ftp:", win->current_node->url, 4) == 0) {
...
static XmxCallback(ftpput_win_cb)
{
    ...
    XtVaGetValues(fsbList, XmNselectedItems, &selected_items, XmNselectedItemCount, &count, NULL);
    ...
    if (count) {
        ...
        XtVaGetValues(win->ftpput_win, XmNdirSpec, &st, NULL);
...
mo_status mo_post_ftpput_window(mo_window *win)
{
    ...
    if (!win->ftpput_win) {
        ...
        fsbList = XmFileSelectionBoxGetChild(win->ftpput_win, XmDIALOG_LIST);
        XtVaSetValues(fsbList, XmNselectionPolicy, XmEXTENDED_SELECT, NULL);
    } else {
...
mo_status mo_post_ftpmkdir_window(mo_window *win)
{
    if (!win->ftpmkdir_win) {
        ...
        buttons_form = XmxMakeFormAndFourButtons
            (form, ftpmkdir_win_cb, "Create", "Clear", "Dismiss", "Help...", 0, 3, 1, 2);
        ...
        XmxSetConstraints
            (win->ftpmkdir_text, XmATTACH_FORM, XmATTACH_NONE, XmATTACH_WIDGET, XmATTACH_FORM, NULL, NULL, label, NULL);
