...
static void make_write_iov
DTM_PROTO((IOV_BUF * iov, int fStartSeq, int fEndSeq, char *hdr, int hdrsize, VOIDPTR data, int datasize));
static int outp_count DTM_PROTO((DTMPORT * pp));
static int writev_buffer DTM_PROTO((DTMPORT * pp, IOV_BUF * iov_buf, int fStartSeq));
...
#endif
{
    ...
    FOR_EACH_OUT_PORT(pcur, pp) {
        if (pcur->connfd == DTM_NO_CONNECTION) {
            if (dtm_connect(&pcur->sockaddr, &pcur->connfd) == DTMERROR) {
    ...
    FOR_EACH_OUT_PORT(pcur, pp) {
        ...
        if (!(pcur->availwrite)) {
            ...
            if ((pp->qservice == DTM_SYNC)
                ...
                while (dtm_select(pcur->connfd, &tmp, 0) == TRUE && tmp >= 4);
...
#ifdef DTM_PROTOTYPES
static void make_write_iov(IOV_BUF *iov, int fStartSeq, int fEndSeq, char *hdr, int hdrsize, VOIDPTR data, int datasize)
#else
static void make_write_iov(iov, fStartSeq, fEndSeq, hdr, hdrsize, data, datasize)
IOV_BUF *iov;
int fStartSeq;
int fEndSeq;
char *hdr;
...
#endif
{
    ...
    FOR_EACH_OUT_PORT(pcur, pp) {
        ...
        status = dtm_writev_buffer(pcur->connfd, iov, iovlen, iovsize, NULL, 0);
...
#endif
{
    /* you didn't see this */
    int p;
    for (p = 0; p < DTMptCount; p++)
        if (pp == DTMpt[p])
            break;
    if (pp->porttype == INPORTTYPE && pp->XaddInput) {
        inp->XinputId = pp->XaddInput(inp->fd, XtInputReadMask, dtm_handle_in, (caddr_t) p);
    }
}
...
#endif
{
    ...
    while (TRUE) {
        ...
        if ((fd = dtm_accept(pp->sockfd, &pp->sockaddr, fWait ? &timeout : 0)) == DTMERROR) {
            if (!fWait)
                return DTM_OK;
            DTMerrno = DTMTIMEOUT;
            return DTMERROR;
...
*/
#ifdef DTM_PROTOTYPES
int DTMselectRead(Dtm_set *dtmset, int dtmnum, Sock_set *sockset, int socknum, int period)
#else
    ...
    do {
        ...
        for (p1 = dtmset, index = 0; index < dtmnum; index++, p1++) {
            ...
            if (pp->porttype == INPORTTYPE) {
                fNewConnections = fNewConnections || (select_one_connection(pp->sockfd) > 0);
                p1->status = DTM_PORT_NOT_READY;
            } else {
            ...
            if (p1->status == DTM_PORT_READY)
                fReady = DTM_PORT_READY;
        }

        /*      Check whether any socket has something to be read */

        for (p2 = sockset, index = 0; index < socknum; index++, p2++) {
            p2->status = FD_ISSET(p2->sockfd, fchk) ? DTM_PORT_READY : DTM_PORT_NOT_READY;
            if (p2->status == DTM_PORT_READY)
                fReady = DTM_PORT_READY;
        }
...
#endif
{
    ...
    while (iSent++ <= DTMSendCTSAhead) {
        ...
        if (!inp->fCTSsent && ((fWait && (iSent == 1)) || (dtm_select(inp->fd, &tmp, 0) == TRUE && tmp >= 4))) {
...
#endif
{
    ...
    while (dtm_read_buffer(inp->fd, &inp->blocklen, dtm_discard, DISCARDSIZE) > 0);
    inp->fCTSsent = FALSE;
...
*/
#ifdef DTM_PROTOTYPES
int DTMreadMsg(int p, char *hdr, int hdrsize, VOIDPTR data, int datasize, int datatype)
#else
...
#endif
{
    ...
    FOR_EACH_OUT_PORT(pcur, pp) {
        if (pcur->connfd == DTM_NO_CONNECTION)
            continue;
        if ((pp->qservice == DTM_SYNC && !pcur->availwrite) || !pcur->seqstart) {
            DTMerrno = DTMCALL;
            return DTMERROR;
        }
    }
...
*/
#ifdef DTM_PROTOTYPES
int DTMwriteMsg(int p, char *hdr, int hdrsize, VOIDPTR data, int datasize, DTMTYPE datatype)
#else
