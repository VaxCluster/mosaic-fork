...
*/
static int dtm_send_control DTM_PROTO((int fd, char *msg, int msize, char *sendto_addr));
...
#endif
{
    ...
    if ((status = dtm_writev_buffer(fd, iov, 1, 4, (struct sockaddr *)&sockaddr, sizeof(struct sockaddr_in))) < 0) {
    ...
    return dtm_writev_buffer(fd, iov, 1, msize, (struct sockaddr *)&sockaddr, sizeof(struct sockaddr_in));
}
...
#ifdef DTM_PROTOTYPES
int DTMsendRoute(int fd, char *sendto_addr, int addcount, char **add_addresses, int delcount, char **del_addresses)
#else
int DTMsendRoute(fd, sendto_addr, addcount, add_addresses, delcount, del_addresses)
int fd;                         /* output socket */
char *sendto_addr;              /* destination address addr:port number */
int addcount;                   /* number of addresses to connect to */
char **add_addresses;           /* array of addresses to connect to */
...
#ifdef DTM_PROTOTYPES
dtm_nsend_sockaddr(int fd, char *sendto_addr, char *refname, char *portname, S_ADDR *sockaddr)
#else
...
#ifdef _ARCH_MSDOS
    sprintf(mbuf, MREG, MREGID, refname, portname, inet_ntoa(inaddr.s_addr), ntohs(sockaddr->sin_port));
#else
    sprintf(mbuf, MREG, MREGID, refname, portname, inet_ntoa(inaddr), ntohs(sockaddr->sin_port));
#endif
    mbuf[MAX132 - 1] = '\0';

    DBGFLOW("dtm_nsend_sockaddr: Message:- ");
    DBGFLOW(mbuf);
    DBGFLOW("\n");
