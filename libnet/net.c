...
char *NetDTMErrorString(error)
{
    ...
    case DTMHUH:
        return (" (2) Unknown port definition ");
    case DTMNOPORT:
        return (" (3) No DTM ports available ");
    case DTMPORTINIT:
        return (" (4) DTM port not initialized ");
    case DTMCALL:
        return (" (5) calling routines in wrong order ");
    case DTMEOF:
        return (" (6) EOF error ");
    case DTMSOCK:
        return (" (7) Socket error ");
    case DTMHOST:
        return (" (8) That hostname is not found/bad ");
    case DTMTIMEOUT:
        return (" (9) Timeout waiting for connection ");
    case DTMCCONN:
        return (" (10) DTM cannot connect (network down?) ");
    case DTMREAD:
        return (" (11) error returned from system read ");
    case DTMWRITE:
        return (" (12) error returned from system write(v) ");
    case DTMHEADER:
        return (" (13) Header to long for buffer ");
    case DTMSDS:
        return (" (14) SDS error ");
    case DTMSELECT:
        return (" (15) Select call error ");
    case DTMENV:
        return (" (16) Environment not setup ");
    case DTMBUFOVR:
        return (" (17) User buffer overflow ");
    case DTMCORPT:
        return (" (18) Port table corrupted ");
    case DTMBADPORT:
        return (" (19) Port identifier is bad/corrupt/stale ");
    case DTMBADACK:
        return (" (20) Bad ack to internal flow control ");
    case DTMADDR:
        return (" (21) Bad address ");
    case DTMSERVER:
        return (" (22) Problem communicating with the server ");
    default:
...
static int DEFUN(NetUserListRemove, (name), char *name)
{
    int rc;
    return name ? (rc = ListDeleteEntry(userList, name), FREE(name), rc) : -1;
}
...
{
    if (user && userID && strcmp(userID, user) || !user && userID)
        NetUserListRemove(userID);

    if (user && userID && strcmp(userID, user) || !userID && user) {
...
int NetRegisterModule(name, netType, new, newData, change, changeData, destroy, destroyData)
...
char *header;
{
    ...
    if ((num = DTMreadDataset(n->port, primbuff, MAX_SDL_VERTICES, DTM_TRIPLET)) == DTMERROR) {
...
char *header;
{
    ...
    default:
        d->dost = DOST_Char;
        elementSize = 1;
        printf("VDATA of unknown type just received casting to char\n");
    };
    ...
    if (!(d->magicPath = (VdataPathElement **)
          MALLOC(sizeof(VdataPathElement *) * d->pathLength))) {
...
char *header;
{
    ...
    if (-1 == SDSgetDimensions(header, &rank, dims, sizeof(MAX_ARRAY_DIM))) {
    ...
    default:
        dostType = DOST_Char;
        printf("SDS of unknown type just received casting to char\n");
    };
    if (d = (Data *) DataSearchByLabelAndDOTAndDOST(title, DOT_Array, dostType)) {
        itsNew = FALSE;
    } else {
        itsNew = TRUE;
        if (!(d = DataNew())) {
    ...
    if (type == DTM_FLOAT || type == DTM_DOUBLE) {
        ...
        if (SDSgetMinMax(header, &d->min.f, &d->max.f) == DTMERROR)
            ...
            d->min.i = (int)d->min.f, d->max.i = (int)d->max.f;
    }
    if (d->associated)
        FREE(d->associated);
...
char *header;
{
    ...
    if ((!title) || (!strlen(title)))
        strcpy(title, "Untitled");
    if (d = (Data *) DataSearchByLabelAndDOTAndDOST(title, DOT_Array, DOST_Char)) {
        itsNew = FALSE;
    } else {
        itsNew = TRUE;
        if (!(d = DataNew())) {
    ...
    if (DTMreadDataset(n->port, d->data, xdim * ydim, DTM_CHAR) == DTMERROR) {
...
char *header;
{
    ...
    if (selType == COL_DOODLE_DISC) {
        if ((num = DTMreadDataset(n->port, triplet, MAXDRAWDOODLE, COL_TRIPLET)) == DTMERROR) {
...
char *header;
{
    ...
    case SRV_FUNC_DISCONNECT:
        ...
        SRVgetInPort(header, s.inPort, 80);
        sprintf(buff, "Just received a disconnect from\n(%s)\n", s.inPort);
        WriteMesg(buff);
        if (strlen(s.inPort)) {
            while (out) {
...
char *ExceptModuleName;
{
    ...
    if (d = DataSearchByLabelAndDOT((*dSend)->label, DOT_Array)) {
        ...
        if (d->dost == (*dSend)->dost)
            if (DimensionsEqual(d->dim, d->rank, (*dSend)->dim, (*dSend)->rank)) {
                ...
                FREE(d->data);
                d->data = (*dSend)->data;
                CopyDimensions(d->dim, &(d->rank), (*dSend)->dim, (*dSend)->rank);
            }
...
char *ExceptModuleName;
{
    ...
    if (d = DataSearchByLabelAndDOTAndDOST((*dSend)->label, DOT_Array, (*dSend)->dost)) {
...
char *ExceptModuleName;
{
    ...
    if (d = DataSearchByLabelAndDOTAndDOST((*dSend)->label, DOT_Array, DOST_Char)) {
        /* if this isn't the same dost, could be trouble */
        if (d->dost == (*dSend)->dost) {
            if (!DimensionsEqual(d->dim, d->rank, (*dSend)->dim, (*dSend)->rank)) {
...
NetPort *n;
{
    ...
    if ((length = DTMbeginRead(n->port, header, DTM_MAX_HEADER)) == DTMERROR) {
        sprintf(buff,
                "Error reading DTM header from port %s (%d) ret %d\nDTM error= %s\n",
                n->portName, n->port, length, NetDTMErrorString(DTMerrno));
        ErrMesg(buff);
#ifdef DEBUG
...
static int NetOpenPort(netPort)
NetPort *netPort;
{
    if (!netPort->open) {
        int out;
...
#ifdef USE_WRITEMSG
        if (DTMERROR == DTMwriteMsg(netPort->port, header, strlen(header) + 1, data, num, type))
#else
...
#ifdef DEBUG
    printf("Attempting to send \"%s\" to netPort=%x %x (%s)\n", header, netPort, netPort->port, netPort->portName);
#endif
...
*/
        ...
        fprintf(stderr, "port = %x (%s) ,sq->netPort=%x (%s)\n",
                port, port->portName, sq->netPort, sq->netPort->portName);
        fprintf(stderr, "*");
    }
#endif
    while (sq) {
        if (sq->netPort == port) {
            ...
            fprintf(stderr, ".");
#endif
            if (status = NetSend(sq->netPort, sq->header, sq->data, sq->num, sq->type)) {
                if ((status == 1) && (sq->cb))
...
*/
                if (t > (sq->netPort->queueTime + netTimeOut)) {
                    sprintf(buff,
                            ...
                            sq->header, sq->numTries, t - sq->netPort->queueTime);
                    ErrMesg(buff);
...
static void DEFUN(NetFreeDataCB, (p, client_data),
/* Free space after it has been sent */
                  GenericPtr p AND caddr_t client_data)
{
    FREE(p);
}

int NetSendDoodle(netPort, title, length, width, doodle, color, sendDiscrete, doQueue, distributeInternally, moduleName)
NetPort *netPort;
          ...
          /* in most cases this would be the calling module's name */
{
    ...
    if (distributeInternally) {
        NetCOLDistribute(NetMakeCOLFromDoodle(title, a, length + 1, sendDiscrete), moduleName);
    }

    status = NetClientSendMessage(netPort, header, a, (length + 1), COL_TRIPLET, NetFreeDataCB, 0, 0, 0, doQueue);
...
int x, y;
{
    ...
    if (!(p = (struct COL_TRIPLET *)MALLOC(sizeof(struct COL_TRIPLET)))) {
        ErrMesg("Out of Memory.  Can't send point select\n");
        return (0);
    }
    p->x = (float)x;
    p->y = (float)y;
    status = NetClientSendMessage(netPort, header, p, 1, COL_TRIPLET, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
...
int x1, y1, x2, y2;
{
    ...
    if (!(a = (struct COL_TRIPLET *)MALLOC(sizeof(struct COL_TRIPLET) * 2))) {
    ...
    status = NetClientSendMessage(netPort, header, a, 2, COL_TRIPLET, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
...
int x1, y1, x2, y2;
{
    ...
    if (!(a = (struct COL_TRIPLET *)MALLOC(sizeof(struct COL_TRIPLET) * 3))) {
    ...
    status = NetClientSendMessage(netPort, header, a, 3, COL_TRIPLET, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
...
char *title;
{
    ...
    if (!(a = (struct COL_TRIPLET *)MALLOC(sizeof(struct COL_TRIPLET)))) {
    ...
    status = NetClientSendMessage(netPort, header, a, (long)1, COL_TRIPLET, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
...
char *buf;
{
    ...
    status = NetClientSendMessage(netPort, header, a, length, COL_TRIPLET, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
...
int doQueue;                    /* TRUE -> Save and resend; FALSE -> let client resend */
{
    ...
    status = NetClientSendMessage(netPort, header, a, length, COL_TRIPLET, NetFreeDataCB, 0, 0, 0, doQueue);
...
char *moduleName;
{
    ...
    buff[t->dim] = '\0';
    status = NetClientSendMessage(netPort, header, buff, t->dim, DTM_CHAR, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
    return (status);
}

int NetSendPalette8(netPort, title, rgb, associated, distributeInternally, moduleName)
NetPort *netPort;
char *title;
unsigned char *rgb;
char *associated;
          ...
          /* in most cases this would be the calling module's name */
{
    ...
    status = NetClientSendMessage(netPort, header, p, 768, DTM_CHAR, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);

    return (status);
}

int NetSendArray(netPort, d, shouldCopy, distributeInternally, moduleName, isAnimation)
NetPort *netPort;
Data *d;
int shouldCopy;
int distributeInternally;
...
#endif
        status = NetClientSendMessage(netPort, header, buff, size, dtmType, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
    } else {
        status = NetClientSendMessage(netPort, header, d->data, size, dtmType, 0, 0, 0, 0, 1);
#ifdef DEBUG
        if (dtmType == DTM_FLOAT) {
            printf("\n\n\n\n###########\nThe first number is %f\n\n\n", *((float *)buff));
        }
...
DEFUN(_NetSendRaster8, (netPort, d, shouldCopy,
      ...
      NetPort *netPort AND
      Data *d AND int shouldCopy AND int distributeInternally AND char *moduleName AND char isAnimation)
{
    char header[DTM_MAX_HEADER];
    char *buff;
    int status;
    ...
    if (shouldCopy) {
        ...
        for (x = 0, pbuff = buff, pdata = d->data; x < buffSize; x++)
            *pbuff++ = *pdata++;
        status = NetClientSendMessage(netPort, header, buff, buffSize, DTM_CHAR, NetFreeDataCB, 0, NetFreeDataCB, 0, 1);
    } else
        status = NetClientSendMessage(netPort, header, d->data, d->dim[0] * d->dim[1], DTM_CHAR, 0, 0, 0, 0, 1);
...
char *moduleName;
{
    ...
    if (d->dot == DOT_Array)
        status = NetSendArray(netPort, d, shouldCopy, distributeInternally, moduleName, TRUE);
    else
        status = _NetSendRaster8(netPort, d, shouldCopy, distributeInternally, moduleName, TRUE);

    if (next && next->dot == DOT_Palette8)
        NetSendPalette8(netPort, next->label, next->data, dName, distributeInternally, moduleName);
...
int NetSendRaster8(netPort, d, shouldCopy, distributeInternally, moduleName)
NetPort *netPort;
Data *d;
...
char *moduleName;               /* Send internally to all DOCB except this one */
          /* in most cases this would be the calling module's name */
{
    return _NetSendRaster8(netPort, d, shouldCopy, distributeInternally, moduleName, FALSE);
}

int NetSendRaster8Group(netPort, title, charData, xdim, ydim, palette8, shouldCopy, distributeInternally, moduleName)
NetPort *netPort;
char *title;
          ...
          /* in most cases this would be the calling module's name */
{
    ...
    rasStatus = NetSendRaster8(netPort, d, shouldCopy, distributeInternally, moduleName);
    palStatus = NetSendPalette8(netPort, title, palette8, (char *)NULL, distributeInternally, moduleName);
...
int NetSendVData(netPort, label, magicPath, pathLength, nodeID, nodeName,
                 field, numRecords, numElements, type, vdata, shouldCopy, distributeInternally, moduleName)
...
char *moduleName;               /* distribute Internally to all except */
{
    ...
    if (shouldCopy) {
        ...
        status = NetClientSendMessage(netPort, header, copyData,
                                      numRecords * numElements, dtmType, NetFreeDataCB, 0, 0, 0, 1);
    } else {
        status = NetClientSendMessage(netPort, header, vdata, numRecords * numElements, dtmType, 0, 0, 0, 0, 1);
    }
          ...
          /* in most cases this would be the calling module's name */
{
    ...
    while (d) {
        ...
        switch (d->dot) {
        case DOT_Palette8:
            status = NetSendPalette8(netPort, d->label, d->data, d->associated, distributeInternally, moduleName);
            break;
        case DOT_Array:
            if (d->rank == 1) {
                d->rank = 2;
            ...
            if (d->rank > 3) {
                WarningMesg("Collage doesn't support greater than 3D data\n");
                /* should free data here? */
            }
            status = NetSendArray(netPort, d, shouldCopy, distributeInternally, moduleName, FALSE);
            break;
        case DOT_Image:
            if ((nextD)
                && (nextD->dot == DOT_Palette8) && !nextD->associated)
                nextD->associated = d->label;
            status = NetSendRaster8(netPort, d, shouldCopy, distributeInternally, moduleName);
            break;
        case DOT_Text:
            t.title = d->label;
            t.id = userID;
            ...
            t.dim = d->dim[0];
            t.textString = d->data;
            status = NetSendText(netPort, &t, distributeInternally, moduleName);
            break;
        case DOT_VData:
            status = NetSendVData(netPort, d->label, d->magicPath,
                                  d->pathLength, d->nodeID, d->nodeName,
                                  d->fields, d->dim[0], d->dim[1],
                                  d->dost, d->data, shouldCopy, distributeInternally, moduleName);
            break;
...
caddr_t failCBData;
{
    ...
    return (NetClientSendMessage(netPort, header, 0, 0, 0, NetExecCB, ecbd, NetExecFailCB, ecbd, 1));
}

int NetSendHostStatus(outPortAddr, retAddress, timeStamp, load1, load5,
                      load15, numUsers, cb, cbData, failCB, failCBData)
char *outPortAddr;
char *retAddress;
char *timeStamp;
...
caddr_t failCBData;
{
    ...
    return (NetClientSendMessage(netPort, header, 0, 0, 0, cb, cbData, failCB, failCBData, 1));
}
